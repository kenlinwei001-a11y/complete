 ...
 * comprehend 解析出 Process/Equipment 对象 + supplier_disruption_radius 求解器 + 降级规则 + B 栈倒推；而该求解器
...
const KIMI_OUTPUT: LlmComprehendOutput = {
  ...
  solverNeeds: [{ solverKey: "supplier_disruption_radius", inputFields: [{ typeKey: "Process", propKey: "procId" }, { typeKey: "Equipment", propKey: "equipId" }] }],
};
...
describe("§2 LLM comprehend（接 Kimi 解析任意故事）", () => {
  it("assemblePlanBody（纯函数）：LLM 三件 → 完整 plan（对象/规则/求解器 + B 栈倒推）", () => {
    ...
    expect(body.solverNeeds.map((s) => s.solverKey)).toEqual(["supplier_disruption_radius"]);
    // B 栈确定性倒推：每对象→切片；求解器→意图/计划/场景/agent
    expect(body.sliceNeeds.map((s) => s.sliceKey)).toContain("slice_process");
    expect(body.agentNeeds.map((a) => a.agentKey)).toContain("agt_supplier_disruption_radius");
    expect(body.intentNeeds[0]!.planRef).toBe("plan_supplier_disruption_radius");
    // 规则 scope 必须在已建对象内才保留
    expect(body.rules.map((r) => r.key)).toEqual(["R_DOWNGRADE"]);
  });

  it("绑定 Kimi（脚本化）→ 新颖故事经 LLM 倒推出 Process/Equipment + supplier_disruption_radius", async () => {
    ...
    expect(bp.solverNeeds.map((x) => x.solverKey)).toContain("supplier_disruption_radius");
    // 诚实：supplier_disruption_radius 系统未注册 → 自检暴露缺口（不空心、不谎报 ANSWERABLE）
