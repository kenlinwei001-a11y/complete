...
describe("generic_inference 通用 what-if 求解器（H · G-5 通用 what-if，工业级）", () => {
  it("注册：SOLVER_KEYS 含通用求解器（=46，含 DS.2 cockpit_kpi + 轨B·增量1 优化模板池 5 核心 + 增量3 optimize_whatif）+ 输出形状已声明（chain:check/SHAPE 覆盖）", () => {
    ...
    expect(SOLVER_KEYS.length).toBe(55); // WO-CEO-Q7 +1 supply_demand_gap_attribution；WO-ATP-PROMISE +1 atp_check；integ-wave-11 +4：WO-JOBSHOP-SCHEDULE job_shop_schedule + WO-CROSS-OBJECT-MULTIOBJ multi_objective/cross_object_occupancy + WO-A sop_reschedule；WO-PORTFOLIO-OPTIMAL +1 portfolio
