...
describe("generic_inference 通用 what-if 求解器（H · G-5 通用 what-if，工业级）", () => {
  it("注册：SOLVER_KEYS 含通用求解器（=46，含 DS.2 cockpit_kpi + 轨B·增量1 优化模板池 5 核心 + 增量3 optimize_whatif）+ 输出形状已声明（chain:check/SHAPE 覆盖）", () => {
    ...
    // ⚠ SOLVER_KEYS 55·WO-B base_capacity_outlook（54→55）与 WO-PORTFOLIO-OPTIMAL portfolio 双占 55·reviewer 合两条时 reconcile 到 56。
    expect(SOLVER_KEYS.length).toBe(55); // WO-CEO-Q7 +1 supply_demand_gap_attribution；WO-ATP-PROMISE +1 atp_check；integ-wave-11 +4；WO-B +1 base_capacity_outlook（前瞻产能推演 F1/P1）
