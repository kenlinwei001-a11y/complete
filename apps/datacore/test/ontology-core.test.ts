...
describe("generic_inference 通用 what-if 求解器（H · G-5 通用 what-if，工业级）", () => {
  it("注册：SOLVER_KEYS 含通用求解器（=45，含 DS.2 cockpit_kpi + 轨B·增量1 优化模板池 5 核心）+ 输出形状已声明（chain:check/SHAPE 覆盖）", () => {
    expect(SOLVER_KEYS.includes("generic_inference" as (typeof SOLVER_KEYS)[number])).toBe(true);
    // 轨B·增量1：40 → 45（facility_location/min_cost_flow/set_cover/independent_set/combinatorial_auction）。
    expect(SOLVER_KEYS.length).toBe(45);
