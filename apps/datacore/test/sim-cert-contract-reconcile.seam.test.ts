...
describe("WO-CERT-CONTRACT-RECONCILE ④ 传导相入参只有唯一装配处（tick 路与认证路同源）", () => {
  ...
  it("④ 结构判据：全仓只有一处装配传导相入参（新增第二处 = 两条路会静默分家）", async () => {
    ...
    // 事实锚（WO-C 修法）：唯一装配处**住在哪个文件**不是事实 —— 按「定义本尊」全树定位（搬家不红；
    // 定义被删/抄出第二份 ⇒ 下面的 toHaveLength(1) 当场红）。
    const dc = checkedTree("apps/datacore/src", "propagateTick", 150);
    const homes = factHits(dc, /export async function buildPropagationInputs(?!\w)/);
    expect(homes, "buildPropagationInputs 必须全树唯一定义").toHaveLength(1);
    const inputs = stripComments(readRepo(homes[0]!));

    // 金丝雀：先证明这个扫描确实能命中已知存在的东西（否则"0 命中"不可信）。
    expect(app.length, "app.ts 读到空内容 —— 路径漂了，先修路径再看结论").toBeGreaterThan(1000);
    // 金丝雀（原「app 里有 buildPropagationInputs」语义：确实存在调用方）：树级调用式探针（排除定义自身）。
    expect(
      factHits(dc, /(?<!function\s)\bbuildPropagationInputs\s*\(/),
      "金丝雀：没有任何调用方 ⇒ 扫描器坏了或装配处无人接线",
    ).not.toEqual([]);
    ...
    const propImports = [...app.matchAll(/import\s*\{([^}]*)\}\s*from\s*"\.\/sim\/propagation\.js"/g)]
      .map((m) => m[1] ?? "")
      .join(",");
    // 金丝雀：全树确实有文件 import 了传导核（否则下面判据 A 的"没 import 装配原语"是工具坏了，不是代码干净）。
    expect(
      factHits(dc, /import\s*\{[^}]*\bpropagateTick\b[^}]*\}\s*from\s*"[^"]*propagation\.js"/),
      "金丝雀：没有任何文件 import 传导核 ⇒ 抽取坏了",
    ).not.toEqual([]);
