...
describe("轨L 增量2 · demo 本体经真建模链（chainMode·provenance 因果真实）", () => {
  it("viaModelingChain → 34 类型由链 CREATE：sourceBindings 真、派生属性/中文名/域保留、obj id 字节同 A 路", async () => {
    ...
    // 85 类型全由链 CREATE（66 基线 + WO-CEO-2 供应链/地缘/决策域 5 类 + WO-CEO-3 TriggerRule 1 类
    ...
    // + WO-ATP-PROMISE 1 类：OrderPromise（订单承诺台账·commercial 域）
    // + WO-ORDERLINE 1 类：OrderLine（订单明细行·product 域·SO→型号行·一单多型号多行））。
    ...
    // WO-INVENTORY-3TIER 例外物化：WorkOrder（完工源）+ FinishedGoodsInventory + InventoryTxn 三层闭环落实例。
