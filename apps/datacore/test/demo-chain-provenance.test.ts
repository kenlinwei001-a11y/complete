...
describe("轨L 增量2 · demo 本体经真建模链（chainMode·provenance 因果真实）", () => {
  it("viaModelingChain → 34 类型由链 CREATE：sourceBindings 真、派生属性/中文名/域保留、obj id 字节同 A 路", async () => {
    ...
    //   + WO-LAST3-RELATIONS 行政区 1 类（Region·`located_in` 的锚点·13 省实例）→ 97。
    //     同 CapacityPool 一个路子：把**已经存在于三个载体上的** `province` 字符串换个承载，
    //     不是第二个真值 —— 行数由 Base/Warehouse/CustomerLocation 的 province 取值**并集**派生。
    //   + WO-CUSTOMER-GROUP 客户集团 1 类（CustomerGroup·`customer_belongs_to_group` 的锚点·16 集团实例）→ 98。
    //     同 Region 一个路子：把**已经存在于客户名册上的**归属换个承载，不是第二个真值 ——
    //     行数由 `CUSTOMER_REGISTRY.group` 取值去重派生（20 家客户 → 16 个集团：
    //     广汽系 3 家 / 长安系 2 家 / 上汽系 2 家 + 13 家各自单体）。
    expect(types.length).toBe(98);
  ...
  it("R6 确定性：viaModelingChain 同 seed 重跑 → 类型键集 + 对象 id 集字节一致", async () => {
    ...
    expect(a.types.length).toBe(98); // +WO-CUSTOMER-GROUP CustomerGroup（客户集团·`customer_belongs_to_group` 锚点·16 集团实例·97→98）；+WO-TIER3 GrossMarginBridge；+WO-ADOPT-MITIGATION AdoptedMitigation（零实例 → objs 计数不变）；+WO-SANDBOX Cadence（节拍·8 实例）；+WO-SANDBOX-D2 CustomsClearance/IncomingInspection（92→94）；+WO-RULE-SCOPE-TRIAD Outsource（94→95）；+WO-CAPACITY-EDGE CapacityPool（产能池·一线一池 130 实例·95→96）；+WO-LAST3-RELATIONS Region（行政区·`located_in` 锚点·13 省实例·96→97）
    ...
    // WO-ORDER-BOOK-500：订单簿 24→500（+476 Order）、拆行 38→873（+835 OrderLine）、
    // 客户名册 8→20（+12 Customer）、ATP 承诺 24→50（+26 OrderPromise）⇒ 对象总数 11337→12740。
    // 只改数字：本条守的仍是「同 seed 重跑对象 id 集字节一致」，见下一条 R6 断言。
    // WO-MATERIAL-REPRICE：碳酸锂行情 4 → 9 条（+5 CommodityPriceTrend）⇒ 12740→12745。
    // 只补实例、**类型集仍 95**（CommodityPriceTrend 早在册）。为什么要续：演习默认窗口 30 天，
    // 而行情只到 2026-06-22 ⇒ 窗口后半段无对照行情。续期后 `licarb_pct_cum` 14.29→18.45，
    // 两条阈值线（trig-backup-cert 默认 12 / C3 用例改写的 20）都仍在原来那一侧，见 decision-play.test.ts。
    // WO-WORKORDER-CATALOG-FIX：12745→12706（**−39，全部在 FinishedGoodsInventory：57→18**）。
    // 不是丢数据，是删掉了物理上不可能存在的成品库存：FG 行按 `FG-{modelId}-{基地成品仓}` 去重，
    // 而修前工单型号按 lineId 哈希选、不看基地产能 ⇒ 57 行里 23 行是目录外型号（储能-280Ah/314Ah）、
