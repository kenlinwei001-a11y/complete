...
describe("轨L 增量2 · demo 本体经真建模链（chainMode·provenance 因果真实）", () => {
  it("viaModelingChain → 34 类型由链 CREATE：sourceBindings 真、派生属性/中文名/域保留、obj id 字节同 A 路", async () => {
    ...
    expect(types.length).toBe(82); // WO-ORDERLINE：+1 OrderLine 订单明细行（81→82）
  ...
  it("R6 确定性：viaModelingChain 同 seed 重跑 → 类型键集 + 对象 id 集字节一致", async () => {
    ...
    expect(a.types.length).toBe(82); // WO-ORDERLINE：+1 OrderLine 订单明细行（81→82）
    expect(a.objs.length).toBe(3323); // WO-CEO-1a：+10 对象（7 顶层/细分 Metric + 3 细分业务线 Principal）；WO-CEO-2/3：+22 对象（长协/备份池/矿价趋势/决策缺陷/因果因素 + 触发规则；类型集 66→72）；WO-CEO-DATA-2：+35 对象（商业/财务域每指标因果 drill 实例；类型集 72→81）；WO-ORDERLINE：+48 对象（24 单拆行：10 单 1 行 / 4 单 2 行 / 10 单 3 行=48 行·首行保原单 model·其余取互异型号·Σ行 qty BY orderRef===单头 qty·独立哈希子流 hashString("oline_"+so) 不动 24 单头级基线；类型集 81→82）
  });
});
