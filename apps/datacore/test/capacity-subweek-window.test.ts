 ...
 * 导致「1天」与「1周」返回**字节相同**的结果（实测 p50 均 1.9642）——
 ...
 *   weeks=1 → p50 1.9642 ·  weeks=6 → p50 12.3016
...
interface CapOut { p50: number; p90: number; gap: number; perBaseRows: { cumTotal: number }[] }
...
describe("WO-SCENARIO-INPUT-PHASE0 收口 · 亚周窗口（1天 ≠ 1周）", () => {
  it("头号效果断言：weeks=1/7（1天）与 weeks=1（1周）结果**必须不同**（相同即痛点未修）", async () => {
    ...
    // 退单前：两者 p50 都是 1.9642（floor 抹平）。修后必须真不同。
    expect(day.p50, "1天的产能不得等于1周（旧 Math.floor 会抹平）").not.toBeCloseTo(week.p50, 3);
    expect(day.p50).toBeLessThan(week.p50);
    // 线性口径：1 天 ≈ 1 周 × 1/7（爬坡曲线同一周内不变 → 亚周为精确线性）
    expect(day.p50).toBeCloseTo(week.p50 / 7, 3);
  }, 180000);

  it("R6 不回归：整数窗口 windowScale 恒 1 → 与既有实测锚一致（weeks=1→1.9642·weeks=6→12.3016）", async () => {
    const t = await makeApp();
    await seedBattery(t);
    expect((await forecast(t, 1)).p50).toBeCloseTo(1.9642, 3);
    expect((await forecast(t, 6)).p50).toBeCloseTo(12.3016, 3);
  }, 180000);
  ...
  it("下界诚实：weeks=0 / 负数 / 缺省 → 夹到最小 1 天（1/7 周），不产生 0 产能或 NaN", async () => {
    ...
    for (const w of [0, -3, 0.001]) {
      ...
      expect(Number.isFinite(out.p50)).toBe(true);
      expect(out.p50).toBeGreaterThan(0); // 绝不返回 0 产能（比 floor→1 更糟的另一种坑）
      expect(out.p50).toBeCloseTo((await forecast(t, 1 / 7)).p50, 3); // 与下界同值
    }
  }, 180000);
  ...
  it("非整数多周：1.5 周落在 1 周与 2 周之间（旧 floor 会把它砍成 1 周）", async () => {
    const t = await makeApp();
    await seedBattery(t);
    const w1 = (await forecast(t, 1)).p50;
    const w15 = (await forecast(t, 1.5)).p50;
    const w2 = (await forecast(t, 2)).p50;
