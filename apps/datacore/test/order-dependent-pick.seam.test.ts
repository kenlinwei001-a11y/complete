...
describe("WO-ORDER-DEPENDENT-PICK · 「取数组第一条」一律换成「按判据选」", () => {
  ...
  it("A·L2 占位在种子期**原样落库**，解析发生在查询期（合并 WO-DYNAMIC-DRILL-RESOLVE 后的新不变量）", async () => {
    // ⚠ **本例在收编时被整条改写，原因必须写清，否则下一个人会把它改回去。**
    //
    // 原判据（本单交回时）断言的是「种子期动态绑定已解析成真对象」，金丝雀写着
    // `expect(capShort.drillId).not.toMatch(/^DYNAMIC/)`。同批并入的 WO-DYNAMIC-DRILL-RESOLVE
    // **把种子期解析整个删掉了** —— 占位原样落库，`solvers/dynamic-drill.ts` 在查询期按因子
    // 自己声明的 `drillPick` 解析。于是原金丝雀断言的是一个**已被有意删除的行为**，恒红。
    //
    // ⚠ 更要紧的是：那两张单**各自独立发现了同一个口径分歧**，而新实现正是它的解药 ——
    //   种子期 `Equipment.oee_current` **还不存在**（后续由 TS 聚合物化，`__prov.source="TS_AGGREGATE"`），
    //   所以旧代码只能拿**铭牌** `oeeA×oeeP×oeeQ` 顶替，冻下 `changzhou-formation-winding-E2`(0.769233)；
    //   而按因子自己声明的 `oee_current` 真正最差的是 `jinhua-slitting-winding-E1`(0.710781)。
    //   **冻在播种期 = 拿一个尚未成形的世界态当结论。** 移到查询期之后这条分歧自然消失，
    //   故本例改为守新不变量，而不是继续守那个错口径。
    //
    // A 类「并列裁决」这一档对这两个符号**已无对象可守**（`worstEquip`/`worstMbal` 随种子期解析
    // 一并删除）。同类加固仍在 `solvers/service.ts` 的 `worstMat`/`worstStale` 上，由 A·L1 守。
    // 查询期解析的正确性由 `dynamic-drill-resolve.seam.test.ts`（DR-1 对上率 / DR-2 对上的是不是同一张）守。
    const cfs = (await t.repos.objects.listByType(T, "CausalFactor")).map((o) => o.props);
    const capShort = cfs.find((c) => c.factorId === "cf-capacity-short")!;
    const matShort = cfs.find((c) => c.factorId === "cf-material-short")!;

    // ① 新不变量：种子里**必须**还是占位（这条与旧金丝雀方向相反，是有意的）。
    expect(String(capShort.drillId), "占位被提前解析了 ⇒ 又回到「拿尚未成形的世界态当结论」").toBe("DYNAMIC-EQUIP");
    expect(String(matShort.drillId), "占位被提前解析了 ⇒ 通配性丢失，其余 mbal 又会读成「无因子」").toBe("DYNAMIC-MBAL");

    // ② 口径由**数据声明**，不是引擎里的 if 链（R14）——因子自己写着查哪张表、按哪个字段挑。
    expect(capShort.drillPick, "口径没写在数据里 ⇒ 解析规则又散回引擎").toBeTruthy();
    expect(matShort.drillPick).toBeTruthy();

    // ③ 反面判据：解析**真的**发生在查询期 —— 归因树上读到的是真对象 id，不是占位。
    const g = await ga(t, "demand_attain");
    const drilled = (g.levels.find((L) => L.depth === 2)?.nodes ?? []).map((n) => String(n.provenance?.drillId ?? ""));
    expect(drilled.some((d) => d.startsWith("DYNAMIC")), "占位漏到了归因树上 ⇒ 查询期解析没跑").toBe(false);
    expect(drilled.some((d) => d.startsWith("mbal-")), "归因树上一个真物料落点都没有 ⇒ 上面断的是没人看的字段").toBe(true);
  }, 240000);
