 ...
 *  ② 复杂度敏感：多作用域类型 → workflow 含 resolve_slice；有入参 → 含 bind_args；skill.resources 非空（真作用域切片）；
 *  ③ agent.systemPrompt 含故事上下文（script 摘要 + 作用域对象）；
 *  ④ 沙盘划界（钉死）：绝不派生 propagation_rule/state_var（propagationRuleNeeds/stateVarNeeds 恒 []）；
 *  ⑤ R6：同 core 双跑字节一致。
 */
...
describe("WO-DB-BSTACK-DERIVE · B 栈按故事真派生（洞B·非模板）", () => {
  ...
  it("④ 沙盘划界：不派生 propagation_rule/state_var（恒空）", () => {
    const b = assemblePlanBody(complex, "复杂故事", 1, SOLVER_KEYS);
    expect(b.propagationRuleNeeds).toEqual([]);
    expect(b.stateVarNeeds).toEqual([]);
  });
