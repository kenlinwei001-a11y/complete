...
const passTrial = (): TrialTickInput => ({ passed: true, derivationNodes: 4, propagationCovered: false, at: "2026-01-01T00:00:00Z", error: null });
...
describe("增量2 · deriveCertification 纯投影", () => {
  ...
  it("trial FAIL（派生有环）→ 不进 L3，canEnterSimulation=false，gaps 含 TRIAL_TICK_FAILED ∧ FANOUT_UNSAFE", () => {
    const trial: TrialTickInput = { passed: false, derivationNodes: 0, propagationCovered: false, at: "t", error: "CYCLIC_DERIVATION: a → b → a" };
