...
describe("大数据量压测 · 1000 工序 / 10000 订单", () => {
  ...
  it("shared_bottleneck 在 10k 对象上正确求解 + 线性性能", async () => {
    ...
    await t.repos.ontologyTypes.put({ id: "ot_p", tenantId: "demo", key: "Proc", displayName: "工序", domain: "process", version: 1, status: "ACTIVE", derivedProperties: [], sourceBindings: [], properties: [{ propKey: "procId", dataType: "string", isPrimaryKey: true, unit: "dimensionless", scale: "absolute" }, { propKey: "capacity", dataType: "number", isPrimaryKey: false, unit: "dimensionless", scale: "absolute" }] });
