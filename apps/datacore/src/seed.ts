...
> = [
  ...
  //    确定性（R6）不受影响：全程零随机、零时钟。
  {
    ...
    reaction: {
      ...
      // **显式写死 `RULE_TABLE`，不靠 zod 默认值兜**：这一行是"本次还手的选择零 LLM 参与"
      // 这句话在**种子侧**的唯一出处。靠默认值 = 这件事在代码里一个字都看不见，
      // 而披露层却要据它对用户宣称"这是按规则算的"—— 那就成了没有出处的话。
      selectedBy: "RULE_TABLE",
  ...
  //
  // ── ⚠️ 2026-09-19 WO-DEMANDLOAD-BUDGET 订正：上面这段有一句**读起来像结论、实际是错的** ──
  //    原文「**在册口径读不到该字段**」**今天不成立**，照它去做会找错方向。
  //    **真起数据实测**（`seedBattery` 后 `repos.objects.listByType("demo","FinishedGoodsInventory")`
  //    逐条读 `o.props`）：`qtyAvailable` **18/18 行都有、且 18/18 为正**
  //    （样例 26838 / 27887 / 43961 / 10588；`qtyOnHand` 同样 18/18 为正）。
  //    🐤 金丝雀：同一把尺子量 `Order.qty` 得 **500/500 为正** ⇒ 量法是好的。
  //    ⇒ **读不到的不是「字段」，是「口径」** —— 字段就在对象上，缺的是一条
  //    「measure = 源.qtyAvailable，normalize = IN_EDGES」的在册口径。两件事修法完全不同：
  //    前者要补种子数据（做不了），后者只要加一个 registry 条目（做得了，只是不在本单范围里）。
  //    **形态**：「我用『在册口径读不到它』当作『这个字段读不到』的证据，而前者并不度量后者。」
  //
  // ── ✅ 本单落的改动：`null` → `equal_share`（Σw 从 N 归一到 1）──────────────────────
  //    ⛔ **不是为了让预算达标**（达不了标，见下）——是因为 `null` 在**这条边上**
  //    让种子自己声明的两条断言同时不成立，两条都能独立实测：
  //
  //    ① **「两次一样大」这条记账约定被破坏了，倍数恰好 = N。** 本段上方原文写着
  //       「同一笔量，来回记两次账，两次一样大」。实测：去程
  //       `demo_model_demand_to_fg_drawdown` 的 **Σw = 1**（`fg_of_model` 是 N:1，
  //       每行现货**有且只有一个**型号 ⇒ 组内只有一行）；回程本条 `weightRef: null`
  //       ⇒ 每行现货各加一份满额 ⇒ **Σw = 3**（18 行现货 ÷ 6 型号；`Nmax = 4`）。
  //       **去程记 0.6，回程记 1.8。**
  //    ② **「不会发散」这条稳定性断言今天是假的。** 本段上方原文写着
  //       「回路环增益 = 0.6×0.6 = **0.36 < 1**」—— 那个算式两侧都当 Σw=1 算，
  //       而回程真实是 3 ⇒ 真环增益 = `(0.6×1) × (0.6×3)` = **1.08 > 1**
  //       （现货最多的那个型号 N=4 ⇒ **1.44**）。**|环增益| > 1 正是本段上方
  //       「实测 60 拍 13.77/22.44/27.90/11.52/0/0/0/0/0/0/4.36/10.59」那串振荡的成因**——
  //       原文把它归给「1 拍延迟 + 下界硬地板」，那只解释了**为什么会回升**，
  //       没解释**为什么会冲过头**；冲过头需要的恰恰是 |环增益| > 1。
  //       归一到 Σw=1 之后，上面那句 0.36 才真的成立。
  //
  //    ⚠ **为什么本边用 Σ=1 不违反契约「广延型目标绝不能用 `equal_share`」那条警告**：
  //      那条警告的判据落在**源侧是不是彼此独立的相加单位**上，不是落在目标格子名字上。
  //      它点名保护的是 `demandLoad` 的**两条 Order 边**——500 张单是 500 笔独立需求，
  //      「单越多负荷越大」为真 ⇒ 必须 Σw=N。
  //      而本边的源是**同一个型号的库存被切成几行仓位**：同样一批货存在 1 个仓还是 3 个仓，
  //      被提走产生的需求缓解**必须一样大**。今天它恰好差 3 倍，这不是广延性，是**仓位行粒度**。
  //      同落点邻居 `demo_fg_cover_days_to_model_demand` 早已按同一条判据用 `equal_share`。
  //
  //    ⚠ **仍然缺的那一项，如实写在这里**：`equal_share` 只表示「三行仓位一样重」，
  //      而实测 `qtyAvailable` 在三行之间差到 **4.15×**（43961 vs 10588）——
  //      库存多的仓位该出更多缓解。升级路径是加一条
  //      `basis: "source_avail_qty_share"`（measure = `源.qtyAvailable`，normalize = `IN_EDGES`），
  //      落点在 `PAIR_WEIGHT_BASIS_REGISTRY`（`packages/contracts/src/sim.ts`）+ `pair-weights.ts`。
  //      **它改的是三行之间怎么分，不改本条要修的那个「合计是 1 还是 3」** ⇒ 预算读数一模一样，
  //      故不是本单的阻塞物。⛔ 不在本单范围内硬塞（契约包不在 🚦范围边界里）。
  // ══════════════════════════════════════════════════════════════════════════════════
