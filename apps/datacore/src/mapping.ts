...
const ACTION_TYPE_REG: MappingRegistries["actions"] = [
  ...
  // WO-U6-ADOPT：推演页「采纳结论」通用动作（optimize-whatif / cleanroom-attr / disruption-radius）。
  { name: "采纳推演结论", params: "来源页(source) / 判别字段(族·分析·断供根) / 推演快照(随 source 判别联合)", check: "payload 过 SimConclusionAdoptionPayloadSchema 契约 · 需含审批人(C10)", target: "SimConclusionAdoption 台账对象（写回）", perm: "发起:规划员 · 审批:生产计划部" },
