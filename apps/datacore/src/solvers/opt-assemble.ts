...
export async function assembleParetoModel(
  ...
  /**
   * WO-MULTIOBJ-CONVERGE：违约金报缺时「最近的落点」—— **现扫全本体**，不写死任何类型名（R14）。
   * 判据与订单侧那一格逐字相同（命中 `cost` 词库、不是强度量），只是换个类型去找 ——
   * 这样换个租户换套本体，这句话指的仍然是那个租户真实存在的字段。
   */
  ...
  const unavailableObjectives: ParetoObjectiveGap[] = [
     ...
     *
     * **今天的行为是 X**：本装配器只在订单侧命中「成本词库 ∩ 非强度量」时才声明 `penalty` 轴；
     * 接不到就**整根轴消失**，屏上什么都不说。而同一租户的另一个多目标界面
     * （多目标联合 WHAT-IF）屏上**有**一根违约金轴，读数由前端自定的
     * 「优先级 → 元/套」系数算出来 —— 那是客户端另造的口径，不是本体事实。
     * 两个界面于是对同一件事给出「有」与「没有」两个答案。
     * **应该是 Y**：接得到就是真轴，接不到就**显式报缺 + 说清最近的落点**，两个界面读同一份。
     *
      ...
      : [{
          ...
          reason:
            ...
            (penaltyElsewhere.length > 0
              ...
              : `全本体里没有任何命中该词库的数值字段。`) +
            `要补齐需先在本体上给出按订单（或按客户合同）计价的违约金额字段。`,
        }]),
    /**
     * ── WO-MULTIOBJ-CONVERGE · 换型成本（同上，且这一根卡在**族**上不是卡在字段上）──
     *
     * 换型成本是「上一个型号 → 这一个型号」这件事的代价，要先有**次序**才有换型事件。
     * 本族（`cross_object_occupancy`）是订单×产线×合同的**指派**问题，解里没有任何次序 ——
     * 一条线上排了哪几单是集合不是序列 ⇒ 换型次数无从起算。
     * 与「现金」那一根同源：卡在这个族没有时间/次序维，不是卡在某一格没绑上。
     */
    {
      key: "changeover",
      label: "换型成本",
