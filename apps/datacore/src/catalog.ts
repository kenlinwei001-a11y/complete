...
export const COCKPIT_SOLVER_CATALOG: CatalogItem[] = [
  ...
  { key: "metric_rollup", name: "经营指标卷算", description: "从对象库读 Metric 一等对象，对齐目标树(PlanTarget) target 算 delta/miss，输出 Metric[]（各视图 KPI 单一出处 R-一致，派生投影非新真值）。回答『各经营指标目标 vs 实际达成、哪些越线』。", argHints: { level: "层级(op/month/quarter/year，可选)" }, domain: "decision" },
];
...
/** A1 求解器全集目录（业务场景 22 + 通用 9 + 决策/骨架 2 = 33，与 SOLVER_KEYS 对齐；漂移由 catalog.test 守护）。 */
...
export class CatalogService {
  ...
  /**
   * A1：求解器全集注册表（业务场景 22 + 通用 9 + 决策/骨架 2 = 33）。供 AgentCore 构建 `solvers` MCP server 的
