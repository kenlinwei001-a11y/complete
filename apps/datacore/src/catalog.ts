...
export const SOLVER_CATALOG: CatalogItem[] = [
  ...
  // Q30-P3 求解器横铺 B（新域·中成本·DESIGN-query30 §1 P3 行）：3 新域（真读 SEED 对象派生）+ 2 复用类（真调子求解器）。
  { key: "cash_projection", name: "现金流投影", description: "由订单/回款/占用推现金曲线：真读基地财务账户期初现金 + 订单营收(量×价)/交期/毛利 + 客户账期(T+termDays 回款) → 逐周回款/付款净现金累积成现金曲线 + 13 周现金安全垫(最低点·C18 口径)。回答『未来 N 周现金流怎么走、安全垫最低多少、哪周最紧』。", argHints: { horizonWeeks: "投影周数(默认13)", baseId: "基地 ID(可选·限定单基地)" }, domain: "plan" },
  { key: "labor_balance", name: "人力平衡", description: "按工序/班次配工与缺口：真读班组编制(白/夜班 headcount/可借调/技能矩阵) + 订单派线需求(allocatedLineIds) → 逐产线需求人·班 vs 可用人·班的缺口(需求系数 laborPerWan 系估算参数·标 PARTIAL)。回答『各产线人力够不够、哪些线欠配、缺多少班』。", argHints: { laborPerWan: "人力需求系数 人·班/万套(默认取参数)", baseId: "基地 ID(可选)" }, domain: "plan" },
  { key: "energy_cost_schedule", name: "能耗成本排程", description: "分时电价×排程：真读能耗计量(单耗/电网碳因子) + 订单需求 → 逐基地能耗(kWh)/碳排(kg)逐周排程。⚠ 种子无分时电价(TOU)数据→电价须经 args.tariff 提供，缺失则成本诚实空缺不虚构峰谷价。回答『各基地能耗/碳排/(有电价时)成本怎么排』。", argHints: { tariff: "分时电价 {peak,flat,valley,shares?}(元/kWh·种子无 TOU 故须调用方供)", horizonWeeks: "排程周数(默认4)", baseId: "基地 ID(可选)" }, domain: "plan" },
  { key: "reroute_decision", name: "改道决策", description: "复用最小成本流(min_cost_flow·CP-SAT 真调)：断供/停线产线的产量改道到能产同型号(认证量产)且有余量的候选产线，建带容量/成本网络求最小成本流分配。arc 成本=真换型矩阵换型分钟(非魔数)。回答『某线停了产量改道到哪几条线、总改道成本多少』。", argHints: { lineId: "断供/停线产线 ID(缺省取首线)", seed: "随机种子(默认42)" }, domain: "plan" },
  { key: "multi_constraint_schedule", name: "多约束联合排产", description: "复用排产族三子求解器联合解(真调各子解·非各自为战)：sequencing_optimize(排序最优·CP-SAT) + changeover_sequence(换型最小·逐单换型分钟) + cert_schedule(认证排周·未量产阻塞窗) 三约束联合成一张既最优排序、又计真换型成本、又受认证就绪门控的排产表。回答『这批单怎么排既最优又少换型又不违认证』。", argHints: { jobType: "作业对象类型(默认 Order)", groupField: "换型分组字段(默认 model)", seed: "随机种子(默认42)" }, domain: "plan" },
];
