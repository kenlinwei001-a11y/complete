...
export const COCKPIT_SOLVER_CATALOG: CatalogItem[] = [
  ...
  /**
   * WO-METRICKEY-EMPTY-PROMISE · **本条目没有 argHints，这是结论不是遗漏——别再把 `metricKey` 加回来。**
   *
   * 原先声明 `argHints: { metricKey: "达成率指标 key(缺省用 S&OP 产销缺口)" }`，而
   * `solvers/service.ts supplyDemandGapAttribution()` 函数体内**零处读 args** ⇒ 传了既不报错也不生效，
   * 答案按缺省口径算出来、调用方以为是按自己指定的那个指标算的（**静默错答**：屏上没有任何东西能分辨这两种情形）。
   *
   * 为什么撤承诺而不是实现它（判据不是"哪个好做"）：
   *  · 被归因量 G = Σ SopVersionRow max(0, demand−supply)，量纲 **万套**；两端驱动（预测偏差/在手订单/结构漂移
   *    ⊥ 产能缺口/物料缺口/OEE 损失）也全是万套等效 —— 这套分摊只对「产销供需平衡」这一个口径成立。
   *  · 本体里**没有任何 Metric 以万套计量**（`GOAL_REGISTRY` 全域实测：营收/毛利/现金 = 亿，
   *    毛利率/份额/达成率/保障率 = %；细分 `seg_attain_*` = %）。拿一个 % 或 亿 的缺口去切万套驱动，
   *    得到的占比是**凭空造出来的口径**，不是这条链上任何真值派生得出的。
   *  · 也没有「指标 → 需求端/供给端驱动族」的登记册可依（`metric_causal_binding` 登记的是
   *    指标 → CausalFactor 因果根，服务于 `gap_attribution`，不是本求解器的两端驱动族）。
   *  · 且缺省值 `rootMetric.key = "sop_demand_supply"` **根本不是任何 Metric 对象的 key** ——
   *    原 argHint 那句「缺省用 S&OP 产销缺口」描述的东西压根不在 metricKey 的命名空间里。
   *
   * 想按别的指标看根因 → 走 `gap_attribution`（`argHints.metricKey` 在那里是**真读**的：
   * `service.ts` 的 `gapAttribution()` 以 `args.metricKey` 命中 `Metric.key/metricId`）。
   * 调用方若仍向本求解器传 `metricKey`，回包会在 `ignoredArgs` + `summary` 里**点名说它没生效**
   * （不静默吞、也不 400 打断既有 CEO 深问路 —— `ceo-route.ts` 的通用 else 支会无条件带上 focus.metric）。
   */
  { key: "supply_demand_gap_attribution", name: "供需失衡双向归因", description: "产销缺口(SopVersionRow Σmax(0,demand−supply))→**双向**分摊到需求端(预测偏差 Σ|P50−实际|/在手订单/结构漂移)⊥供给端(产能缺口/物料缺口/设备OEE损失)，两侧真颗粒驱动值各 Σ 按占比切缺口→需求端贡献+供给端贡献+residual=总缺口(硬勾稽≤1e-4)，各端下钻叶带 drillType/drillField/drillValue。改 DemandSegment.demandWanPerYearP50→需求端占比变·改 Equipment.oee_current/Line 产能→供给端变。回答『供需为什么对不上——是需求预测虚高还是产能/物料供不上、各占多少、每叶证据是什么』。**本求解器不吃任何入参**：被归因量固定为 S&OP 产销缺口(万套)，无法按指定指标归因(本体内无万套计量的 Metric)——要按指定指标看根因请改调 gap_attribution(metricKey=…)；传进来的参数会在回包 ignoredArgs 里被逐条点名忽略。", argHints: {}, domain: "decision", answersQuestions: ["供需为什么对不上", "需求预测虚高还是供不上", "各占多少"], tags: ["supply-demand", "gap", "attribution", "forecast"] },
  ...
  { key: "base_capacity_outlook", name: "每基地前瞻产能推演", description: "给定基地，按 30/60/90 天窗口前瞻推演四线对比：可用产能(Σ Line.capacityDaily×(1−util/100)×窗口) vs 在产订单占用(未完工 WorkOrder.qtyActual 铺窗) vs 未来订单(Order.due 落窗 Σqty·首基地=本基地) vs 销售预测(ΣDemandSegment.demandWanPerYearP50×1e4 按产能占比摊窗) → 缺口/富余标记 + 累计需求越线日(crossDay)。缺口窗给逐日行动过程 dayPlan(触发缺口→加班/跨基地/外协贪心补→收窄·每步 provenance)。改 Order.due/DemandSegment.demandWanPerYearP50/Line.capacityDaily 颗粒→前瞻真变。forecastStart 时间锚(确定性 R6)·每线/每日值可溯(R13)·系数 RuleEntry.params(R14)。回答『这个基地未来 30/60/90 天产能够不够、缺口哪天出现、逐日怎么处置』。", argHints: { baseId: "基地 ID 或名称(必填·如 hefei/合肥)", horizon: "窗口天数(可选·默认 30/60/90 全出)" }, domain: "plan", answersQuestions: ["未来产能够不够", "缺口哪天出现", "前瞻 30/60/90 天", "逐日怎么处置"], tags: ["capacity", "outlook", "forecast", "前瞻", "产能"] },
];
...
export const GENERIC_SOLVER_CATALOG: CatalogItem[] = [
  ...
  // ── WO-ARGHINTS-12-LOUD · 下面这批 argHints 原先声明的键**实现根本不读** ────────────────────
  // 目录是**模型看得见的那一份**（agentcore `tools/executor.ts` 把 argHints 原样放进 discover 的
  // items 交给 LLM）。声明 A 而实现要 B ⇒ 模型照 A 生成参数 → **400 响亮失败**（不是静默错答，
  // 是「照说明书做做不通、白跑一趟」）。故一律改成**实现真读的键全集**，并标 必填/可选 + 缺省值。
  // 逐条证据（求解器实现方法 · 抛 400 的判据行）见各条行末注释；口径与
  // `apps/datacore/test/solver-arghints-per-solver.test.ts` 的逐求解器断言同源。
  { key: "shared_bottleneck", name: "共享瓶颈", description: "读对象图，按 viaField 把上游对象分组到共享资源，需求和>产能即瓶颈，按优先级判哪张单降级。净室通用。", argHints: { resourceType: "必填·共享资源对象类型（被挤占的那一方，如 Line/Equipment）", sharedByType: "必填·挤占该资源的上游对象类型（如 Order）", viaField: "必填·上游对象上指向共享资源的字段", capacityField: "可选·资源产能字段（缺省 capacity）", demandField: "可选·上游需求量字段（缺省 qty）", priorityField: "可选·优先级字段（判哪张单降级；不给则按需求量序）" }, domain: "generic", answersQuestions: ["哪些对象挤同一共享资源", "共享资源产能不够谁降级", "隐性共享瓶颈在哪"], tags: ["共享瓶颈", "shared", "bottleneck"] }, // 实现 SolverService.sharedBottleneck；缺三个必填 → 400「shared_bottleneck 需 resourceType/sharedByType/viaField」
  { key: "concentration_risk", name: "隐性集中度", description: "多跳反向聚合，沿暗线找单点集中（看似分散实则汇聚到同一上游）。净室通用。", argHints: { startType: "必填·起点对象类型（「看似分散」的那一层，如 Customer）", path: "必填·多跳路径 [{viaField,toType}]（沿暗线一跳跳走到终端根）", minDependents: "可选·判定集中的最小依赖方个数（缺省 2）" }, domain: "generic", answersQuestions: ["看似分散实则集中到哪个上游", "单点集中风险在哪", "暗线是否汇聚到同一来源"], tags: ["集中度", "concentration", "单点集中"] }, // 实现 SolverService.concentrationRisk；缺必填 → 400「concentration_risk 需 startType + path:[{viaField,toType}]」
  { key: "margin_attribution", name: "毛利倒挂归因", description: "成本项拆解 + 倒挂群主驱动聚合，定位毛利倒挂的根因成本项。净室通用。", argHints: { targetType: "必填·成本承载对象类型（毛利算在谁头上，如 Order）", costFields: "必填·成本项 [{field,label?}]（拆哪几笔成本）", revenueField: "可选·收入字段（缺省 revenue）", marginThreshold: "可选·倒挂判定阈值，毛利率低于它即倒挂（缺省 0＝亏本）" }, domain: "generic", answersQuestions: ["毛利为什么倒挂", "哪个成本项拖垮了毛利", "毛利倒挂的根因成本项是哪些", "成本项怎么拆解定位倒挂", "倒挂群的主驱动成本是什么"], tags: ["margin", "毛利倒挂", "成本项", "cost", "attribution"] }, // 实现 SolverService.marginAttribution；缺必填 → 400「margin_attribution 需 targetType + costFields:[{field,label?}]」
  ...
  // ⚠ 下面四个 `*_optimize` 的入参是**对象类型键 + 字段名**（求解器自己去对象图取候选项），
  //   **不是**直接喂数组 —— 原先声明的 `items`/`bins`/`jobs` 会让模型去构造一个没人读的数组。
  { key: "selection_optimize", name: "组合最优化", description: "通用 0/1 选择最优化（CP-SAT 可证最优）：预算约束下选价值最大子集。贪心给不出最优时用。候选项由求解器按 itemType 从对象图读取（不需调用方喂数组）。", argHints: { itemType: "必填·候选项对象类型（求解器按它读对象图取候选集）", budget: "必填·预算上限（Σweight 的上界，数值）", valueField: "可选·价值字段（缺省 value）", weightField: "可选·重量/耗用字段（缺省 weight）", maxCount: "可选·最多选几项", minValue: "可选·总价值下限", seed: "可选·求解种子（缺省 42·确定性）" }, domain: "generic", answersQuestions: ["预算内怎么选价值最大子集", "0/1 选择最优组合", "有限预算下最优选哪些"], tags: ["组合最优化", "selection", "cp-sat"] }, // 实现 SolverService.selectionOptimize；缺必填 → 400「selection_optimize 需 itemType + budget（+ valueField/weightField）」
  { key: "assignment_optimize", name: "指派最优化", description: "通用指派最优化（CP-SAT 可证最优）：把待办项指派到容器/基地，最小化总成本，满足容量约束。待指派项与容器均由求解器按类型键从对象图读取。", argHints: { itemType: "必填·待指派项对象类型（如 Order）", binType: "必填·容器对象类型（如 Base/Line）", weightField: "可选·待指派项占用量字段（缺省 weight）", capacityField: "可选·容器容量字段（缺省 capacity）", costField: "可选·每项的指派成本字段，对所有容器同成本（缺省 cost）", seed: "可选·求解种子（缺省 42·确定性）" }, domain: "generic", answersQuestions: ["待办怎么指派到容器最省成本", "容量约束下最优指派", "指派最小化总成本"], tags: ["指派最优化", "assignment", "cp-sat"] }, // 实现 SolverService.assignmentOptimize；缺必填 → 400「assignment_optimize 需 itemType + binType（+ weightField/capacityField/costField）」
  { key: "sequencing_optimize", name: "排序最优化", description: "通用排序最优化（CP-SAT 可证最优）：把作业按 groupField 排序，最小化相邻组之间的换型次数。作业集由求解器按 jobType 从对象图读取；换型矩阵默认「不同 group 计 1」，不需调用方喂矩阵。", argHints: { jobType: "必填·作业对象类型（如 Order/WorkOrder）", groupField: "可选·换型分组字段，相邻不同组即一次换型（缺省 group）", seed: "可选·求解种子（缺省 42·确定性）" }, domain: "generic", answersQuestions: ["作业顺序怎么排切换成本最小", "最短切换路径序列怎么求", "排序最优化"], tags: ["排序最优化", "sequencing", "cp-sat"] }, // 实现 SolverService.sequencingOptimize；缺必填 → 400「sequencing_optimize 需 jobType（+ groupField）」
  { key: "packing_optimize", name: "装箱最优化", description: "通用装箱最优化（CP-SAT 可证最优）：按容量把项装入最少容器（产能填充/批次合并）。待装项由求解器按 itemType 从对象图读取。", argHints: { itemType: "必填·待装项对象类型", binCapacity: "必填·单箱容量（数值）", sizeField: "可选·待装项尺寸字段（缺省 size）", seed: "可选·求解种子（缺省 42·确定性）" }, domain: "generic", answersQuestions: ["怎么用最少容器装完", "批次合并怎么最优", "装箱最优化"], tags: ["装箱最优化", "packing", "cp-sat"] }, // 实现 SolverService.packingOptimize；缺必填 → 400「packing_optimize 需 itemType + binCapacity（+ sizeField）」
  ...
  { key: "facility_location", name: "选址最优化", description: "通用选址最优化（CP-SAT 可证最优）：在开设成本与服务成本下选最优设施集并分派需求点。经本体绑定喂任意行业（仓/店、诊所/社区…）。", argHints: { facilities: "必填·候选设施数组 [{id,openCost,capacity?}]", clients: "必填·需求点数组 [{id,demand?}]", assignCosts: "必填·服务成本数组 [{client,facility,cost}]（成本矩阵按对展开，不是二维矩阵）", seed: "可选·求解种子（缺省 42·确定性）", facilityType: "可选·设施对象类型名（仅用于回包标注，不影响求解）", clientType: "可选·需求点对象类型名（仅用于回包标注，不影响求解）" }, domain: "generic", answersQuestions: ["在哪开设施服务成本最低", "选址加分派需求点", "最优设施集怎么选"], tags: ["选址", "facility-location", "cp-sat"] }, // 实现 SolverService.facilityLocation；缺 assignCosts → 400「字段 'assignCosts' 需为数组」（asArr），空数组 → 400「facility_location 需 facilities[] + clients[] + assignCosts[]」
  ...
  { key: "set_cover", name: "集合覆盖", description: "通用集合覆盖最优化（CP-SAT 可证最优）：用最小成本子集覆盖全部元素。", argHints: { sets: "必填·候选子集数组 [{id,covers:[元素id],cost?}]", universe: "可选·待覆盖元素集 [元素id]（不给则取所有 sets[].covers 的并集）", seed: "可选·求解种子（缺省 42·确定性）", setType: "可选·子集对象类型名（仅用于回包标注，不影响求解）" }, domain: "generic", answersQuestions: ["用最小成本子集覆盖全部元素", "集合覆盖最优化", "最少子集覆盖怎么选"], tags: ["集合覆盖", "set-cover", "cp-sat"] }, // 实现 SolverService.setCover；缺 sets → 400「字段 'sets' 需为数组」（asArr），空数组 → 400「set_cover 需 sets[]」
  ...
  { key: "combinatorial_auction", name: "组合拍卖", description: "通用组合拍卖赢家裁定（CP-SAT 可证最优）：在物品不重复分配约束下最大化中标价值。拍卖物品集由 bids[].items 的并集推得，不单独传。", argHints: { bids: "必填·投标数组 [{id,value,items:[物品id]}]（物品集从这里推得）", seed: "可选·求解种子（缺省 42·确定性）", bidType: "可选·投标对象类型名（仅用于回包标注，不影响求解）" }, domain: "generic", answersQuestions: ["组合拍卖赢家怎么裁定", "物品不重复分配最大化价值", "组合出价怎么中标"], tags: ["组合拍卖", "auction", "cp-sat"] }, // 实现 SolverService.combinatorialAuction；缺 bids → 400「字段 'bids' 需为数组」（asArr），空数组 → 400「combinatorial_auction 需 bids[]」
