...
export const PROPERTY_UNITS = [
  ...
  "元/kWh",
  "元/吨",
  // WO-DIMENSION-ERRORS · **钱的速率**（分母是时间，不是「套」⇒ 不触 R-UNIT 裁决）。
  // `DemandSegment.revenueWan/marginWan` 的真实量纲：`万套/年 × 万元/套 = 1e8 元/年`。
  // 改前声明 `亿元`（**存量**），而它每一份都是**年度流量** —— 阶错，不是倍数错：
  // 存量与速率之间没有换算系数，把「一年赚 700 亿」读成「账上有 700 亿」是两个不同的量。
  "亿元/年",
  ...
  "电芯/天",
  "GWh/年",
  // WO-DIMENSION-ERRORS · **物料日耗速率**的四个解析目标。`Material.dailyUse` 改前声明 `吨`
  // （既是**族错** —— 8 料里 3 料按 ㎡/L/个 计量，压根不是质量；又是**阶错** —— 名字自陈
  // 「日耗」是速率，却声明成存量）。这四条是 `计量单位/日` 按 `Material.unit` 逐行解析的落点，
  // 缺一条那一行就解析不出（`resolveParametricUnit` 返回 undefined ⇒ 该行量纲未知）。
  "kg/日",
  "㎡/日",
  "L/日",
  "个/日",
  ...
  "元/计量单位",
  "计量单位",
  // WO-DIMENSION-ERRORS · 参数化**速率**：分子由同对象另一格给出，分母是「日」。
  // `Material.dailyUse` 的真实形态 —— 逐行是 `kg/日` / `㎡/日` / `L/日` / `个/日`。
  "计量单位/日",
] as const;
