 ...
 * 种子世界开局推几拍。
 *
 * ── ⚠ 2026-09-16 由 3 改 96（WO-SIM-DESAT-3 ④）。旧依据**不是错的，是不完整的** ───────
 *
 * 旧注释的三条（下面原样保留，它们仍然成立）说的是「世界**看得完整**要几拍」：
 *  · 1 拍：只有即时边到达，延迟边的贡献还挂在 `pending` 里，屏上看不见；
 *  · 2 拍：延迟边落地，世界第一次"完整"；
 *  · 3 拍：`metric-series` 有 4 个点（t0..t3），一条线才**有形状**可读。
 *
 * 它答不了另一个问题：**「世界自己还在动多大」**。而扰动对比读的是**差**，
 * 自漂移只要还高过视图层 0.01 地板，三条臂（0/1/12 件扰动）就会读出同一批订单 ——
 * 用户加多少扰动都盖不过世界自己的余波。**「世界完整了」不度量「世界安静了」。**
 *
 * `96` 的依据同样是实测（`docs/evidence/wo-sim-desat2/floor.mjs`，本单复跑确认）：
 * 零扰动世界自己再推 3 拍，看有多少**未完成单**被推过 0.01 地板 ——
 *
 *      K      3    6    9   12   18   24   36   48   72   96   160
 *   过地板   150  150  150  150  150  150  150  136   40    0     0
 *   订单 p90 8.4e1 … 6.8e-1 … 3.2e-2  1.4e-2  8.1e-3  2.9e-3
 *
 * ⇒ **K=48 时 136/150 张单仍被自漂移标成"被推动"**；96 拍才整体落到地板以下。
 * ⚠ 而**全域漂移格数在 96 拍仍是 3751（不是 0）** —— 世界没有被推成一潭死水，
 *   只是它的余波小于"用户扰动才算数"的那条线。这两件事必须分开量，
 *   否则会把「动静做没了」当成「安静了」。
 *
 * ⚠ **这个数只有在 ①②③ 一起落地之后才成立**：修前世界收敛到一个**饱和的不动点**
 * （压力族均值/越界格数从第 12 拍起逐字节不动，而 `Line.blockedPressure` 作为无界积分器
 *  还在一路长），那时推多少拍自漂移都下不来 —— 前人读到的「12 拍就不动了」正是
 *  **饱和把灵敏度压到 0 之后的残影**，不是收敛。
 *
 * 代价：种子时间与库体积随拍数线性拉长（3 → 96 拍）。这是为「扰动读数可信」付的价。
 */
export const DEMO_SIM_WORLD_TICKS = 96;
