 ...
 *   ⚠ 这 8 个**不许**顺手补域：`chain-loss.ts` 的 `SIM_DAY_STATE_VAR_BY_CARRIER` 是天数族的
 *   **真消费方**（把"这一拍积了几天"叠进链损），给它夹个 100 就是把业务量纲改小；
 *   而 `Supplier.leadDays` 那一路刻意允许负值（在制单可低至 −14 = 逾期 14 天），夹到 0 会算错。
 *
