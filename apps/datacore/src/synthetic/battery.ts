...
export const PROPAGATION_COEF_PARAMS: Record<string, number> = {
  ...
  // ✅ **2026-09-19 WO-DEMANDLOAD-BUDGET 补回负号**：上面这段裁决写着「负号即方向…应当下修」，
  //    而这一行**一直是正的** —— 边的行为与它自己的裁决、与下发到用户屏上的 description
  //    （种子原文「变更频度 × **−0.5** = 需求负载下修量」）**方向相反**。
  //    **病因不是有人手抖**：`seed.ts` 头注那条闭式分配法写的是
  //    `稳态增益_e = |意图增益_e| × f_g` —— **取了绝对值**。全表 4 个负系数里，
  //    另外三个（forecast_bias −0.222 / cover_days −0.185 / alt_switch −0.111）的 `f_g` 都 = 1
  //    ⇒ 没进那条乘法，负号得以保留；**本条是唯一一个既被缩放（f_g≈0.023231）又本该为负的**，
  //    所以是唯一的受害者。|量级| 0.011615 一个数都没动，改的只有符号。
  //    ⚠ **它为什么此前没被任何门咬住**：守它的 `sim-root-triad.seam.test.ts` G-ROOT-2
  //    当时**读数恒 0**（同落点的库存缓冲边 `weightRef: null` ⇒ Σw=N=3 ⇒ 环增益 1.08 > 1，
  //    把 `Model.demandLoad` 直接打到下界地板 0，四跳全是 0.0000）。
...
const modelProps: PropertyDef[] = [
   ...
   * ⚠ 收编订正（WO-INTEG-BATCH-5）：本单原注写「`UNIT_DICTIONARY` 今日只有 9 个词条」——
...
const lineProps: PropertyDef[] = [
  ...
  //
  // ── 收编裁决（`WO-DIM-LABEL-3` × `WO-UNIT-KWH` 并线冲突·2026-08-29）──────────────
  //   两张单都在补这两条同名属性的量纲声明，`unit` 取值不同（见下面 Process 那条）。
  //   本行两边一致（都判 `%`），故只把 DIM-LABEL-3 的 `description` 并进来 ——
  //   注释是给读代码的人看的，`description` 是**随契约下发给消费方**的，两者不互相替代。
  {
    propKey: "utilization",
    dataType: "number",
    isPrimaryKey: false,
...
const processProps: PropertyDef[] = [
  ...
  //
  // ── 收编裁决（`WO-DIM-LABEL-3` × `WO-UNIT-KWH` 并线冲突·2026-08-29）──────────────
  //   两张单在**本行**判得不一样，取 UNIT-KWH 的 `dimensionless`，不取 DIM-LABEL-3 的 `%`：
  //     · DIM-LABEL-3 写 `unit:"%"`，而它自己的描述写的是「**0–1 比率存储**，显示时 ×100」——
  //       在 UNIT-KWH 建立的闭合口径里，`%` 的含义是「**存的就是 0–100 百分点**」
  //       （`Line.utilization` 那条才是），拿它标一个 0–1 的值就是把量纲说反了 100 倍，
  //       正是这两张单都想灭掉的那个病。
  //     · UNIT-KWH 的判据来自**真起后端实测**（SEED_DEMO=1，读数 0.903/0.951/0.957），
  //       且 `scale:"ratio"` 已经承载了「这是比例量、显示要 ×100」这层意思，不必再借 `%` 表达。
  //     · 另有一条硬约束：`PropertyDef.scale` 在 UNIT-KWH 之后是**必填**，
  //       DIM-LABEL-3 那一版没有 `scale`，原样取用 typecheck 当场红。
  //   `description` 是 DIM-LABEL-3 的真实增量（随契约下发给消费方，注释下发不到），予以保留，
  //   仅把其中「与 `%` 同口径」的说法改成与本行实际声明一致。
  {
    propKey: "utilization",
    dataType: "number",
    isPrimaryKey: false,
...
function withGovernance(key: string, props: PropertyDef[]): PropertyDef[] {
  // WO-INTEG-BATCH-5 收编取舍：`wo-unitcost-land` 在此处补过一格 `Model.unitCost: "元"`，
  // 但集成线已把整张 `units` 覆写表**退役**（量纲改必填后单位在构造点声明，类型系统兜底）。
  // 故此处保留集成线的「已退役」态，该单的意图改由 `modelProps.unitCost` 的
  // `unit: "元", scale: "absolute"` 在构造点承接 —— 单一出处 > 并存。
