...
export const BUILTIN_VIEWS: BuiltInView[] = [
  ...
  // WO-IA-E2E5E6 仓主改名：title（导航/页标题命名空间）改「接单可行性」；
  // WO-AGENTCORE-RENAME-TAIL：featureName（功能名册命名空间）同翻「接单可行性」——
  // 与 SHARED_FEATURE_NAMES + AgentCore/frontend-mock 受检副本四方同批原子翻（前单挂账的互锁由此解锁）。
  ...
  // 全局项目推演（portfolio 求解器·全订单×全基地×时间联合最优）：renderer/solver 均已就绪，此前 scenarioSeed 漏接致内存态重启后 404。
  { key: "plan-generate", title: "规划建议", renderer: "plan-generate", featureKey: "view.plan-generate", featureName: "规划建议", seed: true, sim: true, bindings: { intents: ["plan_generate_*"], solverKeys: ["plan_generate"], apiTags: ["plan-generate"] } },
  { key: "project-sim", title: "接单可行性", renderer: "project-sim", featureKey: "view.project-sim", featureName: "接单可行性", seed: true, sim: true, bindings: { solverKeys: ["capacity_forecast"], intents: ["capacity_*"] } },
  ...
  { key: "global-sim", title: "接单组合优选", renderer: "global-sim", featureKey: "view.global-sim", featureName: "接单组合优选", seed: true, sim: true, layout: { solverKey: "portfolio" }, bindings: { solverKeys: ["portfolio"] } },
...
/* ══ 推演沙盘指控台四视图（WO-SIM-BE-VIEWKEY）══════════════════════════════════════
 *
 * ── 病灶（实测坐实，非推测）─────────────────────────────────────────────────────
 * 前端 `views/registry.ts:68/69/70/72` 逐字注册了四个渲染器
 * （`sim-console` / `sim-conduction` / `sim-attribution` / `sim-optimize`，
 * 组件全在 `views/sim/console/`），而**后端整个 src 零命中** ——
 * 三形态定性（铁律 0.5）＝ **「没接线」**，不是"接了线没数据"也不是"接错地方"。
 * 金丝雀：同一条 grep 在 `chain-line-map`（同族已上屏的沙盘子视图）上命中本文件多行
 * ⇒ 工具是好的，是真没下发。
 ...
 * 走**增量视图桶**（`service.ts` 的 `VIEW_DEFS` + 本表）三件全避开，而判据⑦ 照样闭合：
 * 它的供给侧之二**明写**着「或 `service.ts` `VIEW_DEFS` 项的 renderer 字段等于该 key」。
 * 这不是绕道，是仓里既有的第二条正路 —— `process-stuck` 走的就是它
 * （见 `service.ts` `DARK_LAUNCH_EXTRA_KEYS` 与 `ShellLayout.CONSOLIDATED_INTO_SANDBOX` 的 `via:"view-defs"`）。
 *
 * ── 为什么控制键直接取 `sim.sandbox`（照抄，不自定）─────────────────────────────
 ...
 * 两者的**实际判据是同一个键**。故本表不新增功能键，直接把四个 viewKey 映到 `sim.sandbox`
 ...
 * ⇒ **目录无条件收录，闸在请求期判**。
 *
 * ⚠ `options` 里**没有** `sessionId`：会话是运行期的，写进 workspace 配置就成了死值
...
export interface SandboxConsoleView {
  ...
  title: string;
  /** 前端 renderer 注册键（本族与 key 同名，显式写出以免"默认相等"这个隐含假设失守）。 */
  renderer: string;
}
