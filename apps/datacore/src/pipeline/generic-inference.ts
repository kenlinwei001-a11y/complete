 ...
 * 契约（输入/输出）就近定义于此，前端不消费，不进共享 contracts。
 */

// ---- 输入/输出契约（本地，zod + inferred）----------------------------------

export const GenericInferenceChangeSchema = z
  .object({
  ...
  .refine((c) => c.delta !== undefined || c.setValue !== undefined, {
    message: "change 需提供 delta 或 setValue",
  });
...
export interface ChangedProp {
  ...
  before: unknown;
  after: unknown;
  via: string; // "direct" | `derived:${key}` | `link:${key}`
}
...
export interface GenericInferenceOutput {
  deterministic: true;
  changed: ChangedProp[];
...
function recomputeOwnDerived(obj: ObjectInstance, t: InferenceTypeDef, changed: ChangedProp[]): boolean {
  let any = false;
