...
import { SANDBOX_CONSOLE_FEATURE_KEY, SANDBOX_CONSOLE_VIEW_KEYS } from "./synthetic/view-manifest.js";
...
export async function buildApp(deps: AppDeps): Promise<BuiltApp> {
  ...
  const withRouteFeatureAliases = (feats: string[]): string[] => {
    ...
    // 它们是同一个控制台的四种模式，共用沙盘那把闸，见 view-manifest `SANDBOX_CONSOLE_VIEWS`），
