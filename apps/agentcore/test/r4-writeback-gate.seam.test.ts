...
describe("WO-R4-FREEQA-GATE · 写回型技能的人工批复闸：两条路径同一把锁", () => {
  ...
  it("【防回潮】两条 loadSkill 路径必须回报同一份治理口径（差集为空）", async () => {
    ...
    const orch = readFileSync(join(SRC, "router/orchestrator.ts"), "utf8");

    // 金丝雀：一个**已知必在**两处的符号。它若不中 ⇒ 是读文件/匹配坏了，不是代码干净。
    expect(engine).toContain("loadSkill: async (skillId: string) =>");
    expect(orch).toContain("loadSkill: async (skillId: string) =>");

    // 治理位回报的唯一构造器必须在两条路径上各出现一次（谁内联抄第二份口径，这里红）。
    expect(engine.match(/skillGovernance\(skill\)/g)?.length).toBeGreaterThanOrEqual(1);
    expect(orch.match(/skillGovernance\(skill\)/g)?.length).toBeGreaterThanOrEqual(1);
  });
});
