/**
 * WO-DSH-E2E · §16.2 L1 双跑字节比对（62 任务）driver。
 *
...
describe("WO-DSH-E2E · §16.2 L1 双跑字节比对（62 任务）", () => {
  it("A0 语料自检：构成 / 四维覆盖 / gated 槽在册", () => {
    expect(DUALRUN_CORPUS.length).toBe(62);
    expect(new Set(DUALRUN_CORPUS.map((t) => t.id)).size).toBe(62);
