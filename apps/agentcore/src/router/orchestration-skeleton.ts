 ...
 * 真实 QueryTask 轨迹经 projectTrace（§4.3）为各节点填充 status/data/solvers/agents/gapCode。
 *
 * 血缘诚实（R14·簇F 治本）：节点"用到的数据/求解器/agent"**只能**来自真实 trace（task/plan/toolCalls）。
 * 骨架**不再**内联任何业务电池常数（型号/工序/求解器/agent 名，如 4680/化成/聚合求解器/瓶颈诊断Agent）
 * 作为 data/solvers/agents 占位——真 lineage 缺失时节点显空（"本次未记录来源"），绝不冒充真血缘。
 * in/proc/out 为**视图定义级 IPO 文案**（描述该步职责的通用叙述，非某次运行的真值），保留。
 *
