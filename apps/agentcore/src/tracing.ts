/**
 * WO-OBSERVABILITY (OBS-2) · 全链 OTel span 树初始化（AgentCore）。
 *
...
export const SERVICE_NAME = "agentcore";
 ...
 * 使下游服务（AgentCore→DataCore 或反向）续同一 trace。与 x-request-id 双轨并存（不破 AUDIT-OBS）。
 */
...
// 模块 import 即起 SDK（幂等）——必须先于 http/fastify/pg 被 require（见 main.ts 第一行 import "./tracing.js"）。
initTracing();
