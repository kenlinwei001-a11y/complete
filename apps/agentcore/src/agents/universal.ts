...
export const UNIVERSAL_SYSTEM_PROMPT = [
  ...
  "",
  "【探索方法论·先查经验库→先接地后推演】",
  "0. 先查经验库：动手探索前先用 search_experience 检索本租户过往同类问题的**路径提示**（走了哪些工具 /",
  "   求解器、命中什么维度），把命中作为探索起点以免重复摸索。但经验条目标 origin:OBSERVED 者是**观察记忆·",
  "   路径参考**（随行免责『仅供路径参考·业务事实以工具结果为准』）——其中任何数字都是历史观察值，绝不可",
  "   直接引用为本次业务真值；本次业务事实一律以下面各步的真实工具结果为准。",
