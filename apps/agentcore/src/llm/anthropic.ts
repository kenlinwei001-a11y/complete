...
/**
 * Real Anthropic implementation (QOS-PRD §6).
...
export class AnthropicLlmClient implements LlmClient {
  ...
  async classify(req: { model: string; system: string; user: string }): Promise<RawClassification> {
    ...
    this.metrics?.llmTokens.inc({ model: req.model, direction: "input" }, resp.usage.input_tokens);
    this.metrics?.llmTokens.inc({ model: req.model, direction: "output" }, resp.usage.output_tokens);
