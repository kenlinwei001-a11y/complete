...
export async function createPgRepos(databaseUrl: string): Promise<Repos> {
  ...
  return {
    ...
    planBuilders: {
      ...
      async get(id) {
        const r = await q(
          `SELECT id, tenant_id, package_id, key, version, status, name, description, dsl, compiled_plan_id, created_by, created_at, updated_at FROM plan_builder_canvases WHERE id = $1`,
          [id],
        );
        const row = r.rows[0];
        if (!row) return undefined;
