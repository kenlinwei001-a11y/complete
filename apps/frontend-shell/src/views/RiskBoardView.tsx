                          ...
                          <tr data-testid={`risk-plan-detail-row-${i}`} className={styles.rkPlanDetailRow}>
                            <td id={detailId} colSpan={PLAN_COLUMNS.length}>
                              ...
                              {/* WO-DECISION-INFO-FE ③ · 多方案与代价（A/B/C + 成本 + 副作用 + 前置期 R13）：
                                  ...
                                  决策者要的是「有哪几种办法、各要付什么代价」——那一份在 planRows[].options 里。
                                  ⚠ 收编裁决：fe-batch 原本把这两块挂在 `</table>` **之后**，而 canonical 已由
                                  WO-DISPOSITION-INLINE-ROW 把详情改成「紧跟被点那一行」的 `<tr><td colSpan>` 展开行。
                                  原样并入会出现**两份** DispositionDetailPanel（一份行内、一份表外），
                                  且表外那份正是那张单修掉的老形态。故只取 fe-batch 真正新增的 OptionsPanel，
                                  与详情同处展开行内。 */}
