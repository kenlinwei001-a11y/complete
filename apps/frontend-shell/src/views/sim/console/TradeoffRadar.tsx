 ...
 * ⚠ **WO-CONSOLE-BLOCKERS 订正**：上一版取 11（×0.994 ⇒ 屏上 **10.9px**），并写着
 * 「≈ 地板」。**那不是地板** —— 本仓判据 B 的硬底是 **12px**（`FLOOR_PX`），10.9 在它之下。
 * 上一版给的不加码理由是「再往上六根轴的中文标签会互相压住」，这条**当时没有量过**，
 * 只是判断。本单**真量了**：把它抬到 12（屏上 11.9px）后，在 1600×900 视口下逐对算
 * 标签外接盒，重叠对数 = 0（量法见 scratchpad 的 `measure.mjs` radarOverlap 一节）。
 * ⇒ 取 **12**。若日后容器变窄真压住了，正确的修法是**减轴或换布局**，
 * 不是把字缩回读不清 —— 「读不清」和「读不出」都是不可用，前者还多一层"看起来能用"的欺骗。
 */
const LABEL_FS = 12;
