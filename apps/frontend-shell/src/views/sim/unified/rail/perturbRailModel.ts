 ...
 * `startTick: 0` 的扰动 POST 回 201、`appliedPerturbations` 里**还带着它的 id**，
 * 而卡墙上的数**一个都不动** —— 引擎读输入、也算得对，只是这条扰动从来没被落地过。
 *
 ...
 *
 * ── 实测取值域 ────────────────────────────────────────────────────────────────
 * ⚠ **2026-08-29 第二次实测复核·本表已按实测重写**（前一版的数与病因都有偏差，
 *    照它复验会得出「这个修法没有依据」的相反结论 —— 详见本节末「复核订正」）。
 *
 * 取证条件（每一格都可复现）：真后端 `SEED_DEMO=1`、种子世界 `sims_demo_seed_world`
 * （`curTick = 3`）、**每个取值重启一次 datacore**（内存模式 ⇒ 世界字节级一致）、
 * 落点 `obj_model_4680-NCM.supplyRisk`、施加后 `POST …/tick {n:1}`（推到第 4 拍）。
 * 读数口径**逐字复刻卡墙**：`metric-series?limit=500` 回包里同一 stateVar 取 |Δ| 最大的那行
 * （`metricWallModel.pickRepresentative`），读它 `actual` 的末格。
 *
 *   | startTick | POST | 卡墙 supplyRisk | 落盘世界线 vs 回放 |
 *   |-----------|------|-----------------|--------------------|
 *   |  −1       | **400 VALIDATION_ERROR**（契约 `int().min(0)`） | 5496.22（＝未施加） | — |
 *   |   0       | 201  | **5496.22 ＝ 不施加任何扰动的读数，逐字节相同** ⇒ 彻底不生效 | 落盘会动、**卡墙不动** |
 *   |   1       | 201  | 102916.46（set 口径） | ❌ 落盘 102916.46 / 回放 106006.75 |
 *   |   2       | 201  | 同上                  | ❌ 落盘 102916.46 / 回放 105756.15 |
 *   |   3(=cur) | 201  | 同上                  | ❌ 落盘 102916.46 / 回放 104835.51 |
 *   |   4(=cur+1)| 201 | 102916.46             | ✅ **两者相等，gap = 0**（唯一全对的取值） |
 *   |   ≥5      | 201  | 5496.22（＝未施加）   | 入库但本次推不到 ⇒ **屏上与 `0` 长得一模一样** |
 *   | 不发该字段 | 201  | 与 `3` **逐字节相同** | 后端默认 `body.startTick ?? s.curTick` = 3 |
 *
 * ── 🔴 复核订正（这一条本身就是本仓「错误病因比没有病因更危险」的实例）────────────
 * 前一版把「两条真相源打架」写成**与 mode 无关**的性质。**实测不成立**：
 *  · `mode: "delta"` ——「加/减一个量」与施加顺序无关 ⇒ startTick **1/2/3/4 四档全部
 *    落盘＝回放**（gap 均为 0 或 1e-11 量级的浮点噪声）。**照 delta 复验，这条理由一条都验不出来。**
 *  · `mode: "set"` ——「把这一格设成 X」与顺序**强相关**（先设后传导 ≠ 先传导后设）⇒
 *    才出现上表 1/2/3 那三档的 gap（−3090.29 / −2839.69 / −1919.05），
 *    且 gap 随 startTick 逼近 cur+1 **单调收敛到 0**。
 * ⇒ 结论（`curTick + 1`）**成立且未改**，但**理由必须带上 mode 这个前提**。
 *   我自己第一遍就是用 `delta` 量的，差点据此判定「前任的理由不成立、该改回 curTick」——
 *   形态：**「我用『delta 口径下两源一致』当作『两源从不打架』的证据，而前者并不度量后者。」**
 *
 * ── 为什么默认取 `cur+1` 而不是派单书写的 `cur`：**派单自己的验收数就是 cur+1 那一档** ──
 * 派单要求「Δ shortageRisk = Δ supplyRisk × 0.8，误差应为 0」。逐档实测（mode `set`，
 * 幅度 50000 → 99999）：
 *   · `startTick = 4 (=cur+1)`：supplyRisk 52917.46 → 102916.46（Δ **49,999.00**）、
 *     shortageRisk 41307.472 → 81306.672（Δ **39,999.20**）⇒ 比值 **恰好 0.8，误差 0** ✅
 *   · `startTick = 3 (=cur)`  ：Δ supplyRisk 同为 49,999.00，而 Δ shortageRisk = **79,998.40**
 *     ⇒ 比值 **1.6 = 0.8 × 2**（扰动多传导了一拍）❌ 不满足验收式
 * 且派单书引用的两个数（`102,916.46`、`52,917.46`）**本次原样复现**，它们正是 `cur+1` 那一档的读数。
 * ⇒ 措辞与读数冲突时**以读数为准**（读数是实测，措辞是转述）。改回 `curTick` 只需改本文件一行，
 *   全仓没有第二处算它 —— 但那会让派单自己的验收判据当场变红。
 *
 * ── 应该是 Y ─────────────────────────────────────────────────────────────────
