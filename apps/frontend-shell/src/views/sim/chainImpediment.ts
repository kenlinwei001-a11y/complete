 ...
 *                  → 真结论。但**该修什么由引擎给的定性决定**，⛔ 不是一概「数据面缺杠杆」：
 *                    · 不是当前瓶颈 ⇒ 多给它本来就不提产，业务上就该如此，**没什么可修的**
 *                    · 这根杠杆不进产能公式 ⇒ 缺的是模型里那一项，补数据换瓶颈都没用
 *                    · 够不着落点 ⇒ 才是「补落点册」那一类
...
export const CANDIDATE_ABSENCE_LABEL = {
  NONE: {
    label: "查过了 · 确实没有",
    claim:
      "枚举已跑完：join 走到了、档位取到了、逐候选都真试算过了，结论就是这些杠杆都拨不动这个读数。" +
      "这是一个真结论，不是没算 —— 但「拨不动」的原因不止一种（不是当前瓶颈 / 这根杠杆不进产能公式 / 够不着落点），" +
      "逐条原因见下面那行定性，三者修法完全不同。",
  },
  UNAVAILABLE: {
    label: "算不出来 · 不是没有",
