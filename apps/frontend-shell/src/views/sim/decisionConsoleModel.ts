 ...
 * 算法可单测，视图只负责摆位置。
 *
 * ══ 开工前的实测复现（派单要求的「今天的行为是 X，应该是 Y」）· 2026-08-28 ═══════
 ...
 *
 * ⚠ 一条 2026-08-28 实测踩到的坑（复验：打 `POST /a/v1/solvers/invoke` 取
 * `chain_impediment` 回包，找 `自贡分容线` 那条读 evidence 原值）：
 * `自贡分容线` 是 `95.358 vs 95`，四舍五入后屏上写成
...
// ══════════════════════════════════════════════════════════════════════════
// § 3 · 事件顺序线（**标题不许写「传导时间轴」** —— 2026-08-28 实测最长累计 2 天，那张图是假的；
//        复验：打 `POST /a/v1/sim/drill/preview` 把各段 lagDays 加起来看总跨度）
// ══════════════════════════════════════════════════════════════════════════
 ...
 *
 * 2026-08-28 实测会踩到这条的原文（`finance_world_projection.notes[0]`）：
...
/**
 * ⚠ WO-CONSOLE-BLOCKERS · **B2 后半：`（（` 双层括号**（UX 第 7 轮 2026-08-28 实测，屏上原文抄下来是
