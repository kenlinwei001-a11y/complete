...
export const NAV_GROUPS: { title: string | null; collapsed?: boolean; items: NavItemRef[] }[] = [
  ...
  // 「运营复盘」(`review`) 已于 2026-09-12 **整屏删除**（仓主原话：「删除它，以后需要再建新的」）。
  //
  // ⚠ 这里记一笔**我上一版记错的账**：09-11 那版写的是「只摘入口，不删屏也不删组件」，
  //   结果 `review` 仍被后端下发、前端仍渲染得出，却没有任何导航组收它 ⇒ 掉进「其它」折叠兜底桶。
  //   `f61.admin-nav-groups` 当场咬红（「可达 ≠ 可发现」），而我当时报的是「导航测试 RC=0」——
