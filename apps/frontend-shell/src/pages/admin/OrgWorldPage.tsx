          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 12 }}>
            ...
            <div>
                ...
                <tbody>
                  {chart.persons.map((p) => (
                    <tr key={p.principalId} data-testid={`org-person-${p.orgKey}`}>
                      <td>
                        <b>{p.name}</b>
                        <span className="muted" style={{ fontSize: 10.5, marginLeft: 4 }}>{p.title}</span>
                      </td>
              ...
              {!canWriteAvailability && (
                <div className="muted" style={{ fontSize: 10.5, marginTop: 4 }} data-testid="org-availability-readonly">
            ...
            {(authQ.data?.authorities ?? []).map((a) => {
              ...
              return (
                <tr key={a.authorityKey} data-testid={`org-authority-${a.authorityKey}`}>
                  <td>
                    <b>{a.name}</b> <span className="mono muted" style={{ fontSize: 10.5 }}>{a.authorityKey}</span>
                  </td>
                  ...
                  <td style={{ fontSize: 11 }} data-testid={`org-limits-${a.authorityKey}`}>
                    {ls.length === 0 ? (
          ...
          <div data-testid="org-resolution">
            <div style={{ marginBottom: 6 }}>
              ...
              {resolution.diagnosis && (
                <span style={{ marginLeft: 8, fontSize: 11.5 }} data-testid="org-resolution-diagnosis">{resolution.diagnosis}</span>
              )}
            </div>
              ...
              <tbody>
                {resolution.eligible.map((c) => (
                  <tr key={`${c.principalId}:${c.authorityKey}`} data-testid={`org-eligible-${c.orgKey}`}>
                    <td><b>{c.name}</b> <span className="muted" style={{ fontSize: 10.5 }}>{c.title}</span></td>
              ...
              <tbody>
                {resolution.blockers.map((b) => (
                  <tr key={b.authorityKey} data-testid={`org-blocker-${b.authorityKey}`}>
                    ...
                    <td style={{ fontSize: 11 }}>{b.reasons.join("；")}</td>
                  </tr>
                ))}
        ...
        <tbody>
          {rows.map((p) => (
            <tr key={p.principalId} data-testid={`org-node-${p.orgKey}`}>
              <td>
                <b>{p.name}</b>
                <span className="mono muted" style={{ fontSize: 10.5, marginLeft: 4 }}>{p.orgKey}</span>
              </td>
              <td style={{ fontSize: 10.5 }} className="muted">
