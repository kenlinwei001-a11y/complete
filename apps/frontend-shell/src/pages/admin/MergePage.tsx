        ...
        return (
          <div key={c.id} className="panel" data-testid={`merge-cand-${c.id}`} style={{ marginBottom: 10 }}>
            ...
            <table className="cmp" style={{ width: "100%" }}>
              <thead>
                <tr><th>字段</th>{c.objects.map((o) => <th key={o.id} className="mono" style={{ fontSize: 10.5 }}>{o.id}</th>)}</tr>
              </thead>
              <tbody>
                {allKeys.map((k) => (
                  <tr key={k}>
                    <td style={{ fontSize: 11, color: "var(--muted2)" }}>{k}</td>
                    {c.objects.map((o) => <td key={o.id} className="mono" style={{ fontSize: 11 }}>{String(o.props[k] ?? "—")}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
            <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ fontSize: 11.5 }}>选 golden：</span>
              {c.objects.map((o) => (
        ...
        <tbody>
          {mergeItems.map((m) => (
            <tr key={m.id} data-testid={`merge-rec-${m.id}`}>
              ...
              <td className="mono" style={{ fontSize: 11 }}>{m.mergedIds.join(", ")}</td>
              <td>{m.mergedBy}</td>
              <td style={{ fontSize: 11 }}>{m.mergedAt.slice(0, 19).replace("T", " ")}</td>
