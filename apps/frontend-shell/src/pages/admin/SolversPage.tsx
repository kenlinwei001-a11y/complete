...
function SolverImpactPanel({ solverKey }: { solverKey: string }) {
  ...
  return (
    <div style={{ padding: 8, display: "flex", flexDirection: "column", gap: 8 }} data-testid={`solver-impact-${solverKey}`}>
      <div>
        <b style={{ fontSize: 11.5 }}>改它会波及谁（引用反查）</b>
        {refs.isError ? (
          <div className="muted" style={{ fontSize: 11.5 }} data-testid={`solver-refs-error-${solverKey}`}>
            这次没查出来（编排服务不可达）——**不等于**没人引用，别据此放心改。
          </div>
        ) : refs.isLoading ? (
          <div className="muted" style={{ fontSize: 11.5 }}>查询中…</div>
        ) : (
          <div style={{ fontSize: 11.5 }} data-testid={`solver-refs-${solverKey}`}>
      ...
      <div>
        <b style={{ fontSize: 11.5 }}>在本租户本体里绑到哪（A13 地板语义 · 确定性无 LLM）</b>
        {roles.isError ? (
          <div className="muted" style={{ fontSize: 11.5 }} data-testid={`solver-roles-error-${solverKey}`}>
            这次没查出来（后端不可达）——不是「没有绑定」。
          </div>
        ) : roles.isLoading ? (
          <div className="muted" style={{ fontSize: 11.5 }}>解析中…</div>
        ) : roleKeys.length === 0 ? (
          <div className="muted" style={{ fontSize: 11.5 }} data-testid={`solver-roles-none-${solverKey}`}>
            这个求解器不吃角色（后端 SOLVER_FIELD_ROLES 未为它声明）——不是解析失败。
          </div>
        ) : (
          <div style={{ fontSize: 11.5 }} data-testid={`solver-roles-${solverKey}`}>
            {roleKeys.map((role) => (
              <div key={role}>
