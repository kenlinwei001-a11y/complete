 ...
 *
 * ⚠ **收编批次4 改名**：本函数在 WO-CAUSAL-EDGE-CRUD 里原名 `updatePropagationRule`，
 *   与 WO-ONTOLOGY-EDGE-EDIT 的同名函数（`PUT /:id`·整条覆盖）撞名。两条后端路由都在册、
 *   用途不同（PUT 改身份格与说明；PATCH 改单格数值与启停），故**两个都留**，本条改名区分。
 *
 ...
 */
export const patchPropagationRule = (
  id: string,
  patch: Partial<{
    coefficient: number;
    delayTicks: number;
