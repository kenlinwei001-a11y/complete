...
// ── WO-BEFE-WIRE-3 · 上面三条读/写之外，`fork` 与 `diff` **本单接线前**零前端调用方 ──────
// （2026-08-10 实测，门 `befe-seam:check` 载体②：`POST /a/v1/twin/enterprise-states/*/fork` 与
//  `GET /a/v1/twin/enterprise-states/*/diff` 在并集态第一次被照出来。
//  复验：`node scripts/check-backend-frontend-seam.mjs`。
//  ⚠️ 时态要紧：本单落地**之后**这两条已有调用方（下面这几个导出 + `EnterpriseStateTwinPanel.tsx`），
//  所以"零调用方"只在接线前成立 —— 不写时态就是一句会过期的假话。
//  真消费方 = `views/sim/EnterpriseStateTwinPanel.tsx`，挂在推演沙盘右栏「快照分叉与比对」。）
