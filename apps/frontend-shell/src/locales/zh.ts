...
export const zh = {
  ...
  admin: {
    ...
    agents: {
      ...
      console: {
        ...
        kernelExternalTip:
          "本次运行由外部运行时执行（POC 休眠分叉，部署面缺省关闭）。环检测护栏待补 —— 未销账期如实标注；销账后本句移除（N3 销账尾差，跨单回执在案）。",
        /**
  ...
  decisionPlay: {
    ...
    evidence: {
      ...
      typeWord: "依据只对上类型 · 弱",
      typeHint:
        "根因树上确实有同一类对象，但**不是这条方案指的那一个**。两边对「哪个对象算证据」并不一致，" +
    ...
    plays: {
      ...
      vsOptionsBody:
        ...
        "比对矩阵的「每列最优」和决策台账当**真**用（台账写着选了 A、实际执行 B，界面上分辨不出）。" +
        "所以两者各自成区，刻意不混一张表。",
  ...
  processWait: {
    ...
    crosslink: {
      ...
      derivedNote: (n: number) =>
        `另有 ${n} 条实例此刻同样卡着，但它们是**从单据反推**的，单据上没有「第几步」这个事实，` +
        `无法归属到具体站 —— **不计入**下面各站的计数。要看这一批：/v/process-stuck 页顶部那条说明。`,
      /** 从实例层跳回时的行定位标记。 */
