...
export const zh = {
  ...
  risk: {
    ...
    noCross: "未越线",
    /**
     * WO-RISKBOARD-TRUNCATION ·「越线但未上榜」诚实位（后端仅在**真被截断**时下发，故整块可缺席）。
     * 与 `scopeHonesty.sampling`（"算了几张" ≠ "算的是谁"）同一个命题：榜上这几张的正确读法是
     * 「越线的 N 个里排最前的几个」，不是「全网只有这几个越线」。不显示这个数，榜本身就是在误导。
     * ⚠ 屏上不许出现源码文件名/字段名/排期语汇（R-UI-4）——下面每一句都只说业务事实。
     */
    unlisted: {
      chip: (n: number) => `另有 ${n} 个基地已越线未上榜`,
      title: "越线但未上榜的基地",
      topic: "为什么有基地越线却不在榜上",
      expand: "展开查看",
      collapse: "收起",
      ...
      colFactor: "首要因素",
      colPeak: "峰值",
      dayNo: (d: number) => `第 ${d} 天`,
      /** 结论性读法（第一层·不进浮层）：把"榜上几个"与"越线几个"两个数并排摆出来。 */
      reading: (shown: number, total: number) =>
        `榜上 ${shown} 个是已越线的 ${total} 个里排最前的 ${shown} 个 —— 不是全部。`,
      why: "看板容量有限，按「越线日↑ → 当前张力↓ → 峰值↓」只取前几名。排在后面的基地同样已经越线，"
        + "处置不能因为它没出现在榜上就停 —— 一个基地掉出榜可能只是比上一名低了零点几个张力点，不是它不越线了。",
    },
  ...
  sim: {
    ...
    sandbox: {
      ...
      impedimentRow: {
        ...
        severityWhy:
          "这个数说的是「这条卡点**现在**有多严重」，两个因子相乘再开根号做保序缩放：" +
