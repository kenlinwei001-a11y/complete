...
import { OUTSOURCE_REDLINE, outsourceRedlineConstraintExpr, outsourceRedlinePct, outsourceRedlineViolationExprPublished, parityRuleExpression, parityRuleParams } from "@platform/contracts";
import type {
  ActionDraft,
  AdminTenant,
  AdminUser,
...
export const GRAPH: OntologyGraphVM = {
  nodes: [
    ...
    { id: "n-quality", key: "QualityLot", label: "质检批", kind: "object", domain: "quality", tier: 3, sourceSystem: "QMS", properties: [{ propKey: "lotNo", dataType: "string", isPrimaryKey: true }], sourceBindings: [], // DF.14：本体图谱上挂的是**已发布规则**（`OntologyGraphView` 渲染 node.rules 给用户看），故用
// 违规谓词口径 —— 与 `RULES` 一致。约束式只属 A2 抽取候选/制度原文（见下 RULE_CANDIDATES，别顺手统一）。
rules: [{ key: OUTSOURCE_REDLINE.ruleKey, name: "外协红线", expression: outsourceRedlineViolationExprPublished() }], derivations: [] },
    { id: "n-cap", key: "CapacityPyramid", label: "产能金字塔", kind: "object", domain: "capacity", tier: 1, sourceSystem: "派生", properties: [{ propKey: "week", dataType: "string", isPrimaryKey: true }], sourceBindings: [], rules: [{ key: "C03", name: "产能上限", expression: parityRuleExpression("C03") }], derivations: [{ propKey: "p90", formula: "capacity_forecast(p90)" }] },
    { id: "n-forecast", key: "DemandForecast", label: "需求预测", kind: "object", domain: "forecast", tier: 2, sourceSystem: "派生", properties: [{ propKey: "period", dataType: "string", isPrimaryKey: true }], sourceBindings: [], rules: [{ key: "C12", name: "预测重校", expression: parityRuleExpression("C12") }], derivations: [] },
    ...
    { id: "产能预测", key: "CapacityForecast", label: "产能预测", kind: "object", domain: "capacity", tier: 1, sourceSystem: "派生", properties: [{ propKey: "week", dataType: "string", isPrimaryKey: true }, { propKey: "p50", dataType: "number" }, { propKey: "p90", dataType: "number" }], sourceBindings: [], rules: [{ key: "C12", name: "预测重校", expression: parityRuleExpression("C12") }], derivations: [{ propKey: "p90", formula: "p50 × healthFactor" }] },
    ...
    { id: "精度校准器", key: "calibrator", label: "精度校准器", kind: "solver", domain: "solver", sourceSystem: "求解", properties: [], sourceBindings: [], rules: [{ key: "C12", name: "预测重校", expression: parityRuleExpression("C12") }], derivations: [] },
 ...
 *
 * DF.14（本单）：上面那轮只把**值**手工对齐了，两处仍各写一份字面量 —— 机制没变，早晚再漂。
 * 现在 C03/C05/C09/C13 的 expression/params 全部经 `parityRuleExpression()`/`parityRuleParams()`
 * 从契约 `PARITY_RULE_SEEDS` 派生，与后端 `battery.ts BATTERY_RULES` **同一个出处**；
 * C08 走更专的 `outsourceRedlineViolationExprPublished()`（DF.13）。谁改一边，另一边自动跟着变。
 * 门 `rule-parity:check` 守两端都在派生（防「留着 import、把值换回裸字面量」）。
 */
export const RULES: RuleEntry[] = [
  // WO-RULES-CLASSIFY：category 与真后端 battery.ts 种子同步（规则库分类筛选真元数据；约束条件另按 severity=BLOCK 判别）。
  { id: "rule-c03", key: "C03", name: "产能上限约束", expression: parityRuleExpression("C03"), scopeObjectTypes: ["Order", "CapacityPyramid"], severity: "BLOCK", category: "产能", origin: { type: "DOCUMENT", docId: "doc-policy", span: { start: 120, end: 180 }, extractJobId: "job-ex1" }, version: 2, status: "PUBLISHED" },
  ...
  { id: "rule-c13", key: "C13", name: "客户信用额度", expression: parityRuleExpression("C13"), scopeObjectTypes: ["Order"], severity: "BLOCK", category: "财务", origin: { type: "SYNTHETIC" }, version: 1, status: "PUBLISHED" },
  { id: "rule-c05", key: "C05", name: "产线利用率持续越线", expression: parityRuleExpression("C05"), scopeObjectTypes: ["Line"], severity: "WARN", category: "产能", origin: { type: "DOCUMENT", docId: "doc-policy", span: { start: 320, end: 390 }, extractJobId: "job-ex1" }, version: 1, status: "PUBLISHED" },
  // 规则即引用 P1：曾"未找到定义"的规则补为一等规则（含命名阈值 params）——mock 与真后端同步。
  { id: "rule-c09", key: "C09", name: "数据时延临时降级", expression: parityRuleExpression("C09"), scopeObjectTypes: ["DataSourceHealth"], severity: "WARN", category: "质量", params: parityRuleParams("C09"), origin: { type: "SYNTHETIC" }, version: 1, status: "PUBLISHED" },
];
  ...
  provenance: [
    {
      ...
      ...({ stepId: "s2", formula: "affected_orders(base=常州, window=D+0..D+14)", rules: [{ key: "C03", expression: parityRuleExpression("C03") }], value: "3 单 / 4,820 套", valueLabel: "受影响订单" } as Record<string, unknown>),
...
export const ANSWER_A2: Answer = {
  ...
  provenance: [
    ...
    { id: "prov-a2-3", source: "TOOL_RESULT", toolCallId: "tc-a2-2", toolName: "invoke_solver:capacity_forecast", outputPath: "$.gap", snapshotVersion: "ov-12", ...({ stepId: "s3", formula: "gap = demand - p90", rules: [{ key: "C03", expression: parityRuleExpression("C03") }], value: "-1.2 GWh", valueLabel: "产能缺口" } as Record<string, unknown>) } as Answer["provenance"][number],
  ],
};
