/**
 * WO-MOCK-FE-REGISTRY-PARITY · 前端 mock 求解器注册表**全集镜像**（61 条·按 key 字典序）。
 *
 * ── 病灶（本单治的病）────────────────────────────────────────────────────────
 * 此前 `handlers.ts` 内联手抄 4 条（capacity_forecast / bottleneck_matrix /
 ...
 * 已验收）同一份真实注册表返回的逐字段镜像（真起 datacore 后
 * `GET /a/v1/solvers/registry` 的返回），加 `pool` 字段记录 A 侧目录归属。
 * 跨 app import 源码违反 contracts-only-shared，故各 app 各持一份**替身**；
 * 防漂移不靠纪律靠测试：`test/solver-registry-parity.seam.test.ts` 把本文件的
 * 键集与 pool 归属**现算对拍** A 侧 `apps/datacore/src/catalog.ts` 文本，
 * 缺/多哪个 key 当场报名字。
 *
...
/** 求解器全集（61 条·真实注册表返回的逐字段镜像·按 key 字典序·R6 稳定序）。 */
export const MOCK_SOLVER_REGISTRY_FULL: readonly MockSolverRegistryItem[] = [
  {
    "key": "affected_orders",
    "pool": "scenario",
    "name": "受影响订单",
...
/**
 * 发现页展示位策展（4 条）——**只是「展示哪几条」的选择**，不是词表第二出处。
 * 增删展示位改这里；条目元数据不许在此另抄（从全集派生，见下）。
 * 漂移防线：`solver-registry-parity` 断言本数组 ⊆ 全集 key 且发现页实际下发的
 * 恰是本数组这几条（少一条 = 策展 key 漂出全集，当场报名字）。
 */
export const SOLVER_DISCOVERY_DISPLAY_KEYS = [
  "capacity_forecast",
  "bottleneck_matrix",
  "selection_optimize",
  "order_fullchain",
...
/** 发现页子集（`GET /a/v1/solvers/registry` 数据源）：从全集按策展 key 派生，pool 不下发。 */
export const MOCK_SOLVER_REGISTRY: readonly Omit<MockSolverRegistryItem, "pool">[] =
  SOLVER_DISCOVERY_DISPLAY_KEYS.map((k) => MOCK_SOLVER_REGISTRY_FULL.find((s) => s.key === k))
    .filter((s): s is MockSolverRegistryItem => s !== undefined)
    .map(({ pool: _pool, ...rest }) => rest);
