...
export const handlers = [
  ...
  // ---- OntoFlow 统一本体建模工作流（PRD v2 §4，feature.data-builder 门控） ----
  ...
  http.post("*/a/v1/ontology-workflows/:id/publish", ({ params }) => {
    const wf = db.ontologyWorkflows.find((w) => w.id === params.id);
    if (!wf) return err(404, "NOT_FOUND", "工作流不存在");
    const types = wf.nodes.filter((n) => n.kind === "SUBGRAPH_ENTITY").map((n) => (n.kind === "SUBGRAPH_ENTITY" ? { typeKey: n.modeling.typeKey, displayName: n.modeling.displayName } : null!)).filter(Boolean);
    const links = wf.nodes.filter((n) => n.kind === "SUBGRAPH_LINK").map((n) => (n.kind === "SUBGRAPH_LINK" ? { linkKey: n.spec.linkKey, fromTypeKey: n.spec.fromTypeKey, toTypeKey: n.spec.toTypeKey } : null!)).filter(Boolean);
    wf.status = "PUBLISHED";
    wf.updatedAt = new Date().toISOString();
    return HttpResponse.json({ types, links, sliceKey: `${wf.id}-slice`, version: 1, actionDraftId: newId("act") });
  }),
  http.post("*/a/v1/ontology-workflows/:id/scaffold", ({ params }) => {
    ...
    return HttpResponse.json({ views, scenes, agents, scenarioPackages: [`pack-${wf.id}`], solverBindings: ["generic_whatif"] });
  }),
