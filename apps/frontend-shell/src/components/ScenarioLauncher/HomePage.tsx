...
export default function HomePage() {
  ...
  return (
    <div data-testid="home-page" style={{ maxWidth: 1100 }}>
      <h2 style={{ fontSize: 17, marginBottom: 4 }}>{workspace.tenant?.name ?? "全域决策支撑"}</h2>
      <div className="muted" style={{ fontSize: 12, marginBottom: 16 }}>
        一键启动高频场景直达推演，或进入业务视图。按 <kbd>⌘K</kbd> 随处快搜场景。
      </div>

      {hot.length > 0 && (
        <>
          <div className="section-title">⚡ 高频场景</div>
            ...
            {hot.map((c) => (
              <button key={c.sNo} className="panel" data-testid={`home-scenario-${c.sNo}`} style={{ textAlign: "left", cursor: "pointer", display: "flex", flexDirection: "column", gap: 4 }} onClick={() => void launch(c)}>
                <span style={{ fontWeight: 600 }}>
                  {c.name}
                  {c.willProduceDraft && <span className="badge amber" style={{ marginLeft: 6 }}>写回</span>}
                </span>
      ...
      <div className="section-title">业务视图</div>
