...
export function CausalGraphPanel({ source, testid = "causal-graph" }: { source: CausalGraphSourceRef; testid?: string }) {
  ...
  return (
    <div data-testid={testid} data-source-kind={g.source.kind} data-graph-id={g.graphId}>
      <div className="muted" style={{ fontSize: 10.5, marginBottom: 6 }} data-testid={`${testid}-source`}>
          ...
          return (
            <div key={seg} className="panel" style={{ padding: 8 }} data-testid={`${testid}-seg-${seg}`}>
              <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 4 }}>
                ...
                <span className="mono muted" style={{ marginLeft: 4, fontSize: 10.5 }} data-testid={`${testid}-count-${seg}`}>
                  {g.segmentCounts[seg] ?? 0}
                </span>
              </div>
              {gap ? (
                <div data-testid={`${testid}-gap-${seg}`} data-gap-reason={gap.reason} style={{ fontSize: 10.5 }}>
                ...
                nodes.map((n) => (
                  <div key={n.nodeId} data-testid={`${testid}-node-${n.nodeId}`} style={{ fontSize: 11, borderTop: "1px solid var(--line)", padding: "3px 0" }}>
                    <div>{n.label}</div>
                    ...
                    <div className="muted" style={{ fontSize: 10 }} data-testid={`${testid}-prov-${n.nodeId}`}>
      ...
      {g.caveats.length > 0 && (
        <div style={{ marginTop: 8, fontSize: 10.5 }} data-testid={`${testid}-caveats`}>
