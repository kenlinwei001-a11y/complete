...
describe("SEAM · multiobj-real-orderbook-seam", () => {
  ...
  it("② optimize_whatif（family=cross_object_occupancy）加大违约金权重跨断点 → 各目标 Δ 分解真变（非全 0）", () => {
    // 从**营收优先基线**（planner 默认把营收当头号目标）加大违约金权重——跨越最优基断点 → 最优指派真漂移。
    // 诚实边界（faithful mock·非造 Δ）：权重扰动若**未跨断点**（停在同一最优基内），真 CP-SAT 亦返 Δ=0——
    // 这是"改权重没大到改变最优解"的**正确**求解行为，非假。真实订单簿（W5 业务整形后 qty 谱宽大）里
    // 均权 {1,1,1} 基已被违约金维主导（优先级密度谱宽 > 单价密度谱宽），单纯抬违约金停在同基（Δ=0 真值）；
    // 故本例以营收优先基线 {2,1,1} 起、违约金 1→4 强化，落到断点另一侧证"改权重真重排"（与 ② 极端权重同证一事·此处走 whatif Δ 路径）。
    const whatif = mockMultiObj("optimize_whatif", {
      args: crossArgs(scenario, { revenue: 2, penalty: 1, cost: 1 }),
      perturbations: [{ target: "objectives.penalty.weight", value: 4 }],
    }) as { deltaByObjective: Record<string, number>; feasible: boolean };
    expect(whatif.feasible).toBe(true);
    // 至少一个目标 Δ ≠ 0（跨断点后最优真漂移 → 各目标真变）。
