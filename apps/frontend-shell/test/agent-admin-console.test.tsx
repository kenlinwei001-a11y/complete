...
describe("WO-AGENT-ADMIN-CONSOLE · 运行观测台", () => {
  it("① 第一层：KPI 数字由**真实运行列表**算出（不是常量）", async () => {
    await openConsole();
    // mock 的 /b/v1/queries 里 AGENT 路共 5 条、均 COMPLETED；WORKFLOW 路 3 条必须被排除。
    // ⚠️ 金值 4→5 由 WO-BEFE-C 补入 `task-consult-1`（多角色会诊那条）所致：
    // 补之前 `run-fanout-1` 挂在一个**任务清单里不存在**的 taskId 上，界面永远展不开它。
    expect((await screen.findByTestId("kpi-total")).textContent).toContain("5");
    expect((await screen.findByTestId("kpi-completed")).textContent).toContain("5");
