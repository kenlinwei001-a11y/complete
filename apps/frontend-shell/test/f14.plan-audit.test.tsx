...
describe("F14 · 规划体检（plan-audit）改参即重算", () => {
  it("基线预填（V1 定稿）→ 改 cashCushion=45 → X05 硬卡(C18) → 一键 fix → 降为软风险；评分/档位色联动；重置回基线", async () => {
    ...
    await waitFor(() => expect(verdict).toHaveTextContent("体检结论：有条件通过（评分 68/100）"));
    ...
    expect(screen.getByTestId("audit-counts")).toHaveTextContent("1 硬矛盾 / 4 软风险 / 4 建议");
    expect(screen.getByTestId("audit-verdict")).toHaveTextContent("体检结论：不通过（评分 43/100）");
    ...
    expect(screen.getByTestId("audit-counts")).toHaveTextContent("0 硬矛盾 / 5 软风险 / 4 建议");
    expect(screen.getByTestId("audit-verdict")).toHaveTextContent("体检结论：有条件通过（评分 60/100）");
