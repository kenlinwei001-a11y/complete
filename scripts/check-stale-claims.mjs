...
function main() {
  ...
  if (!selftestOnly) {
    ...
    // ⑥b 扫描规模下限：记号在生产源码里**今天真有实例**（@stale-self marks.production ==41
 ...
 *        赌注：@stale-self marks.production ==41
 ...
 *          **这不是放宽，是口径真的变了**：数字往上走 = 有人真的多写了两条赌注。
 *      · 原文（**2026-08-16 实测推翻**）写「`runBaselineFactChecks` 那条今天有 6 条真数据，挂在两条 CONFIRMED-STALE 上」。
 ...
 *      @stale-self marks.production ==41 —— 2026-08-29 先由 35 改到 37，收编 integ-batch-2 时
 ...
 *      **2026-09-11 由 40 改到 41**（WO-UX-UNIFY-STALE）：08-28 决策屏 KPI 下那句
 *      「全屏**只有**「扰动前」「扰动后」**两个**观测点，中间每一拍的读数从未取回」按 ⑤ 补挂记号，
 *      赌 `apps/frontend-shell/src/api/endpoints.ts` 的 `simTick` 回包型
 *      /curTick: number; state: TickState; trace\?: unknown\[\]; disclosure\?: SimRunDisclosure/ ==1
 *      ⇒ 生产实例 40 → 41。**同样不是放宽**：这句话赌的是**上游没有逐拍序列**这件事，
 *      而屏上没有任何运行时守卫在看回包型 —— 后端哪天往 tick 回包里加一条 series，
 *      屏上会继续说「从未取回」而走势线本可以画了。⚠ 赌注刻意**钉在上游契约**而不是本文件
 *      自己的字符串：自指的赌注等于没赌。两次金丝雀：该型现算 1、
 *      同文件 /perTickSeries/ 现算 0 ⇒ 计数器有鉴别力。
 *      ⚠ 同单另两处（KPI「可处置」口径句 · ③ 栏无对策文案）**没有挂记号，是改写** ——
 *      它们是**阈值式定义**与**运行期条件文案**（逐字对应 `candidates.length > 0 / === 0`，
 *      每次渲染现算），不存在「过时」这一态；给恒真的句子挂赌注只会制造一条永远绿的装饰品。
