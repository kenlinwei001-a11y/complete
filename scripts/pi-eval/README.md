...
> ⚠️ **给未来写门的人**：本目录里的 `zz-probe*.test.ts` 文件名带 `.test.ts`，但**它们不是本仓的测试**
> ——是要拷进 pi 的 `packages/agent/test/` 才跑的（故必须保留该后缀）。本目录不在任何 workspace 包内，
> 现有各门也都用固定目录白名单（`check-prd-coverage.mjs` 只扫 `apps/*/test`），今天不会误收。
> **但任何新门若改用 `**/*.test.ts` 全仓 glob，会把这 3 个文件算成我们的测试覆盖** ——
> 那正是本仓反复栽过的"计数看着对、来源是错的"一类。加门时请显式排除本目录。
