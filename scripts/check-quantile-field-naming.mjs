...
// （它的理由是「嵌在函数/块里的 try 不覆盖全流程」）。写成 `if (…) { try {…} }` 语义一样、
// 注入异常也确实退 2，但门会报「无顶层兜底 · 找到 0 处兜底结构」。
// 处置是改本文件去合规，**不是**放宽那道门 —— 为让自己变绿去松判据就是把门做成装饰品。
// （2026-08-14 同一形态在 check-dbui-flow-order.mjs 上已修过一次，本条是第二次。）
