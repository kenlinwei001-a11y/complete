#!/usr/bin/env node
/**
 * 门 `scene-agent-config:check`（WO-SCENE-C/D Phase D·防半截上架门·G-3/G-9）。
 *
 * 让「人机对话入口配置完整」结构性可保证、半截配置上不了架。对每个出厂 SceneEntry 校验：
...
const { agents } = seed.seedRegistry();
...
if (red) {
  console.error("\n✗ scene-agent-config:check 未过：上述对话入口配置不完整/半截。修法：mode≠WORKFLOW_ONLY；AGENT_FIRST 配 defaultAgentId；defaultAgentId 指向已发布 agent 且工具/规则绑定合法（参 WO-SCENE-B agt_plan_audit 模板）。");
  process.exit(1);
}
console.log(`✓ scene-agent-config:check 通过（${scenes.length} 个对话入口配置一致：无 WORKFLOW_ONLY · defaultAgentId 均指向已发布 agent · 工具/规则绑定合法）。`);
