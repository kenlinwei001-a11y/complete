...
/* ── 顶层兜底（WO-R6 收编时补）───────────────────────────────────────────────
 *
 * 本门原版成文于 canonical 并入 `WO-GATE-RC2-DISCIPLINE`（`bcaa0894`）**之前**：
 * `toolBroken()` 已有、已知环境失败路径也都走了它（git ls-files 失败 / 扫描面塌陷 /
 * 基线解析失败 / 金丝雀不符），**但没有顶层兜底** —— 收编时被
 * `check-gate-exit-discipline.mjs` 当场咬出（RC=1，原文「② 无顶层兜底」）。
 *
 * 为什么这一条非补不可：「已知的环境失败」永远是不完整的（只读 FS / 权限 / OOM /
 * node 版本差异都能抛），而 node 对未捕获异常一律退 **1** —— 恰好撞上本门
 * 「真有基线外新增盲点」那个码。于是「门自己崩了」被读成「你的测试有盲点」。
 * 照铁律 0.6 的句式：**「我用『进程非 0 退出』当作『有覆盖率盲点』的证据，而前者并不度量后者。」**
 *
 * 注册在 `toolBroken` 定义之后、任何扫描逻辑之前 —— 兜底要覆盖的正是「后面任何一行崩了」。
 */
...
if (process.env.COVERAGE_BLIND_FORCE_TOOL_BROKEN === "1") {
  toolBroken("（故障注入）模拟扫描面塌陷", "  这是自检在验「环境故障 ⇒ RC=2」这条路径。");
}
