  ...
  for (const k of ["facts", "multiScreenFacts", "pairs", "caliberDivergent"]) {
    ...
    if (now[k] < min) {
      fail.push(`D2/D4 规模棘轮回退：\`${k}\` ${min} → ${now[k]}（少了 ${min - now[k]}）—— 有读取位从受检面掉出去了。` +
        ...
        `\n        若确属有意（页删了 / 读取位真撤了），**手改 scripts/fact-usage-baseline.json 的 min，并在 _doc 里写明哪个页被删、掉了哪几条**。` +
        `\n        ⚠ 别按老话去跑 --tighten —— 它只许上调（"只许收紧，不许放水"），而页删除必然下调，那条路走不通（2026-09-12 删「运营复盘」时实测踩过）。`);
    }
  }
  const noWhy = [];
  ...
  writeFileSync(BASELINE, JSON.stringify(buildBaselineDoc({
    ...
    prose: {
      _doc: [
        ...
        "有意收缩（页删了 / 读取位真撤了）**手改本文件 min + 在 _doc 写明理由**，不许静默；",
    "⚠ 不要跑 `--tighten` —— 它只许上调，页删除必然下调，走不通（2026-09-12 实测）。",
      ],
    },
    computed: {
      min: now,
