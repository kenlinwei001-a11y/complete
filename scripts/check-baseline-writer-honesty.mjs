 ...
 *    没列出来的就是没看见，不是看过了没问题。
 *  · 存量违规**逐条进豁免基线并写明 why**，**不是**把判据放宽到它们也算通过 ——
 *    放宽判据是隐形的，豁免是看得见的，做成前者这道门当天就是装饰品。
 *    ✅ **2026-08-14 回写（WO-RATCHET-NEWFILE-BLINDSPOT）**：原文这里点名
 *    「`scripts/check-ui-first-layer.mjs` 目前**不走**共享写入器（本单禁改文件）」——
 *    **该条已收口**（那单解禁了那个文件，顺手改调 `buildBaselineDoc` 并跑 `baselineDocCanary()`），
 *    豁免基线 **8 → 7 条**。留着旧措辞比没有更糟：写在最容易被信的地方的过期事实会被直接引用。
 *    ⚠ 收口时实测到一条**不在头注里、但判据真的在咬**的形态：把 `buildBaselineDoc(…)` 抽成
 *    `const doc = …` 再 `writeFileSync(BASELINE, JSON.stringify(doc, …))`，本门判 **HAND_ROLLED** ——
 *    判据②认的是「**写入点实参里**调了共享写入器」，抽成变量就不在实参里了。这是对的
 *    （「导入了」证明不了「写的时候用了」），但要改的人得知道：**必须内联在 `writeFileSync` 的实参里**。
 *
 * ══ 金丝雀（保命判据 · 任何模式下先跑）════════════════════════════════════════
