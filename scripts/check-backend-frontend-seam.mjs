...
/**
 * 词法原语与前端消费判据从 `lib/seam-lex.mjs` 单源引入 —— `check-solver-field-seam.mjs`
 * （字段粒度那道门）用的是**同一份**实现。两道门各抄一份正则 = 铁律 0.6 警告的那种装饰品：
 * 改了这边的正则，那边拿旧的去测、照样绿，而两边报的都是**否定结论**（"没有缺口"）。
 * 下面 re-export 保持本文件对外符号不变（金丝雀与既有引用一个字节不用改）。
 */
import {
  M_CODE, M_COMMENT, M_STRING, M_REGEX,
  lex, splitTopLevel, stripComments, objectTopLevelKeys, skipBracedExpr,
  lineOf, walk, frontendProdFiles, readProdTexts, mentionsInProd,
} from "./lib/seam-lex.mjs";
export { M_CODE, M_COMMENT, M_STRING, M_REGEX, lex, splitTopLevel, stripComments, objectTopLevelKeys, frontendProdFiles, mentionsInProd };
...
/* ════════════════════════════════════════════════════════════════════════════
 * 1 · 词法扫描原语 —— **实现在 `scripts/lib/seam-lex.mjs`**（本文件顶部 import + re-export）
 *
 * 为什么不在这里写：`check-solver-field-seam.mjs`（字段粒度那道门）要用同一批原语。
 * 各抄一份 = 铁律 0.6 点名的装饰品形态：改了一边的正则，另一边拿旧的去测、照样绿，
 * 而两边报的都是**否定结论**（"没有缺口"）。原语单源 ⇒ 一处修好，两道门同时修好。
 *
 * 那份实现为什么必须是正则字面量感知的（不是裸正则）：本仓源码里真有 `replace(/\/$/, "")`
 * 这种**正则字面量内含 `//`** 的写法（`apps/agentcore/src/agent/production-cognition.ts:52` 等
 * 5 个文件）。裸正则把那两个斜杠读成行注释起点 ⇒ 该行剩下的代码整段被当注释吞掉 ⇒
 * 抽取器悄悄少数据、报出的是"干净"。详见 lib 顶注。
 * ═══════════════════════════════════════════════════════════════════════════ */
...
// walk / frontendProdFiles / readProdTexts / mentionsInProd 的实现同样在 `lib/seam-lex.mjs`（单源·顶部已 import）。
// 前端生产面判据：`src/**` 剔除 `src/mocks/**` 与任何测试文件 —— **只有 test/mock 引用 = 已排练，不是已实现**。
// 消费判据：名字出现在代码或字符串里即算提到，**注释不算**（注释里写一句「供前端分栏」不是消费；`roleLabel` 那条正死在这上面）。
