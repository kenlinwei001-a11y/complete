...
// ⚠️ 词法/路由原语已抽到 scripts/lib/source-lex.mjs —— `check-mock-fidelity.mjs` 也要用同一批。
//    照铁律 0.6：**不许各抄一份**，两道门 import 同一份实现。
//    这里同时 re-export，保持本文件对外的导出面不变（抽包前后输出逐字节一致）。
import {
  M_CODE, M_COMMENT, M_STRING, M_REGEX,
  lex, splitTopLevel, stripComments, objectTopLevelKeys, skipBracedExpr, lineOf,
  extractBackendRoutes, normalizeTemplatePath, normalizePath, extractRewritePrefixes, pathMatches,
  walk,
} from "./lib/source-lex.mjs";
export {
  M_CODE, M_COMMENT, M_STRING, M_REGEX,
  lex, splitTopLevel, stripComments, objectTopLevelKeys,
  extractBackendRoutes, normalizeTemplatePath, normalizePath, extractRewritePrefixes, pathMatches,
};
...
/* ════════════════════════════════════════════════════════════════════════════
 * 1 · 词法扫描原语 —— **已抽到 scripts/lib/source-lex.mjs**（见文件顶部 import）。
 *
 * 为什么抽走：`check-mock-fidelity.mjs` 要用同一批原语。照 CLAUDE.md 铁律 0.6，
 * 门脚本的金丝雀必须与主逻辑共用同一份实现，**不许各抄一份正则** —— 复制过去就是装饰品。
 * 抽包时一行逻辑都没改，本门抽包前后输出逐字节一致。
 * ═══════════════════════════════════════════════════════════════════════════ */
...
// METHODS / extractBackendRoutes / normalizeTemplatePath / normalizePath 均来自 lib/source-lex.mjs
...
// extractRewritePrefixes / pathMatches 均来自 lib/source-lex.mjs

/* ════════════════════════════════════════════════════════════════════════════
 * 4 · 前端生产代码面与"是否被消费"判据
 * ═══════════════════════════════════════════════════════════════════════════ */

// walk() 来自 lib/source-lex.mjs
