 ...
 *  ④ **扫描面只有 `.css` 与 `.tsx`** —— 写在 `.ts` 字符串字面量里的**内嵌 CSS** 看不见。
 *     实测本仓有一处：`views/graph/mappingExport.ts:69/75`（导出成独立 HTML 的报告模板，
 *     `color:#9aa8b6; font-size:11px` 压白面 ≈ 2.4:1）。它不在本门扫描面内，
 *     **所以「全前端 <12px = 0」这句话要读成「本门扫描面内 = 0」** —— 两者不是一回事。
 *     不顺手扩面的理由：那份模板是**导出产物**（脱离应用主题系统、自带写死配色），
 *     判它需要另一套「导出文档」的口径，混进来会让本门的命题变糊。
 *
 * ══ 金丝雀（保命判据 · 每次运行都先跑 · 双向）═══════════════════════════════
