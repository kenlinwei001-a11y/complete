 ...
 *  |---|---|---|---|---|
 *  | minFontPx | **10** | 10（`small`） | 下限 **11** | 参照物的 10px `small` 装的是拉丁数字与单位；本页 10px 那 12 个装的是**中文**（「/ 吨」「/ 套/天」）。`check-text-legibility.mjs` 文件头已实测记账：9–10px 中文笔画会与抗锯齿糊在一起，**那部分损失颜色公式测不到**。故取在本页现值 10 与参照物 label 11.5 之间的 **11**。 |
...
// ───────────────────────────────────────────────────────────────────────────────
const FLOORS = {
  minFontPx: { cmp: "min", value: 11, why: "本页 10px 装的是中文；参照物的 10px small 装的是拉丁单位。取在 10 与参照物 label 11.5 之间。" },
