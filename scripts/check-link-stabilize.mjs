#!/usr/bin/env node
/**
 * 门 `link-stabilize:check`（WO-DB-LINK-STABILIZE · 洞E 链路派生稳定 · KILL-MOCK-RED · R6 · 堵 §8 G-BUILD-LINK）：
 * 守建域 comprehend 的切片 hops **不再恒 []**——从对象 refToTypeKey（+ FK 名启发兜底补 LLM 丢的链）经
 * `slice-planner.planSlice` 真 BFS 多跳派生真实 hops，供 CLOSURE-HARDEN 的"按真链路生成非空 paths"收口。
 *
 * 静态哨兵：① comprehend.ts deriveBStack/comprehendScript 用 `deriveSliceHops`（非硬编码 `hops: []`）。
 ...
 *
 * 本体登记：docs/SYSTEM-ONTOLOGY.md §8 G-BUILD-LINK。用法：node scripts/check-link-stabilize.mjs
 */
...
// ── 静态：comprehend.ts 用 deriveSliceHops·无硬编码 hops:[] 残留 ──
...
if (src == null) fail.push("缺 apps/datacore/src/databuilder/comprehend.ts");
  ...
  if (/rootType:\s*t\.typeKey,\s*hops:\s*\[\]\s*\}/.test(code)) fail.push("comprehend.ts 仍有硬编码 hops:[]（洞E 恒空切片回潮）");
  if (!/planSlice\(/.test(code)) fail.push("deriveSliceHops 未接 slice-planner.planSlice（未复用真 BFS·H3）");
}

// ── 动态测谎（经 dist·green→red）──
async function dynamic() {
  const dist = abs("apps/datacore/dist/databuilder/comprehend.js");
  if (!existsSync(dist)) { fail.push("comprehend dist 未构建——跳过动态（gates 内含 build）"); return; }
  ...
  // ② 显式 ref → 非空。
  if (!(m.deriveSliceHops(explicit).get("Process")?.length > 0)) fail.push("显式 refToTypeKey 未派生非空 hops（洞E 未收口）");
  // ③ 丢链 + 兜底开 → 补回非空。
