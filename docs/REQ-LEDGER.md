...
| R8 | 假推演大扫除(不止洛阳)(1681/7881/8438) | 🔴 **系统性残口已定位·WO立P0** | 用户抓洛阳·设备OEE假红→**3路审计确认非孤例·是结构性缺口**:披露层(dataMode诚实位)完整·**消费层缺强制排除接线**——全链无一处MOCK→灰/空态。后端6 P0(risk.ts:165 mockTightness哈希恒红·service.ts:1129硬编码×700当产能·extended.ts:542 tightness:85)+门语义缺口(genuine-sim只查字段存在不查MOCK被排除·连SYNTHETIC都不减红)+前端8 P0(零处按dataMode抑制红·DailyDotAxis/RiskPopover签名不带dataMode·契约缺字段)。**WO-KILL-MOCK-RED(P0·TODO)** 三阶段治本(契约+后端不伪造→前端decisionColor门→genuine-sim v2语义门+本体G-DM-1)·验收锚洛阳原案(点它看诚实空态非假红)。诚实标mock≠满足原则。|
...
| **R32** | **数据流可见性 IPO 逐页审计:后端有真值·前端整块无处可见/导航走/藏别处(用户 IPO 方法抓)** | 🟢 **可见性链大部闭(11 WO DONE·CAPACITY-W-RECONCILE 2026-07-05 纠账)**——两波拆出的可见性/IPO 子单已逐一 DONE(AUDIT-LOG-UI/CALIB-CONVERGENCE-UI/INTAKE-VISIBILITY/KB-UI/DATAMODE-SWEEP/SOLVER-BINDING-UI/BIZVIEW-DOWNSTREAM/RESOURCE-REF-NAV/VIS-SIGNALS/VIS-SIGNALS-2/SIM-PRESET-INJECT 等·见本体 §8 G-VIS-1 逐单 ✅)·此前 🔴 头标为陈旧;**余**:IPO 逐页遍历 74 页为持续机制(发现新断层即立 WO)·非"全部页零断层"之断言 | 用户教的 **IPO 方法**(看每个展示数据的 I上游页/P处理过程/O下游页·逐页问上下游在不在过程全不全)抓出审核方漏的接缝断层——审核方此前用「模块/规格」镜头继承每张 WO 的范围盲区(合成谬误·验了模块没验接缝·本体明写"断点常在接缝")。**3路自动审计合并=17断层(5 P0·7 P1·5 P2)**·均「后端 curl 得 X·前端无处可看」。拆 **5 WO 入 dev loop**(`docs/WO-VISIBILITY-CHAIN.md`)：**INTAKE-VISIBILITY**(P0·上传就地可见+SchemaReconcile+空态回接)·**KB-UI**(P0·知识库 S4 前端零页·后端 kb.ts+3路由齐)·**AUDIT-LOG-UI**(P0·审计日志前端零入口·后端 GET /audit-log 有真值)·**CALIB-CONVERGENCE-UI**(P0·越用越准看板前端缺·后端 convergence 有真值)·**VIS-SIGNALS**(P1·未开通空态/通知铃/写回对账/实例数徽章/导航归组)。本体新增断点 **G-VIS-1**「后端产物真存·前端无处可见/导航走/藏别处」。IPO 逐页遍历 74 页为持续机制·发现即立 WO 入队。**第2波**(`docs/WO-IPO-WAVE2.md`·5 并行 agent 逐页遍历首波未覆盖的 5 簇:决策看板/推演/规划/规则求解器资产/IAM运营)=**21 断层去重→6 WO**:**SIM-PRESET-INJECT**(P0·推演 presetContext 注入视图求解器·关联 G-3)·**SOLVER-BINDING-UI**(P0·G-17 命门:DONE 的 SOLVER-BINDING 后端验收全 curl/gate 无 browser→前端零 activate UI·"上传自有类型→真答案"链前端物理走不通)·**BIZVIEW-DOWNSTREAM**(P1·台账下游导航+订单全链接 marginLedger 权威值+geo focus 大小写死路)·**RESOURCE-REF-NAV**(P1·反向引用图"被谁引用"+场景/ruledocs/mcp 导航·补 DONE 的 RESOURCE-REF 未覆盖资源侧)·**OPS-GOV-VISIBILITY**(P1·调度执行状态/LLM 预算/决策溯源链)·**VIS-SIGNALS-2**(P2 透出批×8)。**诚实**:D2 簇 ~15 页(Tenants/Users/Permissions/Features/Synthetic/Validation/…)IPO 健全无断层·清账不凑数;2 张 DONE 后端 WO 前端缺口如实 surface(非否定既有核发·而是其验收无 browser 臂留下的前端洞)。队列现 11 TODO(可见性/IPO)。 |
