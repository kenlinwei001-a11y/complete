...
| 取代/扩展 | 关联 `services/optimizer`（CP-SAT sidecar 范式 = 本 PRD 的接入点样板）· `PRD-A13-floor-semantics-deterministic` · `PRD-A3-reference-ontology-slice-planner` |
...
- **回写承诺**：**实现时**才回写本体 §2.E（新求解器）/§3（sidecar 链）；本设计 PRD 暂只在路线图登记"延后"。
