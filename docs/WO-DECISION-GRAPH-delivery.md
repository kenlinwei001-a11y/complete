...
> ## 📌 收编注记（2026-08-13·`claude/handoff-wo-reclaim-tail`）
> 本单从 `handoff-wo-decision-graph` 收编。分支基线 `f6c61869`，**canonical 已在其后 383 个提交**，
> 收编时按三方合并逐处重接（4 个新文件整取；`app.ts` / `features.ts` / `contracts/index.ts` 三处接线
> 按 canonical 现坐标重挂，未整取覆盖）。
>
> **收编时被 canonical 的门逼出两处分支上没有的新增**（分支 merge-base 早于这两道门，非作者漏写）：
> 1. `scripts/feature-rollout.json` 需为 `decision.causal-graph` 声明投放意图 ——
>    `check-dark-launch-integrity.mjs` 的 A1 判据。
> 2. ⚠️ **踩坑记账**：第一次照 `features.ts` 注释里的「暗发 `defaultOn:false`」字面填了 `stage:"dark"`，
>    被 A5 逼着加进 `*_DARK_LAUNCH_FEATURES` 集合；集合一加，**demo 租户就再也看不见它**，
>    本单自带的 A0 用例当场变红（15 passed / 1 failed，`expected false to be true`）。
>    正解是 `stage:"tiered"` —— A0 用例（`test/decision-causal-graph.test.ts:194-196`）
>    逐字断言它与 `sim.sandbox` 在 demo 上的可见性必须**相等**。
>    形态：**「我用 `defaultOn:false` 当作 `stage:dark` 的证据，而前者并不度量后者」**——
>    `defaultOn` 管的是 L1 平台默认，`stage` 管的是行业模板要不要顺带开，两件事。
>
> **收编时亲手真跑**：`test/decision-causal-graph.test.ts` **16/16 绿**；
> `dark-feature-default-off` 4/4 · `memory-mode-views` 5/5 · `datacore tsc --noEmit` RC=0 ·
> `check-dark-launch-integrity.mjs` RC=0 · `check-system-ontology.mjs` RC=0。
> **变异反证**（读回自证 + 重 build contracts）：拆掉 `causal-graph.ts` superRefine ③ 的悬空边校验
> → RC=1 红（`M4 契约门变异 ... expected [Function] to throw an error`）；还原 → RC=0 绿，
> 与分支原文逐字节相同（`git diff origin/claude/handoff-wo-decision-graph` 空）。
