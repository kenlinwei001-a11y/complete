...
### E. 求解/推演域（DataCore）
- **Solver（SOLVER_KEYS，29 个）**：确定性求解器（电池域纯函数 compute；末七位 `generic_inference`/`shared_bottleneck`/`concentration_risk`/`margin_attribution`/`supplier_disruption_radius`/`selection_optimize`/`assignment_optimize` 为行业无关通用求解器，走对象图而非电池 context；`selection_optimize`/`assignment_optimize` 经自托管 CP-SAT sidecar）· `solvers/service.ts:14`,`extended.ts`。**A8.1 `assignment_optimize`**（订单/需求→基地/产线指派最优化）：itemType 对象指派到 binType，每 item 一指派 + Σweight≤capacity + 按 (item,bin) 成本最小化（资格 mask），经 CP-SAT sidecar `/assignment` 出可证最优 + 证书；未配 OPTIMIZER_BASE_URL 显式"未接入"不兜底（R6 seed+单线程确定）；`solveAssignment` client（A8.2 sequencing / A8.3 packing 待后续）。注册表（`ontology:check` 门禁核对）：`assignment_optimize`
