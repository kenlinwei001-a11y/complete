...
- **经营驾驶舱绿地对象类型（cockpit P1/P2，`synthetic/battery.ts`）**：数字全部经本体关系算出、前后端零写死（R14）+ 可溯源（R13）+ 同 seed 字节一致（R6，独立子流 `mulberry32(seed^hash("cockpit"))`）。P1：`DemandSegment`(forecast，派生 `revenueWan=p50×priceWan`/`marginWan=p50×priceWan×marginPct/100`) · `FinancePlan`(finance，三线与需求交叉一致) · `MaterialBalance`(material，`gapTon=net×(1−lta/100)`)。P2（decision 域）：`PlanKpi`(经营 KPI，actual 经 P1 同源数据算出，派生 `gapPct=(actual−target)/target×100`) · `RootCauseChain`(根因归因模板：kpiCategory→factor→driverType.evidenceField，配成对象供 `plan_rootcause` 求解器据此算 DAG)。经声明式 `DASH_LAYOUT` widget（kpi/dag，`objects-aggregate`/`solver` query）渲染；归 `data-categories` 决策驾驶舱类 + `batteryCoverageSlices` 全字段覆盖。
