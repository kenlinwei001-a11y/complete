...
- **求解器 `optimize_whatif`（增量 3 并入 SOLVER_KEYS/SOLVER_OUTPUT_SHAPES，本步未登记到 SOLVER_KEYS）**：what-if over optimization；可作 G-11 沙盘内一类"优化推演"求解器（`SimSession`/`propagateTick` 可调任意已发布求解器）。增量 1 另加 5 CP-SAT 核心（facility_location/min_cost_flow/set_cover/independent_set/combinatorial_auction）照 `optimizer-client` 扩。
...
- **优化融合门（G-12 · 增量 0 登记 · 许可证/不训练）**：
  ...
  - **`opt-template:check` / `opt-determinism:check`（增量 1–3 待建并入 `pnpm gates`，本步不写未存在脚本路径以免文件锚点门红）**——`opt-template` 守模板池零业务常数（`debattery:check` 扫 `opt/`）+ 每模板有 requiredRoles + provenance；`opt-determinism` 守 CP-SAT seed+单线程字节一致（R6）+ embedding 不在确定性求解路径（FUS2）。
...
| G-12 | 有确定性派生 what-if（`generic_inference`/`recompute(dryRun)`、沙盘 `propagateTick`）但**无优化目标级 what-if**（改一约束/系数→求新最优解→Δ目标值/冲突约束）+ **无行业无关优化模板池**（CP-SAT 能力仅 selection/assignment/sequencing/packing 散在 SOLVER_KEYS，无抽象模板+本体绑定层，换行业需改代码违 R14）→ **◐ 增量 0 本体先行已立（契约/本体/许可证，零求解器代码）**：契约 `packages/contracts/src/opt-template.ts`（OptModelTemplate/OntologyBinding/OptPerturbation/OptWhatifResult）+ 优化融合域(§2.J 3 对象 + optimize_whatif 求解器 + 9 OR 核心模板族) + 优化融合链路(§3) + `opt.*` 4 entitlement 暗发 defaultOn:false(`features.ts`) + 许可证红线(`THIRD-PARTY-NOTICES.md` LIC1 不训练/LIC2 Gurobi 不碰/LIC3 MIT 署名/LIC4 CDLA 取派生) + 门 `solver-license:check`(已并入 gates)。**待增量 1–5**：5 CP-SAT 核心(facility_location/min_cost_flow/set_cover/independent_set/combinatorial_auction)照 optimizer-client 扩 + 并入 SOLVER_KEYS · OntologyBinding 绑定层(A13 角色+slice 范围+DF.8 接地) · optimize_whatif(结构化扰动→sidecar 重解→Δ目标，复用 recompute(dryRun) 不落真值，FUS1 不进 A18 沙箱) · embedding 复用检索(advisory，FUS2 不入确定性路径) · 行业租户绑定演示(R14 两行业证) + `opt-template:check`/`opt-determinism:check` 门。规格 `docs/SPEC-optimization-template-pool.md`（§14 接地核验为准）。借鉴=重写方法+派生产物+评测，**绝不训练**。 | NL→comprehend⊕embedding→OntologyBinding→CP-SAT sidecar 求最优⊕optimize_whatif 扰动重解→Δ目标→R4 采纳 | ◐ 增量 0 已立（契约/本体/许可证/门/暗发）；待增量 1–5（5 核心+绑定层+whatif+检索+两行业） |
