...
## 9. 需你确认（1 点 · 关键尺度）
- **"1:1" 的尺度**：HTML 的**精确演示数值/字符串**（dem 1420/1580/1760、产能决策"枣庄/江门"、触发"海外大单≥80万套"、分解 382/398…、Q3-only 月）——
  - **(A 推荐)** 对齐到 **结构/字段/口径** 即可，具体数值由**电池域生成器种子产出**（不强行等于演示值；守 R6/R14，且系统数值是"活的"）；
  - **(B 严格)** 把 HTML 的**精确数值/字符串**作为电池域种子**逐一对齐**（生成器配置写成与演示一致；仍非前端写死，但工作量更大、且把"活数据"钉成 demo 值）。
  默认 **A**（对齐结构+口径，数值走活管线）。若你要"截图级逐字一致"，选 B。
