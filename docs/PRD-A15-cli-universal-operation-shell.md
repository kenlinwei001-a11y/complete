...
## 9. 需你确认（2 点）
1. **"求解器上传"的语义**：求解器是**确定性代码注册**（R6/安全），不宜任意代码上传。CLI 的 `solver` 默认做 **① 调用既有求解器 + ② 经 FDE 建域注册"求解器需求"(DRAFT scaffold)**；若你要的是**上传 DSL/规则型可计算逻辑**（派生公式/规则即"轻求解"），我归到 `rule`/派生路。要不要保留一个"求解器需求登记"子命令？（默认保留）
