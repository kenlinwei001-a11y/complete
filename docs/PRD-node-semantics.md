...
| `cf` | ❌ 后端无 | 前端语义常量（7 个节点 · 7 条冲突 = 6 种机理，现金垫那条在配对的两侧各挂一次 · 24 条 `file:line` 依据） | **指不出代码依据的一条都没写**；已否掉一条设计稿里的（§4.3） |
...
上表 5 行 = 6 种机理共 7 条 `cf`（现金垫那条在 S&OP 共识会与主计划排产**两侧各挂一次**，
因为耦合是对称的，只挂一侧的话从另一侧点进来就看不见）。
门逐条把 `file:line` 拆开、核对**文件真的存在且行号不越界**（共 24 条依据）。
