...
> - **基线 sha**：`4ee94a94` — **canonical 已在其后 1027 个提交**。
> - **本次有没有重跑**：**没有**。本文是一次性 live 证据（真 LLM·非 CI），收编时**未重跑**——
>   表内墙钟/路由/答案状态一律按「1027 提交前的观测」读，不得当作当下现状。
> - **抽查 3 个锚点的复核结论**（亲手 `sed -n` 逐行核对，非 grep）：
>   | 文中锚点 | 收编日实测 | 判 |
>   |---|---|---|
>   | `apps/agentcore/src/router/ceo-route.ts:39` `RE_WHATIF` | **:39 逐字命中** | ✅ 未漂 |
>   | `ceo-route.ts:337` `route=generic_inference` / `:130 whatIfArgsFrom` | 实为 **:362** / **:149** | ⚠️ 行号漂移（符号在） |
>   | `orchestrator.ts:993 isSimComposeQuery→buildSimNavSlice` | 实为 **:1930 / :1947**（import 在 :47-48） | ⚠️ 行号漂移（符号在） |
> - **⛔ 结论已被推翻的一条（§3 缺口 2）**：原文写「canonical `4ee94a94` **无 sim-compose 路由** ⋯
>   `apps/agentcore/src/agent/sim-planner.ts` ⋯ **尚未并入 canonical**」。
>   **收编日实测：已并入** —— `apps/agentcore/src/agent/sim-planner.ts` 存在，且
>   `orchestrator.ts:1930` `isSimComposeQuery(...)` / `:1947` `buildSimNavSlice(...)` 均已接线。
>   ⇒ **#8 的根因描述已过期**，需要重跑才能判现状；未重跑前不得引用该条排期。
> - 未复核的部分：表中 10 题的墙钟与答案状态（需真 LLM live，本次不跑）。
