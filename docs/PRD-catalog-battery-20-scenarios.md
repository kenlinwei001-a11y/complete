...
## 9. 场景启动器（Scenario Launcher——"打开即见 20 场景、点击即推演"的字面形态）

新增视图 `scenarios`（导航首位，feature key `view.scenarios`，renderer 复用卡片栅格）：

1. **20 张场景卡**（数据=本目录 §1 总表经 API 下发，非前端硬编码）：场景名、一句话说明（取对应 skill summary 能力句）、触发问句、所属域色条、风险级徽章（ACTION_DRAFT 类带"将产生待审批草稿"角标）；
2. **presetContext**：每卡携带预置上下文 `{ targetView, selectedObjects[], slotPresets{} }`（如 S11 预置 lineId=常州·动力线-A，S15 预置 orderRef=电网公司F 最新单）——保证 20 个场景**无一例外一键可推演**，不会点开后被反问槽位；
3. **点击行为**：跳转 targetView → 注入 presetContext（选中对象高亮）→ 自动提交触发问句 → 推演流式呈现——用户的第一次点击距离答案 ≤5 秒（SMOKE 性能断言）;
4. 卡片右上"换个问法"次按钮：只跳转+注入上下文不自动提问（给想自己输入的用户）；
5. 运营态租户中每卡角标显示该场景近 30 日被问次数（来自任务史聚合——演示"这是常用功能"）。

**验收 SL1**：登录→启动器 20 卡齐全→逐卡点击全部得到绿徽章答案且无澄清反问（自动化遍历）；SL2：关闭某场景 feature → 对应卡消失；SL3：czmgr 登录→S02 卡点击后结果仅含常州数据。
