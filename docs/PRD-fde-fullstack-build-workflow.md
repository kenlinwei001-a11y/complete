...
3. **code-agent 工单施工·优先采用开源**（经治理,非运行时 eval）：真正新颖算法 → GrowthTicket（带 I/O 契约 + 输出形状声明）→ code-agent **先检索 GitHub/HuggingFace/web 现成开源**（如归因=SHAP/因果库、排产=OR-Tools、传导=图传播库）→ **适配/封装**而非从零写 → 过 4 闸评审：① 许可证合规(MIT/Apache 可,GPL/商用须审) ② R6 确定性(钉种子/确定性封装,否则不进 R6 链路) ③ R11 声明 `SOLVER_OUTPUT_SHAPES` ④ R13 审计+供应链安全(评审依赖) → 部署。**真无现成才从零写**（§3.8 ③毛利归因优先找开源归因库）。
comprehend/diff 节点据"问题能否被①组合/②兜底"决定落哪级,只有真新算法才到③,且③优先"发现并采用开源"。
