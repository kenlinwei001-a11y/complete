...
| ⑤ 换型损失 | `ChangeoverMatrix.minutes` | **残死（WO-ENGINE-2 已修掉前两重）**：① 属性真名是 `minutes`——旧写法 `changeoverMin` 在对象上不存在（`lever-meta.ts:29` 已登记真名）；② `patchCapacityContext` 的 switch 现已认 `ChangeoverMatrix`（override 真落进 ctx）；③ 残死因：`computeByProcessModel` 的 cellsPerDayP50 = processCap × certFactor × yieldRebase × matFactor **不含换型项**（`capacity.ts` 全文 0 次 changeover）⇒ ∂cellsPerDayP50/∂minutes 仍恒 0。 |
