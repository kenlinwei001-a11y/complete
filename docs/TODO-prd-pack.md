...
> - [ ] **基地集重播（keystone · 最大单体）**：系统 12 基地（常州/合肥/西安/宜宾/溧阳/青岛/南京/成都/福州/长沙/惠州/盐城）→ HTML 12 基地（常州·总部/厦门/成都/眉山/武汉/江门/合肥/信阳/枣庄/邯郸/自贡/洛阳，含 pos 动力+储能 混合业态 + util/bn）。**高 ripple**：BASES（kind 类型须扩 动力+储能 + 9 新城 lon/lat）+ opsteam/pools + livedin/engine + risk 瓶颈种子 + capacity MODEL_CAP_NET（datacore+前端 mock×2 + MOCK_BASES）+ geo-map + model nonProducible（kind×pos 含 hybrid）+ 测 capex/f18/f24。须专项谨慎重播（守 604+210 绿），建议独立 session 做。**解锁** dash K4「江门齐套90」/ map geo-map / risk 基地卡 / model 可产网络。
