...
> - [x] **audit** ✅（§4.5-A1/A2 口径 + C 时序/KSF 此前已落）：segTot 分母 + scoreH/M 22/7 + essShare 49/132（种子配置）；co-regress V6/F14/F20。**余**（A3 verdict 4 态 ✅本轮）：verdict 状态机 3 态（按分数）→ 4 态（按 H/M 计数：站不住 / 可定稿但有重要风险 / 可定稿·关注 N 项风险 / 全部通过·可直接定稿），契约枚举同步扩 + 前端 4 态色板（站不住红/重要黄/关注青/全通过绿）+ 展示插入 M 计数；datacore+mock 同源。测 solvers V6（站不住）+ attribution-routing + f14（三处 verdict）。**E01–E03 ✅本轮**：外部信号诊断项（E01 碳酸锂上行挤压毛利缓冲 C24 / E02 终端上险低于假设 C25 / E03 重点客户负面舆情 C13，纳入软风险 M；阈值 extGmBufferMin/extDemHigh 入 audit config；datacore plan.ts + mock 同源）；联动 f14（基线 0H/6M/58、cash45 1H/6M/36、fix 0H/7M/51）+ V6（5H/4M）+ F20。**仍余** 9-kind audit_timeline 求解器（逐日 series）+ 每项 kind 标注。
> **命名体系裁决（2026-06-23 用户拍板）：以 HTML 参考原型为单一真相源。**
> - [x] CAPEX 情景项目命名对齐 ✅：枣庄储能线 / 江门动力线（datacore capexScenario 已对齐前端 mock；c2c3 测更新）。
> - [x] **基地集重播（keystone · 最大单体）✅**：系统 12 基地 → HTML 12 基地（常州/厦门/成都/眉山/武汉/江门/合肥/信阳/枣庄/邯郸/自贡/洛阳，含 动力+储能 混合业态 + 坐标）。**关键设计**：model→base 可产性由随机改为**确定性 MODEL_BASE_MAP**（HTML MODEL_DEF 范式，保留 rng 步长 → 下游字节流稳定），一举修复 m11 校准对随机基地的脆弱耦合。全链对齐：BASES/kind/bottleneck/opsteam/livedin CASE/capacity MODEL_CAP_NET（datacore+mock×2+MOCK_BASES hybrid reason）/geo-map/CalibrationPage/livedInFixtures/planFixtures/测 capex/c2c3/planviews F23/f18/f24。**datacore 604 + frontend 210 全绿，gates 全过。解锁** dash K4 江门 / map geo-map / risk 基地卡 / model 可产网络真 1:1。
> - [~] **dash**（已起步，基地命名 keystone 解锁后开工）：**G 回采链+模块直达 ✅**（5 节点链 + 6 卡导航路由）· **B 待解决问题面板 ✅**（affected_orders 同源归并问题卡，自下而上）。**余**：A 八卡 KPI 六要素溯源（需 Segment p50/SOP_MAT 种子）· 8 根因 ROOT_LIB/PROB_META 扩展（4→8 backend）· C/E 逐单+问题级+规划推演 3 DAG（ROOT_CHAINS 下发）· D 订单台账筛选 · F 毛利 Σ勾稽 −0.40pct（order_margin_contrib）。逐块串行推进中。
> - [~] **risk**（①基地/型号命名 ✅随 keystone；本轮 §2.4 处置行动计划表 ✅）：risk_timeline 新增 planRows 输出（每基地主因素首选方案 + 峰值≥90 备份 + 14 天内反提 S&OP，owner=基地负责人·X经理 hashN 派生、start/done 按越线日前置 7 天、rule C05/C21、按启动排序），契约 RiskPlanRowSchema + RiskBoardView 行动计划表。测 solvers V5b + risk-plan 前端。**余** ②forecastStart 2026-07→06-10（高 ripple，dueDay 全偏）+ ③targetHash 越线分布口径 + ④delay/⑤arrival_gap 口径 + ⑦逐日 tip 事件文案 desc/src + 圆点轴。
...
> - [~] **PRD-scenario-ontogenesis P1 ✅（治本 · `docs/PRD-scenario-ontogenesis.md`）**：确定性绑定(§2.4)+grow 验证即上架门(§2.2/2.3)+留痕 ScenarioOntogenesisRun(前端可见来源)+诚实缺口(§2.5)；maturity GOVERNED/PROVISIONAL；回写本体 G-9。门B 真前端真后端验证（发育验证 S01→真 QOS→可用+留痕真求解器答案）。
> - [~] **PRD-scenario-ontogenesis P2 ✅（投影渲染闭 G-1 残面 · 用户铁律「推演数据须前端可见、知来源、不写死」）**：grep 实证发现 P1 grow 门把 14/20 占位卡（"XX已完成推演…结果详见步骤溯源"无真数据）**误判 GOVERNED**——典型违则。修：① render 步改 `solver_summary` 通用投影（`workflow/executor.ts summarizeSolverOutput`：求解器**真实输出对象**→KPI/表/规则依据块，`{{steps.s1.output}}` 经既有模板+provId 注入，**不写死任何业务数字/文案**，按 Object.keys 确定性 R6）② grow 诚实门加 `dataBearing` 校验（无承载数据块 → 不充作 VERIFIED，记 `RENDER_NOT_PROJECTED` → PROVISIONAL）③ 修 P1 ScenesPage 溯源链死链 `/task/`→`/tasks/`（门B 实测抓出）。后端测 scenario-ontogenesis ×4（S15 投影真值 GOVERNED / S18 占位诚实 PROVISIONAL）+ f37 锁路由；**门B 真后端真浏览器**：grow S15→任务页见 8 真 KPI（margin 0.2565/floor 0.12/diff/verdict 过线，求解器真值可溯）。datacore 641·agentcore 312·frontend 227·gates ✓。回写本体 G-1 闭残面 + G-9 P2。**④ P2 续：清 G-2 同类残两处**（grow-all 实证 S03/S06 仍 RUNTIME_FAIL，靠门B 真后端抓出 mock 接受、真后端拒的接缝）：S03 render 硬引缺失字段 `data.summary`→改 `solver_summary` 通用投影（门B 真验：任务页 5 KPI+1 表见 risk.level=MEDIUM/utilization=0.83 真切片数据）；S06 action-draft payload `baseId/solutionName`→契约 `base/factor/planKey`+补 factor 槽（可选，presetSlots 填）。**最终 20 卡：18 GOVERNED（全真投影数据）+ S18 诚实 PROVISIONAL（sop 视图指针无求解器）**——无一张占位文案误判可用。测 scenario-ontogenesis ×6（S03 切片投影/S06 草稿契约）。agentcore 314。**待 P3**：rules 自动接 evaluate_rules + slice 自动生成 + GapReport→runGrowthLoop 自动补 + `ontogenesis:check` 扩逐卡断言。原文：场景卡=R16 发育器官（grow 倒序长全闭包 → A10 验证真出答案才 GOVERNED 上架 → 正序确定性绑定跳过 classify → 缺则生长/开单不静默）。架构合理、复用既有机件（StoryBuildRun/verifyBuild/slice-planner/A18/ModuleProvisioner）。大件，分 P1/P2/P3。
> **B. 1:1 视图剩余（可独立推进）**：
