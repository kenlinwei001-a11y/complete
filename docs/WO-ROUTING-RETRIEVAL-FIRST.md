...
| **D5** | 求解在途时**禁用调参控件**（仓主提案）—— 见下方决策记录，**默认不做**，待仓主拍板 | — |

#### D5 决策记录（仓主提案「在跑的时候调参置灰」· 我的建议是**不做全面置灰**，理由存档）

**现状**：`pending` 今天只挂一个徽标（`GlobalSimLevers.tsx:91`「● 待重算 / 求解中」），**不禁用任何控件**——
这是有意为之：这几个页是「拖着看结果动」的活台，且前端竞态处理本身已正确
（`useLiveSolver` 取消前序 `AbortController` + 序号守卫防过期响应上屏）。

**不建议全面置灰的三条理由**

1. **堵不干净**：叠加是**服务端**属性。另开标签页 / 另一用户 / 直接打 API 照样叠加，UI 锁只挡住单用户单页面这一条路径。
2. **修错了半边**：前端已正确发出取消，是服务端不认（E10 真跑坐实）。让做对的那半迁就坏掉的那半 = 用工作流绕过缺陷。
3. **代价是核心交互**：CP-SAT 一次 15 s，置灰即每次调参冻结 15 s，活台退化成表单。

**建议顺序**：**D1（服务端真取消）为主修** —— 服务端认取消后，重新调参即免费、旧求解立即死，叠加自然消失，交互不必牺牲。
若 D1 落地前需立即止血，优先 **D4 在途去重**（纯前端）+ **限定置灰**（只对 `portfolio` 且只在在途超过 N 秒后禁用，轻求解器保持全程可调）。

**若仓主判定「宁可牺牲交互也要先彻底堵死叠加」→ 按全面置灰实施**，本记录保留以说明取舍。
