...
## 四之四 · Track E｜Skill 作为「一个意图一份声明」的单一外壳（仓主**已定案**）
...
**✅ 仓主已定案：Skill 吞并 Plan，不并列 —— 原话「execution 应该是 skill 里面的一个内容」。** 即 Skill 成为一个意图唯一的声明外壳，
确定性步骤降为它的一个字段：
...
**✅ 已定案 → 后续路线随之确定：是「把 32 份既有 Plan 升格进 Skill」的**迁移**，不是「新建 32 份声明」的加法。**

**迁移的四条硬约束（防这次迁移自己变成新的假绿）**

1. **零行为漂移**：升格后确定性题的执行**逐字节等同**今天的 `ExecutionPlan` 语义（R6）。
   验收须有一条「同一意图·同一槽位 → 迁移前后 answer 与 provenance 字节相等」的对照测试，
   而非只断言「Skill 里有 plan 字段」（那是运输层断言）。
2. **单一真源硬门**：迁移完成后 `ExecutionPlan` 不得再有独立的注册/播种/查询入口。
   须加门：`intents` 与 `skills` 之外**不存在**第三处描述「这个意图怎么答」的注册表；
   任一意图若同时能解析出 Plan 与 Skill 两份 → 红。
3. **entitlement 一处**：今天意图受 `intentAllowed` 门控、Skill 另有自己的发布状态。
   升格后二者必须**同一处判定**，否则会出现「意图开着但 Skill 关着」的半开态（#90 同族）。
4. **探索 Skill 必须有真消费方**：`maxBudgetRounds` / `provenancePolicy` 今天**零消费方**
   （与 #92 账本同族）。本 Track 验收必须证明：改 Skill 里的 `maxBudgetRounds` → 该类题的
   实际探索轮次**真的变**（效果层），不是只把字段读出来。
