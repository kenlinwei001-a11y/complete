...
## ⚠ 1.0 · 本节 §1 的形已被 `WO-CHAIN-MAP-LAYOUT` 取代（2026-08-11 并线回写 · **待审核方批准**）

> 铁律 0：**本体不回写即过期失效**。`WO-CHAIN-MAP-LAYOUT` 已把中央画布从
> **椭圆环 + 同心环**改成**横向折行的地铁 / 物流线路图**，故下文 §1 描述的"形"
> **不再是实现的记录态**。本节据实标注，不删原文（原文是当时的真实判断，删了就看不出为什么改）。
>
> **它给出的改形依据（三处实测代价）**：① 放射式标签在圆的顶/底必然拥挤（实测底部三条标签叠成一团）；
> ② 圆上没有阅读起点；③ 圆心整片浪费。一句话：**闭环是语义不是形状**。
>
> **§1 主张的三项信息增益逐条换了承载物，不是被丢弃**：
>
> | 原增益（§1） | 环的做法 | 横向版的新承载物 |
> |---|---|---|
> | ① 闭环 cash-to-cash | 环自身闭合 | 右端一条**回流箭头**（`ClosureReturnVM`），`closureBasis` 原文仍常驻 |
> | ② 多族并行 | 三圈同心环 | **并排多行**（`MetroRowPlan` / 每族一条线） |
> | ③ 共线区段 = 共享瓶颈 | 三个半径上**同角度**对齐 | 同一 `stepId` 落**同一列**（故多族时只有最后一条线标名） |
>
> **环几何本身没有被删**：`chainLineMap.ts` §5.2「环几何的对外单源」原样保留，
> 因为**在途 / 在制图层仍然靠它**（`TransitFlowLayer.tsx` 直接 import 那 8 个符号）。
> 并线时实测过：两套几何共存、各自的门同时是绿的。
>
> **⚠ 待裁**：这一改动把本文档的"设计记录态"从环换成了横线，属于产品判断，
> 并线单不自行拍板。审核方批准后，请把下文 §1 正文改写为横向版（或明确保留环为长期目标、
> 横向版为过渡态），并同步 §4 里「播控仍由 `TransitFlowLayer` 提供（作为线路图图层）」那句的措辞。
> 判据与几何常量的唯一实现始终在 `chainLineMap.ts` / `chainFamilyLines.ts`，本文不复述。

---

## 1 · 形：从「两根直线」到「环 + 同心环」（**已被 §1.0 取代，原文存档**）
