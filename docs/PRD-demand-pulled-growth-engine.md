...
> **归一说明（2026-06-18）**：本 PRD 是发动机的**「运行期 / 问句驱动」燃料口**（反应式：失败问句→QOS 实跑探针→诊断→补→施工→成长账本），是 g8 的**母体**。另一个燃料口**「构建期 / 故事驱动」**（主动式：故事脚本→全栈 A+B 倒推→跨系统 scaffold→闭包→历史推演记录 + InputManifest 自省 + 去模板化合成 + 存量回填）见 `docs/PRD-fullstack-story-build-g8.md`（v0.2，已并入另一并行稿）。两个燃料口共用 `classifyGap` 7 码分类法、`GapReport` 契约、`runGrowthLoop` 收敛 LOOP 与 GrowthTicket/GrowthLedger；**`StoryBuildRun`（构建期记录）与 `GrowthLedgerEntry`（运行期记录）归一为同一"历史推演记录"的两面**（分工边界表 + 归一三点详见 g8 §9）。本 PRD 的 P1–P6 已落部分（见 TODO）即两个燃料口共用的内核。
...
## 15. 决策记录（原 OPEN_QUESTIONS，2026-06-17 已裁决）

1. **K（收敛轮数上限）= 前端可配置**：默认 8，构建驾驶舱（§16）暴露配置项，按需调；不做隐式动态。
2. **多缺口补齐策略 = 同类一次补齐、跨类串行重跑**：一轮探针出多缺口 → 同类（如多处缺数据）一次性补齐，跨类（数据→切片→求解器）逐类补、逐类重跑，**保精确归因**（哪一类补齐推进了哪一步）。
3. **code agent 触发 = 推为默认触达 + 拉永远兜底**：`growth.ticket_opened` 事件 + 可选 webhook 主动唤起 agent 会话（低延迟）；同时 MCP/CLI 工单列表**始终可轮询**（保厂商中立、对 agent 宕机鲁棒）。配 dispatch 走推、未配走拉。
4. **成长账本平台级聚合 = 可配置可逆**：默认**关**（严格按 §10.1 两级域 + R2 tenant 隔离，账本只在本租户）；平台运维可**显式开启**元租户级盲区聚合视图（高频缺口跨租户统计），该视图是**对各租户账本的只读投影**（无数据迁移、可随时关、关即消失 = 可逆）。

## 16. 前端 · 构建驾驶舱（页面要求 —— 收编历史 PRD 未落地的页面缺口）

> 现状 `DataBuilderPage.tsx` 只有"七阶段状态灯 + 闭包数字 + JSON dump"，**远落后于 `PRD-unified-build-engine.md` §1.1.8/§3/§5.3/§8-P5 的"瀑布流逐产物 HITL"要求**（差距详见本节末尾审计）。本节把这些页面缺口 + 本 PRD 新增的驾驶舱要求**统一收编为前端验收项**，分期对齐，不再漂着。

| 页面能力 | 来源 | 现状 | 期 |
|---|---|---|---|
| **瀑布流逐产物展示**：每生成产物（对象类型/规则/切片/意图/计划/agent/数据）作为可展开卡片，非状态灯 | unified §1.1.8/§5.3 | ❌ 仅状态灯 | P6 |
| **逐产物明细**：原始数据需求/字段/切片覆盖/规则/约束/skill/意图/计划/agent | unified §5.3 | ❌ 无 | P6 |
| **逐产物 HITL + 就地 Action 审批**：admin 当前页预览 diff + 批复，无跳转 | unified §1.1.8/§3 + 本 PRD §6.4 | ❌ 无 | **P2** |
| **GapReport 面板**：缺口分类 + QOS 实跑证据（断在哪步） | 本 PRD §5 | ❌ 无 | **P1** |
| **缺数据真人正门可视化**：生成 Excel→连接器导入→字段画像→物化对象，逐跳可见可溯 | 本 PRD §6.1 | ❌ 无 | **P2** |
| **全链闭包可视化（CHAIN/SHAPE）**：故事→意图→计划→求解器→渲染 每段 BOUND/MISSING | unified §4（后端已产 findings，前端未接） | ◐ 后端有前端没接 | P3 |
| **K（收敛轮数）+ seed 配置项** | 本 PRD §8（决策 1） | ◐ 仅 seed | P3 |
| **LOOP 重跑/收敛态展示** | 本 PRD §8 | ❌ 无 | P3 |
| **GrowthTicket 看板**：工单列表/状态/认领/草稿/重跑 | 本 PRD §7 | ❌ 无 | P5 |
| **成长账本视图**：问题→缺口→补法→状态，demand-indexed，可反查"哪些问题卡在哪个缺功能" | 本 PRD §9 | ❌ 无 | P4 |
| **量化指标仪表盘**（§11） | 本 PRD §11 | ❌ 无 | P6 |

**前端 DoD**（补 §12）：
- GE-UI-1 就地审批：admin 在数据构建发动机页内批复一条补齐草稿（不跳转）→ 物化生效（对齐 GE-C）。
- GE-UI-2 GapReport 面板：实跑缺口分类 + 证据可见、可下钻到断点步。
- GE-UI-3 真人正门可视化：缺数据补齐后，连接器页见该 Excel、对象库见物化对象（对齐 GE-B）。
- GE-UI-4 瀑布流逐产物：每产物卡片可展开预览 + 逐产物确认。
- GE-UI-5 成长账本/工单看板：可反查需求↔缺口↔补法↔状态。
