...
- **rev.1**（`a9ce41c8`）初稿。§5 的四类计数（37/114/41/38）是**拼出来的**：三个判据各跑一遍再凑和，
  桶与桶之间不互斥，230 这个和是硬凑的。
- **rev.2**（本次）§5 重跑：按 **A → C → D → B 单一顺序落桶**，得到互斥且自洽的
  36 / 113 / 40 / 41 = **230**，并加了两条自洽性检查（A 桶含 wave4 与正线自身；C 桶分组和为 40）。
  同时补 C-2 的**冲突面实测**、补 §⑥ R10、补组 F 漏掉的 `CLAUDE.md` 第二条新约定。
  纠正一处差点写错的判断（见 §5.3 C-2 表下方的注）。
