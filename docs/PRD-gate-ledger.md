...
| ④ **处置完备** | 非 `GATES_CHAIN` 的每一条，`disposition` 与 `owner` 均非空且 `owner ≠ "TBD"` | 新门挂着不定性 → 红 |
...
| A4 | 18 个非 gates 门**逐个**有 `disposition` + 实名 `owner` | 账里 grep `"TBD"` 命中数 = 0 |
