...
1. ~~**步骤模板层** —— B4 的前置~~ ✅ **已交付已接线**（闭环坐标见 B4 行：接缝实测 7/7 走通；同族 `advance` / 实例详情由 B4a 接棒，卡在导航信息架构裁决）
...
> 📒 **2026-08-18 WO-TRACE-LEDGER-SWEEP 收敛记**（非列表项，探针不数）：本章节原躺**两份编号清单**，
> 第一份（上方，带完整 ✅ 回写记录）是闭环证据；第二份（下方）是它的**陈旧记账回声**——同 5 条事的短句副本，
> 删除线一直没回写齐，被调度探针按「未划掉列表项」数成待派 6。本次处置：第二份逐条划掉并注明闭环坐标，
> 归并进第一份；第 6 条「mock 与真后端 S&OP 量级差」自述「只报不动」、**不是可派工单**，划掉改为登记留痕。
> 自此本章节只以第一份为活清单，未划掉列表项 = 0（探针实测口径）。
1. ~~**步骤模板层** —— B4 的前置~~ ✅ **已交付并已接线**（见 B4 行；接缝实测 7/7 走通）。
   **改由 B4a 接棒**：同族的 `advance` / 实例详情两条仍无前端消费方，卡在导航信息架构（仓主决策）
2. ~~**13 类需求卡片补齐** —— C10~~ ✅ **已闭**（见上 C10 行；完整回写记录归并进本章节第一份清单第 2 条，功能在 `6ddf76f6`/`2242f9bc`/`dac4b1d2` 早已落地）
3. ~~**`STALE-8` 正则盲区**（实测漏 6 条：带点 slug 与非 `view.` 前缀）~~ ✅ **WO-STALE-REGEX-BLIND 已收单**（2026-08-16；盲区本身早在 `8244c82b` 修好，完整回写归并进第一份清单第 3 条）
4. ~~**`sandboxConsoleModel.ts:709` 过时文案**（写着已被删除的 `worstMbal`）~~ ✅ **WO-STALE-TEXT-4 实测已修**：该串只剩合法历史记录（2026-08-15 订正块），剥注释后可执行代码 0 处（完整回写归并进第一份清单第 4 条）
5. ~~**agentcore 3 处 stale 文案** —— `navigation-slice.ts` 把已不存在的 `p50` 当关键属性**喂给 LLM**（⚠️ 不报错，只是模型拿不到值）~~ ✅ **WO-STALE-TEXT-4 实测 40 处已修归零**（完整回写归并进第一份清单第 5 条）
6. ~~**mock 与真后端 S&OP 量级差 4–12 倍**（改它=改值，只报不动）~~ ⚠️ **登记类非工单**（2026-08-18 WO-TRACE-LEDGER-SWEEP 订正）：本条自述「只报不动」，**不是一张可派的工单**，留在「未派」章节会被探针永远数进去；划掉改为登记留痕，不派。
