...
| # | 派单/台账原文 | 实测 | 影响 |
|---|---|---|---|
| 1 | 「本仓刚补完 **873 个属性的 unit**」 | 881 条属性里带 `unit` 的 **24 条**（12/98 个类型）。`unit:` 在 datacore 源码出现 254 次，但多为**指标/杠杆/切片层**的单位 | 不改结论，**反而更硬**：语义层比台账说的稀疏得多，而**屏上一条都填不了、改不了**（API 可改，见 §1 动作 3） |
