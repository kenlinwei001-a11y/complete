...
> **改接线改母体 §3，再跑 `node scripts/build-ontology-slices.mjs` 同步**（勿直接改本文·门 `ontology-slices:check` 守漂移）。母体 hash `f48f794a85420f64`。
...
- **WO-SA-3 新增 `Workshop`（车间）对象类型**（§2 本体对象域·B 电池域·`domain=factory`）：4 字段 `workshopId`(PK)/`baseId`(ref→Base)/`name`(中文车间名·searchable)/`process_type`(enum·SLURRY/COATING/CALENDER/SLITTING/WINDING/ASSEMBLY/ELECTROLYTE/FORMATION/AGING/PACK·docx workshop.csv 对齐)。层级由 3 层 `Base→Line→Process→Equipment` 升 4 层 `Base→Workshop→Line→Process→Equipment`。**§3 新增两链路**：`workshop_belongs_to_base`(N:1)·`line_belongs_to_workshop`(N:1)；`Line` 加 `workshopId`(ref→Workshop) 回填。合成路线①（每基地一代表车间·确定性按基地序取工艺码·**零 rng 不移位既有 R6 字节流**·demo 对象 560→572·类型 40→41）；`Workshop` 归 `capacity_base` 数据接入分类。门 `synthetic-field-alignment`（判据②·revert 漏填 process_type→`Workshop.process_type` missing 红·亲验）。三改全落 battery 合成包（R14 平台零业务常数·`debattery:check` 绿）。
- **WO-SA-4 `Base` 补工厂台账 6 字段**（对齐 docx factory.csv）：`baseProps` 增 `factory_code`(string)/`province`(string·searchable)/`city`(string·searchable)/`factory_type`(enum·CELL/PACK/MATERIAL)/`status`(enum·ACTIVE/CONSTRUCTION/PLANNED/SUSPENDED)/`start_date`(date)。**记录型真事实非决策 → 每基地常量映射 `BASE_LEDGER`（非 rng·同基地恒定 R6）**，`start_date` 年份对齐 `BASE_REGISTRY.prodYear`（常州→江苏省/常州市/CELL/2015-01-01）。诚实标注：12 座皆运营中电芯厂 → `factory_type` 全 `CELL`、`status` 全 `ACTIVE`，enum 其余值仅前向可扩展域。合成 `bases.map` 尾部 `...baseLedgerOf(baseId)` 展开（零 rng·不消费 rng 序列·不位移既有字节）。
- **WO-SA-5 `Line` 补产线台账 4 字段**（对齐 docx production_line.csv·不重复既有 oee/OEE 三分解）：`lineProps` 增 `line_code`(string)/`max_capacity_day`(number·cell/日)/`target_yield`(number)/`status`(enum·RUNNING/IDLE/MAINTENANCE)。`line_code`/`status` 于 `lines.push` 确定性填（零 rng）；数值 `max_capacity_day`(=运营上限 capacityDaily 上浮 5–20% 设计铭牌)/`target_yield`(0.95–0.99) 于**第二遍 line 富化（全部拓扑 rngTopo 抽样之后·无下游 rngTopo 消费者）末位追加 `rngTopo()`**（守 R6 不位移既有字节）。
- **WO-SA-6 `Equipment` 补设备台账 5 字段**（对齐 docx equipment.csv·置于 SA-2 mtbf/mttr/health_score 之后）：`equipmentProps` 增 `equipment_code`(string)/`equipment_type`(enum·值域=`WORKSHOP_TYPES.code` 10 工艺码)/`manufacturer`(string)/`install_date`(date)/`status`(enum·RUNNING/IDLE/MAINTENANCE)。**equipment.csv 未随附 → 值域自定并声明**：`equipment_type` 由 `processId` 工序 step 确定性派生（coating→COATING/winding→WINDING/assembly→ASSEMBLY·⊆ 声明域）；`status` 确定性 hash（hash%6·偏向 RUNNING）；`manufacturer` 取小池 5 家锂电设备商确定性 hash 选；`install_date` 建厂日(SA-4 start_date)后 0–2 年确定性；`equipment_code` `EQP-`+6 位 hash。**全字段确定性派生·零 rng·不位移既有 R6 字节流**。SA-4/5/6 三改全落 `synthetic/battery.ts`（R14 battery-template 记录事实·不外泄平台常量·`debattery:check` 绿）；门 `synthetic-field-alignment`（判据②非派生全填）+ `ontology-validate` enum/date 值域（equipment_type 越域即红·亲验）。
