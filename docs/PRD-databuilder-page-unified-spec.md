...
1. **一页统揽（三页归一）**：`DataBuilderPage` 成为数据构建发动机的**唯一控制台**，消除与 `/admin/growth`(自成长驾驶舱) 及 `/admin/synthetic`(合成数据页) 的分裂——后两者分别降为本页内嵌区（区 6/区 8）与 rawin 填数模式（区 4）。保留"故事建域"与"快速合成"两个入口，不强迫先写故事才能合成。
   - **双输入（问题 ⊕ 自己的数据）**：用户既可输问题/故事脚本，也可上传自己的数据；comprehend 对账两者 → 倒推"为回答这些问题、在你已有数据之上还缺哪些字段/时序/切片/规则/workflow/agent/求解器"，自动补大部、求解器缺则出工单。
...
### 区 1 · 入口（复用 + 扩 · **双输入：问题 ⊕ 自己的数据**）
- **持续输入故事脚本/问题**输入框 + 「自动生成脚本」按钮（复用故事入口 PRD 的自动生成器）+ 「选择历史 run」下拉。
- **同时可上传自己的数据**（Excel/CSV/JSON，复用区 4 rawin 上传/模版校验）：用户带着「问题 + 一批已有数据」进来。
- **comprehend 对账双输入 → 倒推还缺什么**：把「问题要求的全栈制品」与「用户已上传数据能覆盖的对象/字段」对账——已覆盖的标 REUSE、未覆盖的标"需补"（原始数据字段/时序/切片/规则/workflow/agent/求解器），逐项进区 5 同步矩阵 + 区 6 故事覆盖度。即「**带数据来，发动机告诉你为回答你的问题还差哪些、并自动补大部、求解器缺则出工单**」。
- 数据来源：`POST /a/v1/databuilder/runs`（提交，body 含 `script` + 可选 `uploadedDatasetIds[]`）；`feature.data-builder` 门控（R3）。
