...
- ◐ **余 stage3（最后一块,侵入性最大）**：AgentCore 工具执行器运行时关卡——`query_objects`/`search_knowledge`/MCP 工具返回对象时经 `POST /a/v1/ontology/validate-output`(OBO) 校验,不符按 policy 拒/进隔离区 OC4 + 降 trustLevel;含未来 DL-as-MCP 输出兜底（决策红线3）。【覆盖"约束执行层 + 值域自动校验";对象类型级默认 policy 亦待补（当前连接器级 + 显式 + 默认三层够用）】
