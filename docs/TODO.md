...
- ✅ **g8 主线全部收口（P1–P6 已落）**：故事→全栈倒推→跨系统 scaffold→闭包→自检/压测→存量回填→推演回填,数据发动机成为"持续输入/自动生成故事脚本→倒推一切→历史推演记录"的发动机。
- ✅ **技术债已清（2026-06-18）**：① **跨系统 HARD 门前置 publish**（`executeStoryBuild` dry build→A 闭包→scaffold→全链闭合才真建落库,否则拒发布——R11 真阻断,非建完记终态）;② **倒推全栈扩 workflow/skill/agent**（comprehend 每求解器→工作流+技能+Agent;AgentCore scaffold 全建 DRAFT → skill/workflow/agent 页配置可见,直答"为何看不到对应配置"）;③ **mock 保真**（db.connections 加合成数据源、object-types fixture 扩到推演全套类型——直答 mock 模式"连接器/图谱看不到对应"）。回归:scaffold ×3 扩 workflow/skill/agent;f-suite 全绿。**经验证（curl 真后端 SEED_DEMO）**:连接器 1 合成源 + 26 RawDataset + 26 图谱类型 + 对象——推演非写死假数据,是合成真管线;"看不到"系 mock fixture 薄+单一全局源未按场景标注,现已补真+可经存量回填按场景生成。
...
| **本体统一可编辑面**（2026-06-18 用户观察） | ⬜ **能力已具但割裂 + 浏览器只读**：创建/编辑（`POST /a/v1/ontology/types` upsertType + 建模工作台 `ModelingPage` 草案→发布 R4）、分域（`/a/v1/ontology/domains` 注册表 + `DomainsPage` + upsertType 域 FK 校验，app.ts:932/948）**都已有**，但分散在 ModelingPage/DomainsPage 三页、且**本体浏览器/图谱 `OntologyGraphView` 只读**（仅检视器+CSV模版下载，无建/改/删/分域按钮）→ 体感"本体不能动"。**真缺口=统一可发现的本体编辑面 = 图谱即编辑面**：`OntologyGraphView` **分域维度已具**（colorBy=domain 着色/域图例/hiddenDomains 过滤/nodeFilter.domains 取子图）但**纯只读**——目标是从只读升为**可编辑图谱**（节点上直接 增/改/删类型、拖拽连边改 Link、改归域 domain），仍走草案+Action R4 + 字段全建模门。呼应 TODO #7（本体浏览器，🔄）但 #7 未含"浏览器即编辑面"。非缺能力，属 UX/可发现性收口 | `OntologyGraphView.tsx`(只读·已有分域着色) · 创建编辑在 `ModelingPage` · 分域在 `DomainsPage` · 后端 `upsertType`/`ontology/domains` 齐备 |
...
- ◐ **统一"全工具输出按本体类/值域强制校验"关卡 `validate_output_against_ontology`**（约束执行层最大缺口）——**核心已落（2026-06-18）**：① 可配置契约 `ValidationPolicy`（fieldMappings 适配不同源字段 + unknownFields/missingRequired/valueDomain/typeMismatch 各维度处置 REJECT/QUARANTINE/PASS_AND_MARK/DROP/COERCE + domainOverrides min/max/enum;R14 配置非硬编码）;② 纯函数 `validateOutputAgainstOntology(rows, typeDef, policy)`（先按 fieldMappings 归一→按 PropertyDef dataType/enumValues/PK + domainOverrides 校验,确定性 R6,violation 留痕 R13）;③ 端点 `POST /a/v1/ontology/validate-output`（**按租户**:读本租户对象类型,跨租户未知即 400）。回归 ontology-validate ×4（各维度 + fieldMappings + QUARANTINE 不拒 + R2 隔离）。- ✅ **stage2 持久化（2026-06-18）**：`Connection.validationPolicy`（连接器级,按租户）+ `PUT /a/v1/connections/:id/validation-policy` + validate-output 端点经 `connId` 取该源持久化策略为基线（连接器 ← 显式 body ← 全局默认）。回归 ontology-validate ×5（含 connId 取用 + 存得住）。
- ✅ **stage4 前端编辑器（2026-06-18）**：连接器字段画像页 `/admin/connections/:id/schema` 加 `ValidationPolicyEditor`——各维度处置下拉（野字段/值域/类型/缺主键）+ 外部字段→本体属性映射表（datalist 从源字段补全）+ 保存（PUT,按租户）。回归 f47 ×1。
- ✅ **stage3① 导入路径自动校验（2026-06-18）**：`ModelingService.materialize`（连接器→对象的真实导入路径）注入——来源连接器若配了 `validationPolicy`,导入时按本体值域/类型/枚举自动校验,不符行自动进隔离区 OC4（非显式调用,自动强制;无 policy 则向后兼容仅 PK/dup 隔离）。回归 stage3-import-validation ×2（值域违规自动隔离 + 无 policy 兼容）。
- ✅ **stage3② agent 推演侧执行器关卡（2026-06-18）**：工具执行器 `run()` 加 `expectsObjectType` opt-in——MCP/外部工具声明期望本体类型时,输出经 `ontology.validateOutput`(OBO `/a/v1/ontology/validate-output`) 强制校验,不符 → DENIED(拒脏外部数据进推演);`invoke_mcp_tool` 契约加 `expectsObjectType`;OntologyClient.validateOutput + DataCoreHttp + mock。**opt-in 零回归**(未声明则不校验)。含 DL-as-MCP 输出兜底（决策红线3:声明类型即过此关卡）。回归 stage3-mcp-validation ×3（不符 DENIED + 符合 OK + opt-in 兼容）。**余（小）**：对象类型级默认 policy（当前工具输出用全局默认;连接器导入用连接器级,已够）。
- ✅ **`validate_output_against_ontology` 全部收口**：核心(纯函数+端点,按租户) + stage2(连接器持久化+三层合并) + stage3①(导入 materialize 自动隔离) + stage3②(agent 工具输出执行器关卡) + stage4(前端字段映射+处置编辑器)。约束执行层最大缺口闭合;信任边界两条热路径(导入 + agent 推演)均自动强制。
