...
- ◐ **统一"全工具输出按本体类/值域强制校验"关卡 `validate_output_against_ontology`**（约束执行层最大缺口）——**核心已落（2026-06-18）**：① 可配置契约 `ValidationPolicy`（fieldMappings 适配不同源字段 + unknownFields/missingRequired/valueDomain/typeMismatch 各维度处置 REJECT/QUARANTINE/PASS_AND_MARK/DROP/COERCE + domainOverrides min/max/enum;R14 配置非硬编码）;② 纯函数 `validateOutputAgainstOntology(rows, typeDef, policy)`（先按 fieldMappings 归一→按 PropertyDef dataType/enumValues/PK + domainOverrides 校验,确定性 R6,violation 留痕 R13）;③ 端点 `POST /a/v1/ontology/validate-output`（**按租户**:读本租户对象类型,跨租户未知即 400）。回归 ontology-validate ×4（各维度 + fieldMappings + QUARANTINE 不拒 + R2 隔离）。**余（后续 stage）**：② policy 持久化（对象类型级默认 + 连接器级覆盖,两处仓储）;③ AgentCore 工具执行器运行时关卡(query_objects/MCP 输出经此校验,不符拒/隔离 OC4);④ 前端 policy/字段映射编辑（连接器字段画像页 `/admin/connections/:id/schema` 主入口 + 建模页类型级默认）。【覆盖"约束执行层 + 值域自动校验"两项;为 DL-as-MCP 输出兜底,决策红线3】
