...
- 🔄 **8. G-5 去电池锁死 / 多租户配置层**（本轮审计量化：范围远超"一行断点"，**撑不起其他租户/行业**）—— 大 —— **PRD 已出** `docs/PRD-de-battery-multitenant-config.md`（含 R14「应用层无业务常数」+ `debattery:check` 门）；本体 §5(R14)/§8(G-5) 已回写
  ...
  - **8b 业务数据写死（进生产、不可覆盖）**：✅ ProjectSim 型号/地址/物流（WorkspaceConfig）· ✅ GeoMap 基地坐标（Base.props.lon/lat）· ✅ Calibration 基地筛选（Base 对象全量）· 待办：PlanGenerate 目标默认值 · SopBalance 阈值+三段 —— 应从 WorkspaceConfig/SolverParams 取
