...
- ◐ **g8-P1（A 栈先行·G-6 残留收口）** —— **持久层+端点+时间线已落（2026-06-18）**：① `StoryBuildRun`/`InputManifest`/`ScaffoldReceipt` 契约（`packages/contracts/src/storybuildrun.ts`，嵌 BuildPlan/ClosureReport/GapReport，P2/P3 字段 forward-compatible 可选）+ 仓储双实现（R9 四处：repo.ts + memory.ts + pg.ts `story_build_runs` + migration015）；② `POST/GET /a/v1/databuilder/runs` 端点（`runStory` 复用既有七阶段 run + 仓储差集精确捕获产出连接器/数据集，不改既有管线）；③ **历史推演记录前端时间线**（DataBuilderPage `sbr-timeline`：建域并记入历史 + 逐 StoryBuildRun 展开见闭包/全栈计划/产出源数据→连接器页下钻）；本体 §2 回写 StoryBuildRun。回归：storybuildrun-repo ×2 · databuilder SBR1/SBR2 · f46 ×1。**余**：rawin 去模板化统一到 `SyntheticService`（删 `genCsv`，`generateFromSchema` schema 驱动·battery 字节级回归锁 R6，**改既有合成路径，先上回归锁再动**）【覆盖需求 O 过程数据持久化】
