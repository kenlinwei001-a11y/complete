...
- ◐ **余 stage3②（最重,侵入工具执行器热路径）**：AgentCore 工具执行器运行时关卡——`query_objects`/MCP 工具返回对象时经 `validate-output`(OBO) 校验→拒/隔离/降 trustLevel;含 DL-as-MCP 输出兜底。需补对象类型级默认 policy（工具输出不绑连接器）。**单独做以免碰坏 232 测试**。
