...
- 🔄 **4. 结论溯源覆盖 backlog**（附录A 优先级）：✅ **S&OP 六卡（三线 需求/供给/缺口 + 收入/毛利/现金）统一走共享 `<Provenance>` 六要素**（取代原 2 要素自绘浮层 → R-一致「一个事实一个出处」；C21/C15/C18 规则两跳；f17 回归改悬浮断言）· ⬜ 产能推演峰值/对策量 · ⬜ 订单全链 DAG 关键数字 · ⬜ 体检输入项
