...
> ### 📌 并线注记（2026-08-13 · R10 收编时逐条实测，非转述）
>
> 本文档写于 2026-08-08，收编到 canonical 时上面三处回写**已有两处落地**，
> 且 §5 的「1 接线 / 4 缺口」这个比例**已被 canonical 推进到 4 接线 / 1 缺口**。
> 逐条复验（工具自证：同文件 `G-DECISION` 类断点命中非零，故非 grep 坏了）：
>
> | 回写项 | 今日 canonical 实况 | 判定 |
> |---|---|---|
> | 断点表新建 `G-SIM-EVENT-NOSUB` | `docs/SYSTEM-ONTOLOGY.md` 命中 1 处（断点表内），条目自带「立条时（2026-08-08）」时间戳 | ✅ **已落地** |
> | 事件表消费方列 | `apps/frontend-shell/src/store/eventInvalidation.ts` 的 `EVENT_INVALIDATES` 已登记 `sim.scenario_saved` / `sim.session_created` / `sim.branched` / `sim.tick_completed` 四条；同文件缺口台账只剩 `sim.checkpoint_saved` 一条，并注明其成因与那三条**不同类** | ✅ **已落地且已超出本文档口径**（本文档写的 1/5，今日是 4/5） |
> | 链路章新增 **L18 推演沙盘环** | 本体链路编号今日到 `L17` 为止，`L18` **零命中** | ❌ **仍未回写**（残留项，见 §6） |
>
> ⚠️ §0 与 §6.0 的「PRD 字面 A10 未关」**今日仍然成立**，未过期：
> `chain.scan_completed` 全仓唯一命中仍是 `packages/contracts/src/chain-sim.ts:1080` 的一句注释，
> `chain.impediment_resolved` 仍**零命中** ⇒ 生产者一侧依旧不存在。
> 故本文档 §0「勿据本单判 A10 通过」这一条**不许当作历史包袱略过**，它是现状。
