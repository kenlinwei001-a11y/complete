...
#### 裁定三：**不做八个新页面；沙盘完整复用既有模块**

仓主原文：「**我不需要你做8个功能和页面，这些都是用于借鉴的**」
「**"决策推演沙盘"要完整复用系统已有的数据、已有的功能模块的能力，不要重复造轮子**」

⇒ 八页 UI PRD 降为**借鉴清单**，不是实施规格（详见 `docs/RECONCILE-twin-8pages-ui.md`）。
⇒ 本次交付面收敛为**两件**：
  ① **沙盘接真数据** —— `docs/WO-PACK-twin-data.md` 六单（D0/D1/D3/D4/D5/D6）
  ② **沙盘复用既有引擎** —— 复用台账见 RECONCILE §1.5（20 项能力已全部存在）
⇒ **派单红线**：凡 WO 里出现「实现 X」而 X 在复用台账里，一律退回改成「调用 X」。
