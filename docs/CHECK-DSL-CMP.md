...
> 计数不是手数的：由 `scratchpad/tally.mjs` 从本文表格逐行解析 `档` 列得出
> （规则：行首匹配 `| D\d{3} |` / `| C\d{3} |`，取第 3 列起**第一个以档位符号开头**的单元格）。
> 编号完整性同法校验：**D001–D215 无缺号、无重号；C001–C240 无缺号、无重号**。
...
| ✅ 实体层真满足 | 45 | 20.9% |
| 🔗 有实现·接线不全 | 24 | 11.2% |
| ⚠️ 只有 test 引用 / 已排练 | 4 | 1.9% |
| ❌ 无承载物 | 115 | 53.5% |
| ⛔ 文档自标非目标 | 27 | 12.6% |
| **合计** | **215** | 100% |

⛔ 27 条三分：**绝对不做/二期** 1（D037）· **本期不做/另立单** 12（D028/D038/D042/D052–D055/D146/D155–D157/D215）·
**不改不新造（反向断言）** 14（D003/D004/D007/D014/D015/D020/D025/D033/D034/D068/D172/D173/D174/D194）。
反向断言 14 条**逐条查完：无一条被违规做了**；唯一偏离是 D034/D194 的 `execution.mode` 词表分裂（已单列说明）。

> ✅ 45 条里有 **17 条属"文档层交付 / 对现状的事实断言复核正确"**
> （D045–D051 交付项 · D059 P4 · D158 §5.1 判据 · D182 §8.1 分流判据 · D190/D191 命名举证 · D209–D213 事实核实）——
> 这类条目证明的是**"PRD 自己写对了"，不是"系统实现了"**。
> **只看实现层：DSL ✅ = 28/215 = 13.0%。**
...
| ✅ 实体层真满足 | 77 | 32.1% |
| 🔗 有实现·接线不全 | 39 | 16.3% |
| ⚠️ 只有 test 引用 / 已排练 | 1 | 0.4% |
| ❌ 无承载物 | 95 | 39.6% |
| ⛔ 文档自标非目标 | 28 | 11.7% |
| **合计** | **240** | 100% |

⛔ 28 条三分：**绝对不做/二期** 5（C028/C049/C053/C177/C185）· **本期不做/另立单** 9（C014/C021/C048/C050/C150/C152/C153/C166/C174）·
**不改不新造（反向断言）** 14（C007/C013/C031/C037/C051/C052/C088/C144/C145/C146/C165/C170/C182/C208）。

> ⚠️ **CMP 的 ✅ 32.1% 会骗人，必须拆两刀**：
>
> **第一刀 · 剔"盘点正确"**：77 条 ✅ 里有 **26 条**是「CMP 对**现状**的事实断言经复核正确」
> （C001/C010 · C054–C067 §2.1「今天真有的」· C060/C076「断言已过期＝今天已有」· C081 · C147 · C175 · C178 ·
> C188–C190 §7.1 · C237/C238 诚实边界）。它们证明**盘点做对了**，不是**CMP 的能力做出来了**。
> ⇒ **实现层 ✅ = 51/240 = 21.3%。**
>
> **第二刀 · 剔"未并"**：`档` 列标 **[未并]** 的共 **34 条**（✅ 17 · 🔗 16 · ❌ 1），全部依赖 `compiler-s1` 合并才成立。
> ⇒ **canonical 上今天真成立的实现层 ✅ = 51 − 17 = 34 条 = 14.2%。**
>
> **[未并·仍❌] 13 条**（并了也不满足，须另立单）：
> `SkillRuntimePackage`(C003/C093/C215) · `SkillPackageManifest`(C004/C094) · `skill.compiled` 事件(C012) ·
> `skill.compiler` entitlement(C017·**R3 被违反**) · `SkillSource`(C090) · `RefResolver` 可注入 IO 层(C099) ·
> Optimizer(C100) · GR-BUDGET 声明期校验(C121) · publish 装配第 5 段接编译(C160) ·
> `POST /b/v1/skills/compile` 无 id 版(C200) · `SkillRequiresSchema`(C213)。
...
⛔ 55 条三分：绝对不做/二期 6 · 本期不做/另立单 21 · 不改不新造 28。
**28 条反向断言逐条查完：无一条被违规做了**（这是本轮唯一全绿的一档）。
