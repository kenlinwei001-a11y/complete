...
- **前后端闭环 + 像素级（凡触及前端的 WO 强制·用户令）**：验收**不得只 curl 后端**。必须：① **真起前端+后端**按**真实业务场景**跑（非合成 happy-path）② **数据前后端可见**——后端有的数据，在前端对应页面上**看得见同一值**（后端 curl 得 X → 前端页面显示 X）③ **前端像素级辨识**——真浏览器(Playwright)渲染 + **截图** + 辨识**显示的确切值/元素/状态**（closure 里写：截图路径 + 看到的确切内容 + 与后端值比对），非"应该显示"。只后端 curl 绿 = 前端未验 = **不得 done**。若前端真跑受环境限制 → 诚实标"前端未实拍" + `block`（**绝不凭后端绿 done**）。
