...
### 4.5.1 · ⚠ 给审核方的预警：本仓有两处**挂在墙钟上**的断言，并发跑门必炸

第二轮全量复跑（load 8.76）红了 3 条，**全部是机器速度伪影，零断言/逻辑失败**：

| 红的条目 | 失败原文 | 性质 |
|---|---|---|
| `empty-tenant-bootstrap` × 2 | `Test timed out in 180000ms` | 超时 |
| `solver-context-lazy-loading.seam` · **SEAM-PERF** | `冷启（内存模式）≤80ms: expected 90.65490299999874 to be less than or equal to 80` | **墙钟阈值** |

**关键区分（别把这条当成本单的锅，也别当成没事）**：同一次跑里，该门的**等价性三条全绿**——
`SEAM-EQ 逐字节一致` ✓ / `SEAM-EQ 生产路径 invoke(flag off) ≡ invoke(flag on)` ✓ / `SEAM-FLAG-OFF` ✓。
**炸的只有 `≤80ms` 那一行速度界**。即：**我点亮 `dc.lazy-solver-context` 的前置判据（逐字节等价）在那一轮里是真绿的**，
挂掉的是一条与正确性无关的性能界。安静车道单独复跑该文件：**RC=0 · 6/6 全绿**（SEAM-PERF 4481ms 通过）。

**为什么要写进 PRD**：`SEAM-PERF` 的 `≤80ms` 与 `empty-tenant-bootstrap` 的 180s 默认时限都是
**绝对墙钟数**，在 4 核机上只要有第二个 datacore 套件并跑就会红——**而它红的时候看起来和真回归一模一样**。
审核方跑「组合四包 gate」时若恰好与别的 agent 撞车，会拿到一条**指向本单的假红**。
判据：先看失败原文是 `timed out` / `≤80ms` 还是真断言；是前者就单独复跑该文件再定性。
（本单**不改**这两个阈值——那是别人的文件、且"为了让门变绿去放宽门"正是本仓在治的病。只登记。）
