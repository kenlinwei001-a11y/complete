...
- **测试方法学自纠（诚实·撤回误报）**：我一度用**根级 `npx vitest`** 跑 `solvers`/`m11-calibration` 得 16 红并误判为
  "team 分支 pre-existing 红/dev2 回归"。**实为方法学误报**：根级调用**不吃 `apps/datacore/vitest.config.ts` 的 `testTimeout=180000`**、
  退回默认 **5000ms**，而这些重测真跑 47–62s 必超时（含 `git stash` 基线复跑亦同源误报）。用**正确配置**
  `pnpm --filter datacore test` 全跑：**datacore 792 passed · 0 failed（146 文件·含本 WO 9 测）** → **team 分支 datacore 全绿·无 pre-existing 红**。
  本 WO 纯 additive（新契约/纯函数/新端点/1 feature key/1 import）·gates+debattery(R14=0) 绿。**教训**：跑重测必用包级配置，勿根级 `npx vitest`。
