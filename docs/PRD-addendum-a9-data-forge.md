...
## 9. 售前演示剖面（PROFILE_DEMO——首期实施范围）

> 用途定调：生成**给潜在客户看的、内行不穿帮的业务数据**。同一 GenSpec 体系，两种剖面：`PROFILE_DEMO`（本节，首期）与 `PROFILE_VALIDATION`（§5 工程验证，后期）。演示场景下"真实感"的评判者是行业内行的直觉，不是统计检验——本节据此调整范围与校验器。

### 9.1 范围裁剪（相对 §1–§8 全量，首期约省 60% 工作量）

| 能力 | DEMO 剖面 | 理由 |
|---|---|---|
| 提示词编译 + 配置器 + 预览 | **必做** | 按潜在客户行业/规模快速定制是核心价值 |
| 行业字典（§9.2）+ 内行可信度校验器（§9.3） | **必做（本节新增）** | 演示真实感的主承重 |
| 运营态回放联动（lived-in 12 个月历史） | **必做** | "运行了一年"的效果由此而来 |
| 因果规则 + 本体事件互引 | **必做** | 下钻不穿帮的前提 |
| 脏污注入器 | **降级**：强制 `preset=clean`，仅保留"叙事性不完美"（§9.3-5） | 技术性脏数据会让演示难看，且演示不验证管线健壮性 |
| 工程验证场景库（§5）、贴样模式（§2.3）、口径冲突 | **后期** | 与售前无关 |

### 9.2 行业字典（IndustryLexicon，随行业模板配套）

LLM 编译器与 `llm_text` 字段共用的行业知识包，每行业一份：

```ts
interface IndustryLexicon {
  industryKey: string;
  naming: { customers: string[]; productPatterns: string[]; processTerms: string[] };
    // 命名风格池：客户名像真实企业（"XX新能源科技""XX汽车集团"），禁止"客户A/测试1"
  plausibleRanges: Record<string, [number, number]>;
    // 量级红线：电池业 良率[0.88,0.96]、OEE[0.55,0.85]、电芯单价、人均产值……内行一眼扫的就是量级
  calendar: { springFestivalDip: number; holidays: string[]; weekendMode: "continuous" | "weekday" };
    // 中国制造业日历：春节 ±15 天产出深凹是最大的穿帮点；电池厂连续生产 vs 离散加工双休
  concentration: { customerPareto: number; productPareto: number };   // 前 20% 客户占比 ~70%
}
```

### 9.3 内行可信度校验器（Credibility Lint，替代 §4 在 DEMO 剖面的角色）

生成后自动扫描，**任何一条不过都阻断交付给售前**（与 §4 的"报告不阻断"相反——演示数据穿帮即失败）：

1. **量级合规**：全部数值字段落在 plausibleRanges 内；KPI 不得恰好踩整（良率 99.9%、达成率正好 100% 即拒）；
2. **日历真实性**：春节窗产出凹陷 ≥ 字典系数；节假日/周末效应符合 weekendMode；
3. **命名真实感**：无占位符模式（`/测试|demo|样例|客户[A-Z]|test/i`）；命名风格命中字典池；
4. **集中度**：客户/产品帕累托偏差 ≤10pct；
5. **叙事性不完美（强制存在）**：延期订单 ≥8%、被拒审批 ≥10%、至少 2 个未达标月份、至少 1 次处置失败记录——**全绿的系统比有疤的系统更假**；
6. **整数痕迹**：连续数值字段中"圆整值"占比 ≤5%（真实实绩极少是 40.0）。

### 9.4 售前定制流程（运营口径）

售前输入：潜在客户行业 + 规模 + 1–2 个痛点关键词 → 提示词编译 GenSpec（行业字典自动挂载）→ 运营态回放 12 个月 → Credibility Lint 全绿 → 产出独立演示租户（含演示账号），**目标时效：半天内交付一个定制 demo 租户**。演示租户带 SYNTHETIC 水印的展示策略由售前在功能开通里按客户决定（可关水印，合同/口头披露责任在售前流程，不在系统）。

### 9.5 DEMO 剖面验收（替代/优先于 F3/F6/F7/F8）

| # | 用例 | 预期 |
|---|---|---|
| D1 | 量级 lint | 注入良率 0.999 的规约 → 生成被阻断且指明字段与字典区间 |
| D2 | 春节凹陷 | 1–2 月产出时序凹陷显著（t 检验）且与日历字典系数一致 |
| D3 | 命名 | 占位符正则零命中；客户名风格命中字典池 ≥95% |
| D4 | 叙事性不完美 | 延期/被拒/未达标月份三项下限全部满足 |
| D5 | 半天交付演练 | 从一句话输入到可登录演示租户 ≤4 小时（含人工确认环节） |
| D6（人工） | 内行盲测 | 行业背景评审员浏览 10 分钟，指出的"假数据"疑点 ≤2 处且均不致命 |
