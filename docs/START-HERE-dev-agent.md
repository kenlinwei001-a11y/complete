...
| **P** | 1:1 复刻·**建模族**（数据流DAG+L0-L4认证+对象配置） | `SPEC-replica-modeling-family.md`（**先读地基 `SPEC-replica-design-system.md`**） | P1 | 竞品像素级复刻 ModelingPage：左画布换数据流DAG(接轨L provenance·复用PmDag/FdeGraph)+中L0-L4认证面板(接 deriveCertification 现成)+对象配置抽屉+逐对象gauge；**接现有不新建并行·认证数字接真closure非写死·无外部产品名·域色theme-invariant**<br>**【评审棘轮 6-27】① 数据流DAG ✅复验闭合(92160da)** ② **L0-L4认证 ✅复验闭合(bc93ff0)**——审核方两路真跑取证(curl 真 deriveCertification + 真浏览器):绿环35%/L1真级/三维54·100·28·18/L4四真勾+二显式RESERVED **全=后端实算值,非写死**;entOff 诚实降级亦验真。<br>**③ 对象配置抽屉+逐对象gauge ✅复验闭合(3730ef5)**——审核方三重取证:curl 8 对象 LOCAL cert 全异 + 真浏览器 3/3(Order83%/Base65%/Line100% 各匹配自身 LOCAL oracle·三维各异·**非复用全局35/54**) + PATCH e2e(setDomain/renameProperty 真落库·刷新仍在·非乐观);已发布草案**只读诚实降级**(后端409锁+前端不提供假编辑)、③类tab/字段标RESERVED不画假。<br>**④ 收尾 ◐(f51a022·UI真·可合 / 端到端QOS待一行修)**——审核方真跑+网络拦截:6子tab全渲染(图查询**显式RESERVED无假壳**·日志真outbox30事件·Skills/MCP真B4/B3或诚实降级)、Agent指挥台**真发`POST /b/v1/queries`带selectedObjects presetContext注入(补G-3·非假壳)**、token主题——**这些✅可合**。🔴**但端到端QOS 404 PACKAGE_NOT_FOUND**:先存共享bug `components/ScenarioLauncher/useScenarioLaunch.ts:22` `packageId=workspace?.scenarioPackages[0]` **少`.id`**(契约 scenarioPackages 是{id,name}对象数组)→全平台**6处quickLaunch**(场景启动器/CommandPalette/HomePage/Dashboard/本Agent台/DataBuilder)全404跑不通。**一行修 `scenarioPackages[0]?.id ?? ""`**(审核方已用正确字符串验 QOS 返202+taskId+streamUrl)。**待dev补这行+实拍一次"点推演→/v/dash真出QOS流"→复审通过再标✅**(非dev增量4引入,但卡死招牌G-3功能,归收尾)。**③类图查询页后端未建→不做·登记TO-DO(`design-system §10.1`·禁硬造壳)。** |
