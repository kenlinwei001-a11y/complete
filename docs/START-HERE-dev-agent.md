...
**一切以"解决根本问题"为原则，不以"省事 / 快速见效"为原则。** 选实现路径、给评审/用户提建议、决定"够不够交"时，都按"哪个真正解决根本问题"取舍——根因解 > 权宜解，哪怕更重更慢。"这样最省事 / 最快出效果"**不是**理由。诚实兜底 / 标 RESERVED 是地板（必须做），但不等于解决了问题，须同时指向并推动根因解。详见 `.claude/skills/fde-delivery` 铁律 0。
