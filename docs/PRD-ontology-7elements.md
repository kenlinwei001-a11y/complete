...
| 建议编号 | 断点 | 链路位置 | 性质 |
|---|---|---|---|
| `G-SOLVER-OUTPUTSHAPE-SEAM-DROP` | **A 侧已算好并返回的 `outputShape`，在 B 侧 HTTP 客户端 map 时被丢弃**：`app.ts:2685` 返回 → `datacore-http.ts:373-382` 逐字段列举不含它（`clients.ts:196-199` 类型亦无）→ `projectSolvers` 无从填 → `renderDrilPackage:332` 恒退化成裸 key、`resource-router.ts:80-82` 恒取不到。**同一个 map 上方 3 行的注释正是「勿在接缝丢弃」**（上次修了 `answersQuestions/tags`，这个漏了）。demo 租户 `qos.dril-routing` 已开（`seed.ts:65`）→ **活路径**。 | `solverRegistry → datacore-http map → projectSolvers → resource-router → renderDrilPackage` | 🔴 未修 |
