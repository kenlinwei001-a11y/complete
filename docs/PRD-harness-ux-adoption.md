...
> ⚠ **2026-08-18 订正**：表外明账 4 → **3 条** —— B-2（U3 本体链面）随 WO-R13-ONTOCHAIN-PANEL 交付核销出表（去向写在 §4.2 表后）。
>
> 🔒 **口径注（2026-08-20 WO-SPLITACCOUNT-B-CLOSE 加，只收窄不改写既有叙述）**：
> **§4 表的「判不了 0」只覆盖留在表内的那一半。** 拆出去的那一半由 §4.2 的 `B-x` 明账承载，
> 其**在册性**（还在 / 有人认领 / 受理方真实存在 / 状态是真的）由门
> `scripts/check-harness-ux-splitaccount.mjs`（`harness-ux-splitaccount:check`）守。
> 换句话说：**「判不了 0」＋「B-x 明账在册」两句话合起来才等于「这十一条判据今天的真实状态」**，
> 单看前一句会把「我把它们挪走了」读成「我把它们验完了」。
> ⚠ 这一句本身也**不是**「都验完了」的意思 —— 门守的是**账**，不是账的内容（见 §4.2.1 诚实位）。
...
| **U3** | 「点任意节点看到 判定逻辑 / 输入数据 / 来源 / **本体链** / 规则」 | ① 点得开且看得到凭什么 ② 那五样**逐字齐全**（「本体链」在本仓多数页无对位实现） | ①，判据收敛到**来源 + 规则**两样（有这两样，另两样必然在） | ②「本体链逐字齐全」→ R13 溯源链验收线（不在本表射程）。**2026-08-18 核销**：WO-R13-ONTOCHAIN-PANEL 交付三面板本体链（门 B 判据⑤ 现算 5 面板 3 有链，账面理由不成立），结案记录见下文 4.2.1 节 |
...
> `WO-U10-THREE-PAGES`（U10 列 3 格：`project-sim`/`decision-play`/`plan-generate`）
> **两单从同一分叉点 `2a1a412b` 平行开出**，各自把合计写成 **99** 与 **98** ——
> **两个数都只算了自己那一半，取任一方都是错的**；正确结果是**逐格并集**后现算的 **102**。
> 形态（铁律 0.6）：**「我用『我这一单改完之后的合计』当作『表里现在是什么』的证据，而前者并不度量后者。」**
> 并集由脚本逐格现算并对撞检查（同一格两边改成不同值即报错退出，实测**零对撞**：两单翻的是不同列）。
>
> ⚠ **本行此前还脱过两次节，照实留账**（下面两条是历史，不是现状）：
> ① `WO-U3-DAG-REST` 落笔时发现改动前写的是「符合 93 · 不符合 31」，**和 = 132 对得上**，
> 但门现算与逐格手数两个独立口径都给 **95 / 29** —— **符合少记 2、不符合多记 2，两错抵消把「和 = 132」骗过去**。
> 形态：**「我用『四项之和 = 132』当作『四项各自都对』的证据。」**
> ② `WO-U10-THREE-PAGES` 动手前同样量到差 2 格，且 **没被任何机器拦住** ——
> `sim-ux-criteria:check` 的棘轮读的是**表**、不读这行字，脱节时它照样绿。
> ⇒ **对策落在门上而不是自觉上**：改表不改基线由 `sim-ux-criteria:check` 当场报「棘轮松弛」；
> 校验口径从此是**逐格**，不许再用「和对得上」当验算。

> **这一行的口径，以及它为什么只许有一行**（2026-08-17 解合并冲突时定死）：
>
> - **两个独立口径对过账**，不是抄来的：① 逐格数上表 = 符合 102 / 不符合 22 / 不适用 8 / 判不了 0（和 = 132）；
>   ② 门现算 `node scripts/check-sim-ux-criteria.mjs` 打印「符合 102（基线 102）」。两者一致才落笔。
...
> - ⚠ **本行 2026-08-17 又脱过一次节，照实记账（WO-U10-THREE-PAGES 实测）**：
>   WO-U10-THREE-PAGES 动手前，本行写着「符合 93 · 不符合 31」，而**当时表里逐格现算是
>   符合 95 · 不符合 29**（门同时也打印「符合 95（基线 95）」）—— **差 2 格**。
>   即：上一轮有人改了表格里的 2 格却没同改这一行。
>   这正是本节自己第二条警告的那件事又发生了一次，**而它没被任何机器拦住** ——
>   `sim-ux-criteria:check` 的棘轮读的是**表**、不读这行字，所以脱节时它照样绿。
>   ⇒ 本行今天由**两个独立口径**重算落笔（门现算 102 ＋ 逐格脚本重数 102/22/8/0，和 = 132），
>   其中 3 格（U10 × `project-sim`/`decision-play`/`plan-generate`）是本单翻的，
>   另外 2 格是把上一轮漏记的差额补上，**不是本单新翻的**——两笔账必须分开读。
...
> - **2026-08-17 WO-U10-THREE-PAGES（3 格，全在 U10 列，逐格取证见 §4.7）**：U10 `project-sim`/`decision-play`/`plan-generate` —— 三页最小可见字号 9.5 / 10 / 9px → **12px**（字号硬底），实测由 `--survey` 现算复核。
...
> ⚠ **2026-08-18 订正**：明账 4 → **3 条**（B-2 核销出表，去向写在 §4.2 表后）；那句「隔着」现在隔着 3 条。
>
...
- **符合 12 页（全闭，2026-08-17 WO-U10-THREE-PAGES 收口）**：12 页 `minFontPx` 逐页实测均为 **12**，
  `overflowPx` 与 `overflowUnreachable` 逐页均为 **0**。
  ⚠ **「12 页符合」不等于「12 页各自做对了」** —— 其中 8 页是被同一处改动一次性带过线的：
  `EdgeActivePanel.module.css`（12 页里 10 页共用的传导边面板）五处 `<12px` 全部提到 12px；
  下一个人在自己页里写个 10px，那一格立刻掉回 `不符合`。
- **原不符合 3 页 → 符合（WO-U10-THREE-PAGES，2026-08-17）**，逐页写清真凶与改法，不许只报总数：

  | 页 | 改前最小字号 | 真凶（file · 选择器） | 改法 |
  |---|---|---|---|
  | `plan-generate` | **9px** | `views/sim/RadarChart.tsx` 五维雷达轴标签，SVG `fontSize={9}`（装的是中文「盈利/规模/…」） | 字号搬进 `RadarChart.module.css` 的 `.axisLabel` = **12px** |
  | `project-sim` | **9.5px** | `views/sim/PmDag.tsx` DAG 节点「本步」角标 `fontSize={9.5}`（副标签 `fontSize={10.5}` 同批） | 搬进 `PmDag.module.css` 的 `.stepBadge` / `.nodeSub` = **12px** |
  | `decision-play` | **10px** | `views/DecisionPlayPanel.tsx` 比对矩阵表头裸 `<small>`，吃 UA 默认 `font-size: smaller`（`table.cmp th` 12px × 0.8333） | 显式声明 `DecisionPlayPanel.module.css` 的 `.thHint` = **12px** |

  ⚠ **这三处「静态门一处都看不见」，形态各不相同，不许合成一句**（这正是本次的教训）：
  前两处是 **SVG 表现属性** `fontSize={…}` —— 既不是 CSS 文件、也不是 `style={{…}}`，
  不在 `check-text-legibility.mjs` 的扫描面内，故该门 `floor` 计数**一直是 0**；
  第三处**源码里根本没有「10px」这个数**，它是 UA 默认值 × 继承算出来的。
  形态（铁律 0.6 句式）：**「我用『静态门 floor=0』当作『这一页没有小字』的证据，而前者并不度量后者。」**
  ⇒ 修法不止于把数字改大，而是**把字号搬进 CSS 显式声明**，让两道门量的是同一件事。
- ⚠ **「量到 ≠ 守住」在本列曾是真空**，本轮补上（见 §4.7）：棘轮页册 `PAGES` 只守 `sim-sandbox` 一页，
  另 11 页是登记在册的真债 ⇒ **本表这 12 格 U10 判词原本没有任何机器在守**。
  现由 `--survey` 与本表**逐格对账**（不一致即 RC=1），两个方向都咬。
...
### 4.2 被拆出去的那一半 —— 3 条明账，交给门 B（**本表不假装能判**）

§2.1 的六条改写各把「不可判的那半」拆了出来。**拆出去 ≠ 消失**，逐条登记如下。
门 B（真浏览器 / Playwright）至今**未建** —— 所以这四条今天**确实没有验收方式**，如实写在这里。
⚠ **2026-08-18 订正**：原四条中 **B-2 已核销**（判据⑤ 现算推翻账面理由 ⇒ 重判 ⇒ WO-R13-ONTOCHAIN-PANEL 交付 ⇒ 销账，去向见表后）。在册 **3 条**。
...
> ✅ **B-2 核销去向（2026-08-18 · WO-R13-ONTOCHAIN-PANEL）**：账面理由「本仓多数页无『本体链』对位实现」被门 B 判据⑤
> 现算推翻 —— 5 个面板文件中 3 个已长出对位实现（`InspectorNodePanel.tsx` / `SandboxConsole.tsx` / `ProjectSimView.tsx`），
> `3 × 2 > 5`，门按设计变红逼回重判。重判结论：这条账走完全生命周期（判不了 ⇒ 可判且不符合 ⇒ **已交付 ⇒ 销账**）。
> 交付物：三面板每个结论可点开本体链面板，**对象 / 边 / 规则三段逐结论接线**；后端没下发的段一律诚实位
> （`data-present="0"` + 「后端未下发」+ gaps 写明缺哪段、为什么缺），**前端一条链都不许编**。
> 充分条件「逐字齐全」由本单三张接缝测试咬死（`wo-r13-ontochain-{inspector,sandbox,projsim}.seam.test.tsx`，
> 渲染后读面板 DOM，三段独立断言 + 变异反证红在「面板在、规则不在」）。
> **剩余诚实余额**（不进本表 —— 它们不是「账」，是实现面的下一张单）：`DagNodeInspector.tsx`（四页共用面板）与
> `LayeredDag.tsx`（共享组件）未链化 ⇒ 另立挂账 WO（见本单交单报告「没做的部分」节）。
> 销账经门 B 判据⑥ `--tighten` 显式重记基线 —— 棘轮那次红就是这条账的销账凭据。

⚠ **这三条不许被读作「大概也行」。** 它们和「符合」是两个不同的命题 ——
**「表里判不了 0」与「这条要求已经验完」之间，隔着的就是这三条（另：B-2 已核销，见上）。**

> 🔒 **口径注（2026-08-20 WO-SPLITACCOUNT-B-CLOSE 加，只收窄不改写既有叙述）**：
> **§4 表的「判不了 0」只覆盖留在表内的那一半**；拆出去的那一半由本节的 `B-x` 明账承载，
> 其在册性由 `scripts/check-harness-ux-splitaccount.mjs` 守 —— 该门现算本节与 §2.1 / §4 主表 / §5，
> **四条同时成立才 RC=0**：① 编号还在（判据①⑥ 棘轮）② 有人认领（判据①②④）
> ③ **受理方真实存在**（判据③⑧：抽象受理方要可解析，`WO-…` 单号要在**仓里或远端**真查得到）
> ④ **状态是真的**（判据⑨：标了结案的账，其对应判据在 §4 主表里不许还有「判不了」格）。
>
> ⚠ **本注不为本节背书内容**。门守的是**账本身**在不在册，不是这三条账验没验完 ——
> 「门 B 全绿」与「B-1/B-3/B-4 已验收」是两个不同的命题，逐条差什么见 §4.2.1。
...
>
> ✅ **2026-08-18 三次订正（WO-R13-ONTOCHAIN-PANEL）**：**B-2 核销出表**。判据⑤ 现算 5 面板 3 有链 ⇒
> 账面理由不成立 ⇒ 重判 ⇒ 交付 ⇒ 销账（去向写在 §4.2 表后）。在册明账 4 → 3，
> 自陈「内容面机检」1/4 → **0/3**（B-2 不再是在册账，`CONTENT_CHECKED` 与在册账交集为空；判据⑤ 随其出表自动停检）。
> 下文逐条判定表里的 B-2 行保留为**结案记录**，不再是一条在册账。
...
⚠ **2026-08-20 增补（WO-SPLITACCOUNT-B-CLOSE）：七条 → 九条。上面那句「七条判据」自即日起读作前七条。**
新增两条，各堵一个上面七条堵不住的洞（两条都带正反金丝雀与磁盘级变异反证，见 §4.2.2）：

- **⑧ 受理方单号真实存在**：§5 里点名 `B-x` 的行所点名的每个 `WO-…` 单号，必须能在**仓里或远端**查到
  （有同名 git ref，或本 PRD **之外**的已跟踪文件提过它）。
  **为什么判据③ 不够**：③ 只问「点没点名一个**已登记的抽象受理方**」，其中「编排侧评测」那条的
  存在性判据是「§5 里有**任意**一张点名**任意** `B-x` 的单」—— 那是**代理指标**。
  实测：把 §5 派给 B-3 的 `WO-FACT-USAGE-REGISTRY` 换成一个编出来的名字，**③ 一声不吭**
  （「真浏览器」那三个字一个没动，`dispatchAll` 还有三行 ⇒ 恒真），门照样 RC=0。
  形态（铁律 0.6 句式）：**「我用『这张表里还有单』当作『这一条的受理方存在』的证据。」**
- **⑨ 结案状态不许说谎**：凡在 §4.2.1 里被标结案（`核销`/`已交付`/`已闭`/`已完成`/`已结案`）的 `B-x` ——
  ⑨a 不许**同时**还挂在 §4.2 在册表里；⑨b 必须在那一行点名交付它的 `WO-…` 单号；
  ⑨c 该单号必须能在 §2.1 找到**同样标了结案**的那条 `U#`（**只销一头 = 两处账对不上**）；
  ⑨d **该 `U#` 在 §4 主表里不许还有「判不了」格**；⑨e 结案记录上棘轮（只许增不许减）。
  **⑨d 是这条判据的本体**：`§4` 那个「判不了 0」是**拆账拆出来的读数**，
  拆出去的那半自称交付时，留在表内的那半必须真的判得了 —— 否则「判不了 0」就是假的。
  ⚠ ⑨e 存在的唯一理由是**防空转**：把结案行删掉，⑨ 就没有对象可判、整条自动变绿。
...
| **B-2** | ✅ **已核销 · 已交付**（2026-08-18 · WO-R13-ONTOCHAIN-PANEL）—— 本行是**结案记录**，不再是在册账 | 判据⑤ 现算 5 面板 3 有链 ⇒ 账面理由「多数页无对位实现」不成立，门红逼回重判 —— 正是这条账设计要拦的死法。重判结论：三面板结论 ⇒ 本体链三段（对象/边/规则）逐条落地，缺段诚实位（后端未下发 + gaps 写明缺因）；接缝测试三段独立断言 + 变异反证红在「面板在、规则不在」 | 已兑现：R13 溯源链验收（本单交付，见 §4.2 表后去向）。剩余余额：`DagNodeInspector.tsx` / `LayeredDag.tsx` 未链化 ⇒ 另立挂账 WO，不进本表 |
...
**判据⑤ 的现算口径（B-2 · 已完成使命，随账出表自动停检）**：扫描面 = §4.1 的 U3「**符合**」段里点名的**面板文件**
（现算不手抄；⚠ 旧版此处写「刻意不含 LayeredDag.tsx」已过期 —— WO-U3-DAG-REST 把它写进了「符合」段，现算面随之含它）。
逐个**剥注释后**看有无 `本体链`/`ontologyChain` 对位实现，并断言 B-2 账面那句「本仓多数页无对位实现」
**仍然属实**（`有对位实现 × 2 ≤ 面板文件数`）。**读数史**：2026-08-16 现算 = 3 个面板文件
（`ProjectSimView.tsx` / `SandboxConsole.tsx` / `InspectorNodePanel.tsx`）· 有对位实现 0 个 ⇒ 账面属实；
**2026-08-18 现算 = 5 面板 3 有链（`InspectorNodePanel.tsx` / `SandboxConsole.tsx` / `ProjectSimView.tsx`）
⇒ `3 × 2 > 5`，账面理由不成立、门按设计变红、B-2 重判后核销**（基线经 `--tighten` 重记为 5/3）。
这条判据的一生就是棘轮存在的意义：它没拦住实现落地，它拦住的是「实现落地了而账面理由还挂在那儿」。

**门 B 今天到底机检了这三条的哪一面 —— 自陈，且被门自己核对（判据⑦）**：
**内容面机检 0/3**。在册 **3 条（B-1 / B-3 / B-4）的内容面本门一律不机检**，
只机检它们的**账**：不许消失、不许悬空、不许无人认领、不许改绑不吭声。
（原 1/4 的那个 `1` 是 B-2 —— 它随账核销出表，`CONTENT_CHECKED` 与在册账的交集现为**空集**，故 `0`。
门脚本里 `CONTENT_CHECKED = {"B-2"}` 留着不动：它不是「本门还在机检 B-2」，
是「本门机检过的内容面只有过 B-2 这一条」的历史登记，判据⑦ 取的是它与**在册账**的交集。）
这个 `0` 是**门现算**出来的，谁把这句话改成 `3/3` 而没有真加判据，门当场红 ——
拦的正是本门建成之后最可能发生的那件事：**「门 B 建好了 ⇒ 那几条都验了」**。

#### 4.2.2 门 B 的变异反证与退出码（2026-08-20 WO-SPLITACCOUNT-B-CLOSE 磁盘级实跑）

⚠ **先记一笔本单开工时发现的账**：`--mutation-proof` 在集成线上实测**只有 3/6 在打**。
M2/M5/M6 三条的锚点写死了 `B-2` 与「内容面机检 **1/4**」这两串字面量，
而 **B-2 已于 2026-08-18 核销出表、自陈已改成 0/3** —— 锚点**静默失配**，
`--mutation-proof` 自己走的是 RC=2 那条路，而**门本体照常 RC=0**，于是没人发现
「用来证明这道门不是装饰品」的那件东西自己坏了。
形态（铁律 0.6 句式）：**「我用『门 RC=0』当作『门的判据被证明过会红』的证据，而前者并不度量后者。」**
**已改**：受影响的三条改挂在册的 B-3，且 §4 主表那条变异改成**结构化定位**（现算表头找列号），
不再写死行文本 —— 那张表的格子明天还会被别的单翻，写死行文本的锚点天生带保质期。

**退出码三分**（`0` 干净 / `1` 明账真有问题 / `2` **门自己坏了**；`2` 的含义是「我没查出来」，不是「合格」）。
下表每条都是**磁盘上真改文件 → 起进程跑门 → `out=$(… 2>&1); rc=$?` 显式取码 → `git checkout --` 还原**，
不走管道（`cmd | tail; echo $?` 取的是管道末端那条命令的码，恒 0）：

| 变异 | 改了什么 | 改前 RC | 改后 RC | 门说了什么 |
|---|---|---:|---:|---|
| **删一条明账** | §4.2 整行删掉 `B-3` | 0 | **1** | ②正向（U5 挪走了没人认领）+ ⑥销账**点名 B-3** + ⑦分母 3→2 |
| **受理方编出来的名字** | §5 里 `WO-FACT-USAGE-REGISTRY` → 一个仓里/远端都查不到的单号 | 0 | **1** | ⑧ 点名 `[B-3]`：「既没有同名 git ref，也没有本 PRD 之外的文件提过它」。**③ 一声不吭**（这正是加⑧ 的理由） |
| **状态说谎** | `B-2` 仍标「已核销 · 已交付」，而把 §4 主表 U3 列的 `sim-sandbox` 一格翻成「判不了」 | 0 | **1** | ⑨d 点名 `[B-2]`：「U3 在 §4 主表里还有 **1/12** 格是判不了」 |
| **主正则改坏** | `parseSplitAccounts` 的明账 id 正则 `B-\d+` → `B-\d\d\d+` | 0 | **2** | 金丝雀 11 条不中，首条「①必中样例：accounts=0（应 2）」。**不是 RC=0** |
| **主常量改坏** | `UNDECIDABLE = "判不了"` → `"判不了了"` | 0 | **2** | 「⑨必中样例·§4 主表：U9=0/2（应 1/2）」+「⑨d 没有红在 B-5 上」 |
| **单号正则改坏** | `WO_ID` 的 `WO-` → `WZ-` | 0 | **2** | 「§5 单号实得 []」「⑬必中·⑧ 实得 []」等 7 条 |
| **表头正则改坏** | §4 主表表头形状 `U\d+b?` → `Z\d+b?` | 0 | **2** | 「§4 主表列数=0（应 4）」 |
| **干净态** | 不改 | 0 | **0** | 九条全过；工作区 `git status --porcelain` 为空 |

**这张表要证的只有一件事**：金丝雀与主判据**共用同一份** `parseSplitAccounts()`/`judge()`。
若金丝雀另抄一份正则，改坏主正则后门会**照常 RC=0**（本仓 2026-08-08 实测过这个形态）——
上面四条「改坏 ⇒ RC=2」就是它没有另抄的凭据。**进程内**的 `--mutation-proof` 现为 **10/10**
（新增 M7 判据⑧ / M8 判据⑨d / M9 判据⑨c / M10 判据⑨e），三向断言：变异体≠原文 ·
判据编号集合恰好等于预期 · 被牵连的 `B-x` 集合恰好等于目标那一条（既证会红，也证不误伤）。

⚠ **诚实位（不许把本节读成「四条账验完了」）**：上面证的是**门会咬**，不是**账的内容对**。
...
### 4.8 WO-U10-THREE-PAGES 真改了哪 3 格（2026-08-17 · 逐格 file:line + 变异反证）

**这 3 格全在 U10 列**，且全是同一个病 —— **最小可见字号低于 12px 硬底**
（`check-text-legibility.mjs` 的 `FLOOR_PX`，它是该门判据 A 与本仓调色板的交点、非自由参数）。
逐页真凶与改法见 §4.1 的 U10 段那张表，此处只记**取证与反证**。

**改前/改后实测**（真 Chromium 1440×900 · 扫描根 `main` · 逐元素 `getComputedStyle().fontSize`）：

| 页 | `<12px` 元素数 改前→改后 | 最小字号 改前→改后 | `textEls` 改前→改后 |
|---|---|---|---|
| `plan-generate` | 5 → **0** | 9 → **12** | 267 → 267 |
| `project-sim` | 13 → **0** | 9.5 → **12** | 148 → 148 |
| `decision-play` | 6 → **0** | 10 → **12** | 184 → 184 |

⚠ `textEls` 三页**逐页不变**，是刻意并排列出的：本文件反复警告过「删内容冒充版面变好」
（版面门的守恒判据 C1/C2 就是为它建的）。字号改动不该动墨迹量，**数字不变本身就是没删东西的证据**。

**顺带清掉一条同病的存量红**（不在本单页范围，但不清就给不出「两道门改后都 RC=0」这个判据）：
`views/process/ProcessWaitView.module.css` 的 `.asOf` 11px → 12px ——
随 `445f7cf1`（WO-SCREEN-PLAINSPEAK ②）进的集成线，静态门基线 0 → 实测 1，当场判红。

**两道门 改前/改后**：

| 门 | 改前 | 改后 |
|---|---|---|
| `text-legibility:check` | **RC=1**（`ProcessWaitView.module.css` `.asOf = 11px`，基线 0→1） | **RC=0**（令牌矩阵 60 对全过 · B 低于 12px **0** 条） |
| `layout-legibility:check` | RC=0 | RC=0（沙盘棘轮六项逐项未变坏） |
| `layout-legibility --survey` | RC=0，但**表里 3 格判 `不符合`** | **RC=0 且逐格与表一致**（12 页全 `符合`） |

**变异反证（两道门各一次，逐条给实测原文）**：
- 把 `RadarChart.module.css` 的 `.axisLabel` 改回 `9px` ⇒
  `text-legibility:check` **RC=1**，报文原文
  `【新文件】【B 字号硬底 <12px】apps/frontend-shell/src/views/sim/RadarChart.module.css 0 → 1（+1）：L1 .axisLabel = 9px`
  —— **点名了文件与选择器**。还原后 RC=0。
- 同一变异下 `layout-legibility --survey` **RC=1**，且**点名了页与选择器**（实测原文）：
  `plan-generate：表里写「符合」，实测「不符合」（最小字号 9px < 12）`，
  现场 `<text._axisLabel_m49di_12>「盈利」@583,1063 9px`（另 3 个轴标签同）。
  —— 这是对账的**另一个方向**（表说 `符合` 而屏 `不符合` ⇒ 有人改坏了），
  与上面那条「修好了没回写表」方向相反；**两个方向各有一次实测原文，对账才算证完**。
  注意那个选择器能打出来，正是靠下面那条 `SVGAnimatedString` 修复 —— 否则此处会是
  `[object SVGAnimatedString]`，门红了也指不出改坏的是哪一档。
- 同一变异下 `layout-legibility:check`（门模式）**仍 RC=0** —— 这不是漏报，是**登记在册的真债**：
  棘轮页册 `PAGES` 只守 `sim-sandbox`。本单实测过「把 `plan-generate` 加进 `PAGES`」这条路，
  **当场 4 条红且全部在本单范围外**（`bodyFontPx = 12 < 下限 13` ×2 视口，属 `global.css` 的全站正文档；
  `misalignedGroups = 2 > 上限 0` ×2 视口，属栅格/IA）⇒ 那是另一张单。
  本单改走**对账**这条判据（见下），它不动任何绝对阈值。
  ⚠ **诚实边界（读者最容易在这里读错）**：`--survey` **不在 `pnpm gates` 串里**，
  也不在 `scripts/gate.sh` 里 —— 它只在有人手动跑时才说话。
  所以这三页「≥12px」这件事，**日常自动挡在门上的是静态那道 `text-legibility:check`**
  （字号已从 SVG 表现属性搬进 CSS，它因此看得见），`--survey` 是**第二道、手动的**对账。
  别把「`--survey` 会红」读成「CI 会拦住」。

**本轮建的机制（铁律 0.6 二级处置：不是「下次注意」，是机器先说话）**：
`check-layout-legibility.mjs --survey` 在打印普查表之后，**把现算的 12 格 U10 判词与本文件 §4 表逐格对账，
不一致即 RC=1**。判据刻意是「对账」而**不是**「一律要求符合」——
后者会把**诚实登记为 `不符合` 的欠账**也染红，于是下一个人消红的最短路径变成**改表**，
那正是本文件 §4 合计行那条老账警告过的事。对账两个方向都咬：
表说 `符合` 而屏 `不符合` ⇒ 有人改坏了（**修页面**）；表说 `不符合` 而屏已 `符合` ⇒ 修好了没回写表。
> 实测证据：本单三页改好、表还没改时跑 `--survey`，它**当场 RC=1 并逐格点名那 3 格**
> 「修好了没回写表」—— 回写这一步是**机器提出来的**，不是人想起来的。

**顺带修一个真缺陷**（它决定报文能不能「点名选择器」）：`scripts/lib/layout-probe.mjs` 取类名用
`String(el.className)`，而 **SVG 元素的 `className` 是 `SVGAnimatedString` 对象** ⇒ 打出来是字面的
`[object SVGAnimatedString]`。本单这三页最小字号的现场**全是 SVG `<text>`**，
也就是说门此前一直**指不出是哪个选择器**。改走 `getAttribute("class")`（HTML/SVG 同语义），
并在 `CANARY_BITE` 里加 `<text class="svgTiny" font-size="8">` 钉死。
变异反证：改回 `String(el.className)` ⇒ `--selftest` **RC=2**，报文原文
「类名抽成了「[object SVGAnimatedString]」而不是「svgTiny」」；还原后 RC=0。

**范围边界（本单没做什么，写明免得被读成已做）**：
- **只改字号/间距**，不动数据流与逻辑；`SandboxView.tsx` / `SandboxConsole.tsx` / `ShellLayout.tsx` /
  `locales/zh.ts` / `mocks/**` **一个字没碰**。
- `RadarChart.tsx` 的 `fill="#9AA8B6"` 是**写死的十六进制**（三套主题同一个值），
  **本单刻意不动** —— 那是「颜色不走令牌」的另一笔账，混进来会让本单的命题变糊。故意把 `fill` 留在 TSX 属性上
  也是为此：搬进 CSS 会让静态门拿它去判对比度，把两件事缠在一起。

**「哪些红是我造成的」—— 与未触碰基线逐门对比（2026-08-17 实测，非推断）**：

判据是 **merge-base**（`2a1a412b`）而**不是**「集成线现在的 tip」—— 拿已经前进的集成线当基线，
会把别人的改动算到本单头上。在该提交上另开一棵 worktree、**装同样的依赖、build 同样两个包**
（不装就满屏 `Failed to resolve entry for package "@platform/contracts"` 这类环境假红），
把 `pnpm gates` 里那 63 道门**逐条独立跑**（原串是 `&&` 连的，第一条红之后后面全被遮住），两边逐门对退出码：

| | 基线树 `2a1a412b` | 本分支 | 结论 |
|---|---|---|---|
| 红门总数 | **32 / 63** | **31 / 63** | 净 **−1** |
| 本单**造成**的红 | — | **0** | 无自造红 |
| 本单**修好**的红 | — | **1**（`text-legibility:check` 1→0） | 即 `.asOf = 11px` 那条 |
| 两边都红（既存债） | 31 | 31 | 与本单无关 |

- 那 31 条既存红里 **9 条是 `RC=2`**，报文原文一律是「请先构建再重跑 `pnpm --filter datacore build`」——
  **`RC=2` 读作「我没查」，不读作「你有问题」**（门自己的话），两者处置相反。它们两棵树上一模一样。
- 退一步的复核：**退出码相同**还不够，本单另比了每道门的**报文尾部**——
  24 道有字面差异，逐条看过全部是良性（受控文件数 `2143→2146`，即本单新增 3 个文件；
  绝对路径与分支名；以及本单有意的 `符合 95→98`）。**没有一道门在退出码相同的情况下变差。**
- ⚠ 附带发现（**既存债，本单不改**）：任何一次跑门都会把 `docs/prd-ontology-index.json` 改脏，
  **在未触碰的基线树上同样发生、且 blob 前后哈希逐字相同**（`d19de82c → 84d657f4`）⇒ 是索引在 canonical 上就已过期，
  不是本单产生的。故**刻意不提交它**：它还会把 `apps/agentcore/dist/mocks/seed.js`
  这种**取决于有没有 build** 的条目写进 `brokenArtifacts`，提交等于把环境状态腌进仓库。
- **两道门量的都是 `getComputedStyle().fontSize`，不是屏上的视觉大小**。`PmDag` 的 `<svg>` 带
