...
**合计（12 页 × 11 判据 = 132 格）：符合 101 · 不符合 23 · 不适用 8 · 判不了 0。**
...
> **本轮（WO-U1-U8-SMALL · U1 两格）逐格写清去向，不许只报总数：**
>
> | 页 | U1 改前 → 改后 | 凭什么 |
> |---|---|---|
> | `optimize-whatif` | 不符合 → **符合** | 撤 `ow-solve` 提交闸：四维输入进 queryKey + 800ms 防抖（每次重演 = 真 CP-SAT 两解，故比 what-if 的 300ms 宽）。**先测后裁**：运行点每改一格重解 p95≈9.2ms（页面示例 2×2）/ ≈1.6s（autoBind 演示规模 8×30），流畅；55s 尾部只在手造 2000 对基线 JSON 时出现（非运行点）。护栏 = 防抖 + react-query key 竞态（旧结果冒充新结果结构上不存在）。接缝：`optimize-whatif.test.tsx` U1 结构测（无 `ow-solve` · 计数忠实桩证自动首解 + 改值自动重调） |
> | `sop-balance` | 不符合 → **不适用** | 见 §4.3 —— 五个 `sop-run-N` 是 S&OP 五步法的**流程节点提交**（写版本记录 + 状态机推进 + `planLocked`），判据预设的「提交闸」在这页不存在。三判据区分表与失效条件在 §4.3 专段 |
>
> `global-sim` × U8（本单第三格）另行收口：先定明细抽屉放全量细排还是只放该单，再实现。
