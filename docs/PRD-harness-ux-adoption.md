...
**合计（12 页 × 11 判据 = 132 格）：符合 98 · 不符合 26 · 不适用 8 · 判不了 0。**

> **这一行的口径，以及它为什么只许有一行**（2026-08-17 解合并冲突时定死）：
>
> - **两个独立口径对过账**，不是抄来的：① 逐格数上表 = 符合 98 / 不符合 26 / 不适用 8 / 判不了 0（和 = 132）；
>   ② 门现算 `node scripts/check-sim-ux-criteria.mjs` 打印「符合 98（基线 98）」。两者一致才落笔。
...
>   ⇒ 本行今天由**两个独立口径**重算落笔（门现算 98 ＋ 逐格脚本重数 98/26/8/0，和 = 132），
...
### 4.7 WO-U10-THREE-PAGES 真改了哪 3 格（2026-08-17 · 逐格 file:line + 变异反证）
