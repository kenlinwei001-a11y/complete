...
| 全局项目推演 `global-sim` | **符合** | 不符合 | **不符合** | **符合** | 不适用 | **符合** | **符合** | **符合** | **符合** | **符合** | **符合** |
...
| 优化推演 `optimize-whatif` | **符合** | **符合** | **符合** | **符合** | 不适用 | **符合** | **符合** | **符合** | **符合** | **符合** | **符合** |
...
| 月度规划 `sop-balance` | **不适用** | **不符合** | **不符合** | **符合** | 不适用 | **符合** | **符合** | **符合** | **符合** | **符合** | **符合** |
...
> **本轮（WO-U1-U8-SMALL · 三格全收）逐格写清去向，不许只报总数：**
>
> | 页 | 判据 | 改前 → 改后 | 凭什么 |
> |---|---|---|---|
> | `optimize-whatif` | U1 | 不符合 → **符合** | 撤 `ow-solve` 提交闸：四维输入进 queryKey + 800ms 防抖（每次重演 = 真 CP-SAT 两解，故比 what-if 的 300ms 宽）。**先测后裁**：运行点每改一格重解 p95≈9.2ms（页面示例 2×2）/ ≈1.6s（autoBind 演示规模 8×30），流畅；55s 尾部只在手造 2000 对基线 JSON 时出现（非运行点）。护栏 = 防抖 + react-query key 竞态（旧结果冒充新结果结构上不存在）。接缝：`optimize-whatif.test.tsx` U1 结构测（无 `ow-solve` · 计数忠实桩证自动首解 + 改值自动重调） |
> | `sop-balance` | U1 | 不符合 → **不适用** | 见 §4.3 —— 五个 `sop-run-N` 是 S&OP 五步法的**流程节点提交**（写版本记录 + 状态机推进 + `planLocked`），判据预设的「提交闸」在这页不存在。三判据区分表与失效条件在 §4.3 专段 |
> | `global-sim` | U8 | 不符合 → **符合** | **设计裁决：抽屉放「该单在本版联合解里的明细」，不放全量细排**（理由三条：① U8 要的是看明细不丢现场，数已在屏上、零新取数、与台账/排产表同源勾稽；② 全量细排 = project-sim 整页试算台 = 「做别的事」，判据明示跳去做别的事不算违反，出口收进抽屉 `gs-drawer-goto-project`；③ 抽屉内另跑单排试算会与全局联合解同屏打架）。三处下钻口（台账行 / 被挤单卡 / 固定单卡）由 `<Link>` 跳页改 `GlobalSimOrderDrawer` 受控抽屉；wip/forecast 仍不可下钻只标注。接缝：`wo-sandbox-u8-globalsim.test.tsx` 3 测（交付日与排产表逐字相等 · 被挤量与卡片逐字相等 · 路由不动）＋ `global-sim-drill-seam`/`global-sim-glass` 两口旧跳页断言同步改咬抽屉 |
...
- ✅ **`optimize-whatif` 于 2026-08-17 WO-U1-U8-SMALL 闭（不符合 → 符合）——先量后撤**：
  真 CP-SAT sidecar（`services/optimizer`）直接计时 60 次/档（单次求解 p50/p95；本页每次重演 = 基线+扰动 **2 解**）：
  页面示例 2×2（4 对）p95 4.6ms→每格 ≈9ms · 演示规模 8×30（240 对）p95 805ms→每格 ≈1.6s ·
  12×50（600 对）p95 1.8s→每格 ≈3.7s · 压力上限 20×100（2000 对）p95 27.6s→每格 ≈55s（非本页运行点）。
  结论：**撤**。`ow-solve` 提交闸已删，入参 `{family,baseline,perturbs}` 经 **800ms 防抖**直接进 `queryKey`
  （what-if 是 300ms——本页每发一次是真 CP-SAT 两解，窗口相应放宽）；屏上留状态记号 `ow-live-state`
  （重解中/已按当前输入解出），react-query 以 key 取数 ⇒ 屏上永远只显示最后落定那版入参的解。
  测试：`optimize-whatif.test.tsx` 新增 U1 条（无闸 + 改扰动值自动重调 + Δ 真变），C1/C2/C3 同步去点击化。
- ⬜ **`sop-balance` 于 2026-08-17 裁毕（不符合 → 不适用）**：五个 `sop-run-N` 是 S&OP 五步法的
  **业务流程节点**，不是提交闸 —— 逐格取证与「不适用/不符合」的界限判据见 §4.3 专段。
...
| **`sop-balance`** | **U1** | 五个 `sop-run-N` 是 S&OP 五步法的**流程节点提交**（写版本记录 + 状态机推进 + C22 锁），不是「改输入必须点才重算」的提交闸 —— 详见下方「U1 那一格为什么是不适用」 |
...
**这 9 格全部随页面结构变化而失效，届时必须当场改回 `不符合`**：
U4b 的 7 格 —— **若将来这几页长出了因果图**，判据的对象出现了，豁免就失效；
U4 的 1 格（`cleanroom-attr`）—— **若本页长出以 `SimSession` 为世界的推演块**，同理失效；
U1 的 1 格（`sop-balance`）—— **若本页将来长出步内的推演试算块**（改输入不点不重算），
那一块单独按提交闸标准量，本格豁免不自动覆盖它（详见下方「U1 那一格为什么是不适用」）。
...
#### U1 那一格为什么是「不适用」（2026-08-17 · WO-U1-U8-SMALL 语义裁决）

**问题**：`SopBalanceView.tsx` 有五个 `sop-run-N` 按钮（探针词表曾把它们漏掉，见 §4.1 金丝雀教训）。
若它们是「改完输入不点就不重算」的提交闸，U1 该判 `不符合`；若它们是别的什么，先定性再判。

**取证（追到服务端，不停在前端）**：五个按钮全部调同一个通道
`advanceSopVersion(v.id, step, payload)` → `POST /a/v1/sop/versions/:id/advance` → `sop.ts advance()`：
① 把本步结果**算出来并写进版本记录**（`v.steps.s1 = …` · `repos.sopVersions.put`）；
② **推进状态机**（DRAFT→IN_REVIEW→…→FINAL，步序强约束「run step 1 first」，FINAL 后 `planLocked`）；
③ 载荷是本步的**决议数据**（segments / increments / form / resolutions）——是要落账的月度规划输入，
不是「试算假设」。五步即 S&OP 评审流程本身（产品评审→需求评审→供应评审→财务校验→高管决议）。

**定性**：这是**流程节点提交**，与 U1 点名的提交闸是两个物种，三条判据任一即可区分：

| 判据 | 提交闸（what-if 旧 `wi-run`） | 流程节点（`sop-run-N`） |
|---|---|---|
| 不点会怎样 | 屏上挂着**上一次的旧结果**，且分辨不出（最坏失败模式） | 那一步的区段**根本不存在**于版本记录里——没有「旧结果冒充新结果」 |
| 点下去做什么 | 触发一次**重算**（同一输入算多少遍都一样） | **写入 + 状态机推进 + 留痕**（不可重入，FINAL 后锁死） |
| 「改输入即重演」是否适用 | 是——输入是假设，推演页的天职 | 否——输入是**决议**，每敲一个字就写进版本仓才是事故 |

同形态先例已在本表 U1 段认定过一次：「沙盘推进一格 tick 不算提交闸——那是时间前进，不是填完表再点一下」。
流程节点同理：**它是业务过程本身的一步，不是横在用户与结果之间的闸**。

**⇒ 该格判 `不适用`**：U1 预设的对象（「点一下才重算的推演页」）在这一页不存在。
**失效条件**（与上面 9 格同纪律）：若本页将来长出**步内**的推演块（如步②内嵌一个「改目标线立即重算缺口」的
试算区），那一块单独受 U1 管辖，届时把该块按提交闸标准量，本格豁免不自动覆盖它。
...
| **P1** | **~~`global-sim` 的 U8：下钻靠跳页~~ ✅ 已闭（WO-U1-U8-SMALL，2026-08-17）** | ~~`GlobalSimView.tsx` 的 `drillLink` 跳 `/v/project-sim?order=…`~~ 三处下钻口已改 `GlobalSimOrderDrawer` 同屏抽屉：放**该单在本版联合解里的明细**（分配行 + 两阶段排产链 + 被挤/预扣说明），与台账/排产表同源勾稽；跳页出口收进抽屉（`gs-drawer-goto-project` = 「做别的事」，判据明示不算违反） | ~~一张前端单~~ 已交付 |
...
| **4** | **U8** 看明细不换页 | 3 | **2 + 1**（两种病，修法不同） | ①「明细已取回却没渲染 / 只留一句『+N 更多』」⇒ 内联 `<details>` 就地展开（2 页）；②「下钻靠 `<Link>` 跳页」⇒ 要把目标页那段搬成抽屉，改动量大得多（1 页） | ✅ **3 页全闭**：①的 2 页内联展开 · `global-sim` 由 WO-U1-U8-SMALL 上明细抽屉（2026-08-17，裁决：放该单明细不放全量细排） |
...
| **7** | **U1** 改输入即重演 | 3 | **1 + 2** | `what-if` 是纯前端撤闸（廉价）；`optimize-whatif` 走**真 CP-SAT**、`sop-balance` 的五个闸串的是 **S&OP 业务流程**（评审→平衡→定稿），两者撤闸前都得先量重解成本/先裁决语义 | ✅ **3 页全闭（2026-08-17 · WO-U1-U8-SMALL）**：`what-if` 撤闸 · `optimize-whatif` 实测 p50/p95 后撤闸 · `sop-balance` 裁为**不适用**（流程节点非提交闸，§4.3 登记） |
...
| **U1 × `optimize-whatif`** | ~~差**一次实测**~~ **已测已撤（2026-08-17 · WO-U1-U8-SMALL）**：真 CP-SAT 实测运行点 p95≈9.2ms（示例 2×2）/ ≈1.6s（8×30 演示规模），55s 尾部只在手造 2000 对基线出现（非运行点）⇒ 撤闸 + 800ms 防抖 + key 竞态护栏 | ✅ 闭（WO-U1-U8-SMALL；WO-U1-OPT-COST 不必再派） |
| **U1 × `sop-balance`** | ~~差一次**语义裁决**~~ **已裁（2026-08-17 · WO-U1-U8-SMALL）**：五个 `sop-run-N` 是 S&OP 五步法的**流程节点提交**（写版本记录 + 状态机推进 + `planLocked`），非提交闸 ⇒ 改判**不适用**，§4.3 逐格登记 + 专段给三判据区分表与失效条件 | ✅ 闭（WO-U1-U8-SMALL；WO-U1-SOP-VERDICT 不必再派） |
| **U8 × `global-sim`** | ~~差的是**把目标页那段搬成抽屉**~~ **已定已实现（2026-08-17 · WO-U1-U8-SMALL）**：裁决「抽屉放**该单在本版联合解里的明细**，不放全量细排」（全量细排 = 做别的事，出口收进抽屉）；三处 `drillLink` 跳页改 `GlobalSimOrderDrawer` 受控抽屉，数据与台账/排产表同源勾稽 | ✅ 闭（WO-U1-U8-SMALL；WO-U8-GLOBALSIM-DRAWER 不必再派） |
