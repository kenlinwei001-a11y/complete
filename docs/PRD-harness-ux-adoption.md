...
**合计（12 页 × 10 判据 = 120 格）：符合 17 · 不符合 46 · 判不了 57。**

> ⚠ **这三个数是门算出来的，不是我数出来的 —— 我手数的第一版是 21/45/54，三个全错。**
> `node scripts/check-sim-ux-criteria.mjs` 第一次跑就打印「符合 17」，与我写在这一行的 21 对不上，
> 逐格复算后确认门是对的。**这正是本单建这道门的理由**（铁律 0.6：机制的判据 = 下次同样的错发生时
> **机器先说话，不是人想起来**）。逐判据分布同为门算：
> `U1 判7/符2/不3 · U2 不9/符1/判2 · U3 判6/不6 · U4 符9/不3 · U4b 不11/判1 · U5 判12 ·
> U6 符5/判3/不4 · U7 判12 · U8 判12 · U9 不10/判2`。
...
### 4.2 「判不了」的统一理由（57 格，按判据归类 · 格数由门现算）
...
| U4b（1 格）/ U5（12 格）/ U7（12 格）/ U8（12 格） | U5 需跨屏比对同一事实两处的值；U7 需运行期页面上下文；U8 需渲染后判「原地展开」还是「导航走」；U4b 需渲染后的图形 | 门 B（Playwright）级别的真浏览器验证 |
| U6（3 格）/ U9（2 格） | 探针命中的可能是注释/字段名/引入语句 | 逐行读该文件的渲染路径 |

⚠ **这 57 格不许被读作「大概也行」。** 它们是**今天这道门看不见的部分**，
