...
⚠ **2026-08-18 三次订正（WO-GATE-B-BROWSER-HARNESS）**：上一版这句「门 B（真浏览器 / Playwright）至今**未建**，
所以这四条今天确实没有验收方式」**再次部分过期** —— 门 B（账守卫，2026-08-16 建）之后，
**行为面门 `harness-ux-behavior:check`（真浏览器，2026-08-18 建，已接 `pnpm gates`）把 B-1 与 B-4·U8 的
内容面机检落地了**（逐条见 §4.2.2）。仍无验收方式的只剩 **B-3**（缺事实注册表前置）与 **B-4·U7**
（缺编排侧评测集）两条，如实写在表内。

| # | 拆出去的那半（原判据的另一面） | 为什么静态与单页渲染都够不着 | 要判它得有什么 |
|---|---|---|---|
| **B-1** | **U1 的时延面**：改完输入到结果更新**要多久**、中间有没有一段仍在显示旧值的窗口 | 「多久」是运行期量，源码里不存在这个数；页面可能既自动重算又留着按钮（两者并存不矛盾） | 真浏览器：改一个输入、**不点任何按钮**，断言结果 DOM 在 N 毫秒内变了 ⇒ **2026-08-18 已建**（门 `harness-ux-behavior:check`，见 §4.2.2） |
...
| **B-4** | **U7 的内容面** ＋ **U8 的几何面** | U7 内容面要真跑一次编排 + 真模型才知道答得对不对（本表只判「问答知不知道自己在哪一页」）；U8 几何面要渲染后量浮层贴不贴边、抽屉挡不挡 | U7 归编排侧评测；U8 归门 B ⇒ **2026-08-18 U8 已建**（同一门，遮挡欠账对账登记见 §4.2.3）；U7 仍待编排侧评测 |
...
|---|---|---|---|
| **B-1** | ✅ **能（2026-08-18 起 · 门 `harness-ux-behavior:check`）** | 「改完到结果更新要多久」是**运行期量**，源码里不存在这个数。它的失败态（存在一段仍显示旧值的窗口）也只在渲染后才有 —— 所以本门（门 B · 静态文档面）永远不判它；**真浏览器兄弟门已接棒** | ⚠ **2026-08-17 订正：原文「差一个能渲染真页面的 harness」已过期，照它派单会重造一个已有的东西。** harness **已建**：`scripts/lib/layout-probe.mjs`（真 Chromium 起 dev server + 登录 + SPA 内导航 + 等版面稳定 + `page.evaluate` 逐元素量），由 `layout-legibility:check` 在用，12 页全部真渲染量到。**但它今天只量几何、不量「值变没变」** ⇒ **真实缺口已从「造 harness」缩到「在它上面写一条行为用例」**：改一个输入、不点任何按钮、断言结果 DOM 变了（需要的是「同一页面两个时刻的 DOM 快照比对」这个能力，probe 现在只做单时刻几何）。→ §5 `WO-GATE-B-BROWSER-HARNESS`（**范围随之缩小**，不再含「造渲染 harness」这一项）。✅ **2026-08-18 该单已交付**：probe 长出两时刻 DOM 快照比对（签名 = 可见文本 + SVG path 哈希 + 计数），门对 PRD §4 表 U1 列**逐格对账**（12 页），详见 §4.2.2 |
...
| **B-4** | **U7 不能 · ✅ U8 能（2026-08-18 起 · 同一门）**（两面各缺一样，**不许合成一张单**；2026-08-17 两面进度已分叉，2026-08-18 分叉再进一格，更不许合） | U7 内容面要**真跑一次编排 + 真模型**才知道答得对不对（本表只判「问答知不知道自己在哪一页」）；U8 几何面要**渲染后量**浮层贴不贴边、抽屉挡不挡 | U7：一份编排侧评测集（问题 + 期望要素 + 判分口径）→ §5 `WO-QOS-PAGECTX-EVAL`（**未动**）。U8：⚠ **2026-08-17 订正，原文「差渲染后几何量测」已过期** —— 几何量测**已建并已接 `pnpm gates`**（`layout-legibility:check`：字号 / 左边缘对齐 / 视口利用率 / 横向溢出 / **溢出元素分「总数」与「真够不着」两个数**），U8 那句「看明细不换页」里**「不换页」这一半今天量得了**（同一路由内 DOM 几何变化）。**仍缺的是「浮层/抽屉的遮挡关系」**：probe 量的是元素自身的矩形，**没有**「A 盖住了 B 的哪一部分」这个量（要另加 z-order × 矩形相交判定）。→ §5 `WO-GATE-B-BROWSER-HARNESS`，**范围缩小为「在既有 probe 上加遮挡判定」**。✅ **2026-08-18 该单已交付**：probe 长出 z-order × 矩形相交判定（`elementsFromPoint` 真实绘制序 3×3 采样 + 覆盖百分比），门逐页扫浮层触发器并对 §4.2.3 遮挡欠账登记**对账**；首跑即在 `what-if` 量出 3 笔真遮挡（已登记 §4.2.3），详见 §4.2.2 |
...
> ⚠ **2026-08-18 补注（WO-GATE-B-BROWSER-HARNESS）**：上面那句「本门一律不机检」**对本门仍然属实**
> （`CONTENT_CHECKED` 一格没动，判据⑦ 继续按 `1/4` 核）—— 但 **B-1 与 B-4·U8 的内容面自今天起由
> 兄弟门 `harness-ux-behavior:check` 机检**（真浏览器，逐页对账，见 §4.2.2），不再处于「谁都没在验」的状态。
> 仍谁都没在验的只剩 **B-3**（缺事实注册表前置）与 **B-4·U7**（缺编排侧评测集）两条。

#### 4.2.2 行为面门已建（2026-08-18 · WO-GATE-B-BROWSER-HARNESS）—— B-1 与 B-4·U8 的内容面机检落地

`scripts/check-harness-ux-behavior.mjs`（别名 `harness-ux-behavior:check`，已接 `pnpm gates`）。
**没有新造 harness**：两个能力都长在既有真浏览器 probe `scripts/lib/layout-probe.mjs` 上
（页面装配与 `layout-legibility:check` 共用同一份 `openStablePage`），页名册现读自 §4 表（不手抄）。

**B-1 = 同一页面两个时刻的 DOM 快照比对**：改一个输入、**不点任何按钮**，断言结果 DOM 在 5 秒内变了
（签名 = 可见元素文本 + SVG `<path>` d 属性哈希 + 计数；候选输入逐个试、试间还原，
「改不动」与「改了没变」分开记账）。判法 = 与 §4 表 U1 列**逐格对账**：表说符合而屏上不变 ⇒ 红；
表说不符合而屏上也不变 ⇒ 绿（诚实欠账不染红）；表说不符合而屏上已变 ⇒ 红（修好没回写）。
**B-4·U8 = 「A 盖住了 B 的哪一部分」**：逐页找浮层触发器（`aria-expanded`/`aria-haspopup`），
悬停/点击开出浮层后，`elementsFromPoint` 真实绘制序 3×3 采样判「浮层有没有被别的东西压住」
+ 矩形相交量「浮层盖住了哪些文本元素、各百分之几」。判法 = 与 §4.2.3 遮挡欠账登记**对账**。

金丝雀六向（与主逻辑共用同一份实现）：改输入即变必探到 / 提交闸必咬出且对「符合」格判红 /
被压浮层必报被压点并判红 / 置顶浮层零误报且覆盖量必量到 / 对账三向（登记遮挡放行 ·
未登记遮挡判红 · 修好没回写判红）/ §4.2.3 登记表解析自证。
**独立口径**：12 页一页都判不了 / 一个触发器都找不到 / 一个浮层都开不出来 ⇒ RC=2（探针坏了，不是页面干净）。

**首跑实测（2026-08-18，mock 模式 12 页）**：B-1 判 12 页与 U1 列逐格一致（`optimize-whatif` /
`sop-balance` 的「不符合」欠账照实放行，其余 10 页符合属实）；U8 扫出触发器 41 个、开出浮层 40 个，
其中 **`what-if` 3 个浮层被真压住**（截图实证：浮层底行被下方 `div.panel` 裁断）——
按对账语义登记在 §4.2.3，属前端 z-index/堆叠上下文修复单（不在门单范围内，页面文件一行未动）。

#### 4.2.3 U8 遮挡欠账登记（门 `harness-ux-behavior:check` 现算 ⇄ 本表对账 · 键 = 页 + 触发器 aria 标签全等，同页同名触发器带 `#N` 文档序序号）

| 页 | 触发器（aria 标签） | 判定 | 去向 |
|---|---|---|---|
| `what-if` | 这一页怎么用 —— 说明（悬停或聚焦查看） | 不符合 | 浮层底行被下方「① 设定假设」`div.panel` 裁断（3/9 采样点被压，2026-08-18 截图实证）；修 z-index/堆叠上下文，属前端 InfoPopover 堆叠单 |
| `what-if` | 导出内容 —— 说明（悬停或聚焦查看） | 不符合 | 同上根因（6/9 采样点被 `div.panel`/`label` 压），同一单修 |
| `what-if` | 传导边开关·口径说明 | 不符合 | 浮层底部被传导边 checkbox `input` 压住（3/6 采样点），同一单修 |
| `global-sim` | 传导边开关·口径说明 | 不符合 | 与 what-if 同款：浮层底部被传导边 checkbox `input` 压住（3/9 采样点），同一单修（共享 EdgeActivePanel 的说明浮层，一处修多处愈） |
| `order-chain` | 客户维 —— 说明（悬停或聚焦查看） | 不符合 | 浮层被「经营数据看板」`div.panel` 及表格 `td` 压住（6/9 采样点），同一单修 |
| `decision-play` | 这条方案的依据有多硬 —— 说明（悬停或聚焦查看） | 不符合 | 第 1 处（「缩短备份供应商认证周期」卡）浮层被相邻方案卡 `div.panel` 压住（3/9 采样点），同一单修 |
| `decision-play` | 这条方案的依据有多硬 —— 说明（悬停或聚焦查看）#2 | 不符合 | 第 2 处（「长协加价格联动条款」卡）同款被压（3/9 采样点）；第 3 处无遮挡不登记 |
| `decision-play` | 这一档是从哪来的 —— 说明（悬停或聚焦查看）#2 | 不符合 | 第 2 处浮层被「比对矩阵」`div.panel` 压住（3/9 采样点）；第 1 处无遮挡不登记 |
| `optimize-whatif` | 这份模板清单是从哪来的 —— 说明（悬停或聚焦查看） | 不符合 | 浮层被「按需求找模板」`div.panel`/`div.section-title` 压住（6/9 采样点），同一单修 |

对账语义（与 B-1 对 U1 列对账同一个道理，**不许改表买绿，也不许登记烂账**）：
屏上被压 + 表里有 ⇒ 绿（欠账属实，门照实印出）；屏上被压 + 表里**没有** ⇒ 红（新遮挡，修页面或显式登记）；
屏上无压 / 触发器没了 + 表里登记着 ⇒ 红（**修好没回写 / 登记指向空气**，逼销账）。
触发器文案改了 ⇒ 键失配 ⇒ 门当场红 —— **登记不许无声腐烂**，改名必须连本表一起改。

⚠ **这 9 笔是同一个根因族**（2026-08-18 首跑实测）：`InfoPopover` 的浮层 `z-index:40; position:absolute`
挂在触发器所在的 `.panel` 内，文档序靠后的同级 `.panel`（及其表单控件）新建堆叠后把它裁断/压住。
修法是前端组件级的（浮层改挂顶层 / 提堆叠上下文），一处修多半全部愈 —— 属前端 InfoPopover 堆叠单，
**不在门单范围内**（本单单据只碰 probe + 门 + 文档，`apps/**` 一行未动）。
