...
⚠ **我这把尺子一开始只有正向金丝雀，收尾时补了负向的**（自评：这是本单方法上最该记的一笔）。
八个端口**全报 `ok:true`** —— 而"恒真的坏尺子"给出的结果**长得一模一样**。
补测（`.simprobe/portcanary.mjs`，此刻 5601 正被我自己的 datacore 占着）：

```
5601（我的 datacore 正占着） = {"ok":false,"code":"EADDRINUSE"}   ← 占用能被认出来 ✅
5999（应当空闲）             = {"ok":true}                        ← 空闲也能被认出来 ✅
```

⇒ 双向都有鉴别力，原结论成立。
**不过真正兜底的不是这把尺子，是下面那三行 `lsof`/`/proc` 自证** —— 那是"我连的确实是我自己那一个"的直接证据，
不依赖任何"端口当时空不空"的推断。
...
5. **用的是主 checkout 的 `dist`（构建于 01:58Z），不是我这棵树现编的。**
   ✅ **本轮已复核，这一条可以划掉**（原写「未复核」，收尾时补做了）：

   | 量 | 我这棵树的源 `app.ts` | 主 checkout 的 `dist/app.js` |
   |---|---|---|
   | `requireSim(` 总数 | **41** | **41** |
   | `sim.sandbox` / `propagation` / `checkpoint` / `branch` / `certification` / `agent-proposals` | 21 / 12 / 3 / 2 / 1 / 2 | **21 / 12 / 3 / 2 / 1 / 2** |
   | 9 条 `sim.*` 注册行（含 `requires` 数组） | — | **逐字节相同** |

   ⇒ 被我实测的那个二进制，其 **sim 门拓扑与本报告分析的源码一致**，端点实测结论不受 dist 年龄影响。
   ⚠ 仍未覆盖：`app.ts` 中**与 sim 门无关**的其他改动（不影响本单结论）。
