...
- 复验两把：① 本次 3,580 格变化里**从占位初值变走的 = 0 格**（占位格全是只读源侧/无入边终态）；
  ② 屏上数字出自前端 `buildMoneyView(deltas, orders)` —— 世界差分 × 真订单簿（500 张 ORDER.value
  合计 454.6 亿）自算；与求解器 `carriers`（ORDER_LINE 口径 orderAmount 163.5 亿）是**两个独立真实
  计算**（计数巧合一致 150/17，金额口径不同 156.6≠163.5）。两组数都源自真订单数据，不是占位格外推。
