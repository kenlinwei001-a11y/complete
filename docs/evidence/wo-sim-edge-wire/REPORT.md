...
- 重跑（仓主令「那 8 次测试需要重跑」）：`seam-recheck-run.sh` 编排 4 文件 × 各 2 次，
  ...
  | ② object-constraint-refs.seam | RC=1→**修后 RC=0 · 8/8** | RC=1→**修后 RC=0 · 8/8** | ✅ 根因=测试期望用错排序（localeCompare≠码元序），已修复验（recheck2） |
  ...
  **②③④ 根因（仓主令「找到根源去修复它」第二批）——三份全是同一家族：desat3 系数/权重改革
  ...
  - **② object-constraint-refs ⑤b（:284 实例选择）⇒ 已根因、已修（测试侧）、复验 ×2 全绿。**
    表面是「塌给 id 字典序」病灶复发，实测三层剥离后真相是**测试自己的期望算法与实现不同套**：
    ① 实现的比较器是好的（`service.ts constraintPayload` 的 `cmp`：数字走数值序、字符串走**码元序**，
    `ls < rs`，注释明写「不能只用 num() 比」）；② 测试算期望用 `localeCompare`（拼音序）——
    **CJK 下两种排序不同**（「自贡」U+81EA 拼音序最后，「金华」U+91D1 码元序更大），
    于是测试期望 `zigong-pack`、实现选 `jinhua-calendering`（金华辊压线，活数据自证=码元序最大者，
    三轮字符码位 0x91d1/0x534e/0x8f8a）；③ 误判形态恰好与病灶相同（确定性、不报错、选的"不对"）。
    修为测试侧与实现同套比较（码元序），病灶判据不变钝（塌给 id 时选常州装配线，仍≠码元序最大）。
    复验 recheck2 p1/p2 **8/8 绿 RC=0**（`recheck2-object-constraint-refs-p{1,2}.{txt,rc}`）。
