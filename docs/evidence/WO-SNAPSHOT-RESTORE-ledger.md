...
|---|---|---|---|---|---|
| seed-demo-propagation | 308.6s (rc=0, 9/9) | 285.7s (rc=0, 9/9) | **÷1.08** | 346/200/167 → 362/322/239 | 第二个诚实反例：9 个测试的成本大头是各自 live-fire 真 tick 传导（35 条规则逐条真触发、跨 3 跳扰动），播种一次只占小头 ⇒ 机制边界真实读数，不藏 |
| gap-attribution | 1642.6s (rc=1, 16/17) | 489.0s (rc=0, 17/17) | **÷3.36** | 401/397/301 → 434/419/369 | before 轮 V2·严重度分级 `Test timed out in 180000ms`（812.7s 墙钟，负载抖落签名与 VL2/VL5 同型，非断言失败）；after 轮同测试 23.0s 过。**剔除抖落超额口径**：(1642.6−812.7+23.0)/489.0 = **÷1.74** —— 两口径照实并列 |
| enterprise-state.seam | 161.4s (rc=0, 16/16) | 61.1s (rc=0, 16/16) | **÷2.64** | 357/403/401 → 116/299/360 | after 轮 1min 负载 116 为电池内较闲窗口，÷2.64 是五文件中最接近机制上限的读数 |
| simclock | 81.4s (rc=0, 4/4) | 65.0s (rc=0, 4/4) | **÷1.25** | 42/227/326 → 25/151/280 | after 轮 1min 负载 25（全电池最闲）。**seed=7 冷构建标注**：s7-2717233d.bin 落盘 mtime 19:19 恰在本窗内，T6(after)=20.5s ≈ T6(live)=17.6s ⇒ T6 内含 seed=7 双跑冷构建（安静时刻 ~20s 完成，远在 120s 帽内 —— 冷构建可控性实证 +1，继 VL4 62.8s@负载~400 后第二例）；÷1.25 是含冷构建的保守口径 |

**全套件推算（5 重文件实测汇总定案）**：before 合计 329.8+308.6+1642.6+161.4+81.4 = 2523.8s；
after 合计 327.0+285.7+489.0+61.1+65.0 = 1227.8s ⇒ **÷2.06**。
与阶段① Amdahl 诚实口径 ÷2–3（§4/§9）一致，落在区间下沿 —— 5 个**最重**文件的测试逻辑占比
天然最高、摊薄提速，轻文件播种占比更高 ÷ 应更高，故 ÷2.06 宜读作**保守下界**；
SOP §6 的 ÷5–10 实测不成立，由审核方自己回写，本节不贴金。
（无 live 全量基线可对比：快照接线后不存在「全量 live 模式」的既有测量，补跑一轮
808 次真合成的全量约需数小时且只会重复 §4 的负载污染教训 —— 套件级 verdict 以安静窗
全量绝对墙钟为准，见 §10.8。快照键=2：seed=42 + seed=7，808 调用点中唯一非 42 种子为
simclock T6。）

### 10.6 验收④ 还原隔离性探针 —— **PASS**（2026-09-17 19:21，墙钟 15.4s RC=0）

探针 `zz-probe-isolation.test.ts`（测完即删，删除提交 b9cdaaea9；机制 = 模块级只缓存
Buffer，每次还原重新 `v8.deserialize` 出**全新对象图**再 putMany —— 本探针证机制真生效，
不是信注释）：

1. A、B 两副全新 repos 从同一 buf 还原；金丝雀：还原世界非空（取到
   `obj_annualscenario_AOP-2026-aggressive`），否则探针失效。
2. A 改写该对象 props（probeMarker）→ A 自己可见（写入路径自证）。
3. B 读同 id → **无 probeMarker**（A 的改写不可见）。
4. A 删除该对象 → B 里**仍在**（A 的删除不可见）。
5. 同一 buf 再还原到 C → 对象**复活且无 probeMarker**（还原不共享 A 的对象图）。

输出 `ISOLATION ok: A-mutation invisible in B and C`，1 passed。
证据 `/tmp/wo-snapshot-evidence/zz-probe-isolation.{txt,rc}`。

### 10.7 全量套件首跑裁决 —— **NOT-MEASURED**（负载污染，2026-09-18 10:18 处置）

- 首跑 2026-09-17 ~19:25 起跑（/tmp/wo-snapshot-suite.sh，`--pool=forks --maxWorkers=2`），
  `LOAD_AT_START[full-suite]: 24.02 54.15 183.70` —— 15min 负载 183 已违本 WO 自定的
  「安静时间窗 + maxWorkers=2」（docs/SOP-test-layers.md §4）。
- 起跑时机器另有两个错 cwd run 留下的孤儿 worker（PID 10411/10420，72% CPU × 14.8h，
  协调方 2026-09-18 10:16 按确切 PID 杀掉）；负载随后持续 567–675（4 核）。
- 14.8h 仅推进 39/318 文件标记，日志已现负载型单测试超时（`采纳经营方案` 1872310ms ≈ 31min）。
- **裁决：NOT-MEASURED** —— 无论最终绿红都不作数。2026-09-18 10:18 按确切 PID 杀
  10491 → 10511 → fork workers 12170/12297（`ps` 列树后逐个 kill，未用 pkill）。
