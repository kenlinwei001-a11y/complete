...
**核心立场**：这一行按 **EBIT / 服务水平 / 现金周期** 三个 Δ 给高管排方案[^1][^4]；本仓三块决策屏零金额、零日期，**排不出敞口** —— 缺的不是功能，是计价单位。
...
**金丝雀**：同一抽取正则在「接单组合优选」命中 钱 16（元 / 毛利 / 营收）· 历 70（`2026-06-24`…），在统一推演控制台 / 决策推演 / 月度规划命中 **0 / 0** ⇒ 工具是好的，是这三屏没有。另两处缺口：高管决策会那一档「演习结论」**页签置灰**（title 原文「版面还没接」）；月度规划已写「产品→需求→供应→财务→高管决策会」五步状态机，版本列表**暂无数据**。

**只有我会说的一条**：**我们做了、按敞口排却根本排不上号的，是那 42 条无量纲压力传导边。** 它占掉统一推演控制台与月度规划的整个下半屏，而且真算得动（对照实验：幅度→80，`shortageRisk` Δ+336→+560、`supplyRisk` Δ+280→+350，单调可预言）。但它的产出是 `supplyRisk 2897.89 Δ+210` —— 没有一个 COO 的场景以这个量计价[^5]。最贵的屏占给了排不上号的装置，排第一的钱一个字都没有。
...
### 证据文件
`docs/assets/loop10-researcher/sim-unified.png`（8 页签 · 3 灰）· `sop-balance-empty.png`（五步状态机 · 暂无数据）· `decision-play.png`（六维比对矩阵 · 代价 248/90/173 无单位）
