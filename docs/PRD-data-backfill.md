...
|---|---|
| 版本 | v1.0（2026-08-04） |
| 上游 | `docs/PRD-sandbox-redesign.md` 的 30 个场景例反查出的数据缺口 · 任务 #93 派生 |
| 解决问题 | 沙盘 30 例逐条反查数据可得性，查出 **1 个既有缺陷 + 4 个真缺字段 + 1 个时序粒度空档**。这些**不属于沙盘**——它们动的是数据地基，爆炸半径覆盖所有读这些对象的页 |
...
本单的清单来自对沙盘 30 个场景例的**逐条反查**。过程中我犯了两次同族错误，**两次都是继续追才发现的**，记录如下以免复现：
...
| # | 项 | 形态 | 证据 | 修法 |
|---|---|---|---|---|
| **D1** | `MaterialBalance.coverage` | **② 接了线没数据** | 声明在 `service.ts:322`(`LEVER_FACTOR_PROPS`) + `:344`(`LEVER_PROP_META`) + `navigation-slice.ts:214`(切片投影)；但种子行只有 `gapTon`/`netDemandTon`（`battery-extended.ts:394`），规则 C06/C16 与全部下钻均用 `gapTon` | **二选一**（§3.1 给判据）：补数据 **或** 删声明 |
...
### 3.1 D1 · `MaterialBalance.coverage`（**先判定，再动手**）

**这一项不许直接补**。它是既有缺陷，要先回答一个问题：**齐套率到底该不该是一等属性？**
...
**我的建议：A**。理由是**可比性** —— `gapTon` 是绝对吨数，正极粉缺 1000 吨和隔膜缺 10 吨没法排序；
`coverage` 是比率，才能回答「哪个物料最缺」。而沙盘的断点判定（`PRD-sandbox-redesign` §5.1）
正需要这个排序。**但这是仓主的决定**，因为选 A 要动种子、撞金值。
...
- **用途**：沙盘例 B6「三业务抢产能保谁」、例 B9「新客户 vs 老客户优先」的判据。
...
- **用途**：例 C1「设备停机」、例 C6「检修错峰」、例 B8「交付高峰撞检修窗」。
...
- **用途**：沙盘例 C9「夜班良率低于白班」—— 日粒度下这个差异被平均掉，**shift 粒度才暴露**。
...
1. **G1** · D1 按仓主裁决落地（补数据或删声明），**两种都要求 `LEVER_FACTOR_PROPS` 与对象实际属性对齐**。
...
修改：
  ...
  apps/datacore/src/solvers/service.ts              仅 D1 选 B 时：删 LEVER_FACTOR_PROPS/LEVER_PROP_META 的 coverage
  apps/agentcore/src/agent/navigation-slice.ts      仅 D1 选 B 时：删投影里的 coverage
...
| **A5** | **D1 效果层**：若选 A，「物料齐套」杠杆候选**真的出现在杠杆面板**（今天恒不出现）；若选 B，三处声明确实删净且杠杆改用 `gapTon` 后仍能出现 | 亲手驱动杠杆发现，贴前后对比 |
...
| 本单 **D1** | **依赖仓主裁决**（补数据 vs 删声明） |

**建议顺序**：仓主裁 D1 → 本单 D1+D2+D3+D4 一批（都是属性，同一 gate 轮次）→ D6 单独一批（时序改动面更大，金值影响不同）。

---

## 8. 诚实边界

- 本单的缺口清单**只覆盖沙盘 30 个场景例**用到的数据，**不是本体属性的全面体检**。
  其他页面可能有别的缺口，本单不负责。
