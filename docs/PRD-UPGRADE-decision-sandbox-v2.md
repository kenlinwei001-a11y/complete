...
### 0.27 ⛔ 推演的四层分类（仓主定 · **本文重写时曾把它整节丢掉，2026-08-09 补回**）

> **仓主原话：「所以是四层，不是三层加一层……L2 与 L2′ 是并列不是包含 —— 一个按项目横切，
> 一个按产能纵切，都在 L1 之上 L3 之下。记住，按照这个逻辑搭建。」**

| 层 | 名称 | 分析单元 | 覆盖 | 改企业状态 |
|---|---|---|---|---|
| **L1 对象级** | 项目推演 `project-sim` | 单个项目 | 一张单的全链 | ❌ |
| **L2 组合级** | 全局推演 `global-sim` | 订单簿 | 项目层变量 | ❌ |
| **L2′ 专业域级** | 产能推演 `capacity_*` | 基地 / 产线 / 工序 | 产能一域跨基地 | ❌ |
| **L3 企业级** | **决策沙盘** | **企业状态** | 跨域 ＋ 组织 ＋ 时间 | **✅** |

#### 三条必须写死的判据

1. **L2 与 L2′ 是并列，不是包含。**
   `global-sim` 按**项目横切**（一个订单簿里的所有项目，看项目层变量）；
   `capacity_*` 按**产能纵切**（一个产能域跨所有基地，看基地/产线/工序）。
   两者都在 L1 之上、L3 之下，**互不覆盖也互不包含**。
   ⇒ **禁止**把 `capacity_*` 实现成 `global-sim` 的一个子模式，或反之。

2. **只有 L3 改企业状态。** L1/L2/L2′ 三层**一律只读**。
   这条与 R4（模拟态不写真值）同源：`/act` 与 `/perturbations` 只改 `TickState`，
   **采纳才出 `ActionDraft`**。三个下层若出现任何直写真值的路径 = 越层，退单。

3. **「跨域」是 L3 的定义性特征，不是程度差别。**
   L2′ 是「产能**一**域跨基地」，L3 是「**跨**域」。
   ⇒ 判断一个能力该放哪层，问的不是「它覆盖多广」，而是「**它跨不跨域**」。

#### ⚠️ 与已生效裁决的一处冲突，我按裁决处理并在此显式记账

仓主给的 L3 覆盖原文是「跨域 ＋ 组织 ＋ **批复** ＋ 时间」，
但 §0.2① 已裁「**流程审批不体现**」。上表**去掉了「批复」**并保留其余三项。
这不是我擅自删需求 —— 是两条仓主指令相遇时按**后一条已明确的裁决**执行，
并把冲突摆在这里，若裁决有变请以本节为改动入口。

#### 与 §3.0 八层目标架构的关系（**两个不同的轴，不许混**）

- **本节四层 = 推演的「作用域」轴**：这次推演在看多大范围、改不改真值。
- **§3.0 八层 = 系统的「能力栈」轴**：世界 / 扰动 / 传导 / 本体 / 流程 / 规则约束 / 求解决策 / 技能。

L1–L3 **每一层都要用到八层能力栈里的若干层**；八层里的每一层也**服务多个推演层**。
例：扰动层（L2 能力）既服务 L3 的企业级沙盘，也服务 L1 的单项目 what-if。
⇒ **不许**把「八层」当成「四层」的细化，也不许反过来。
...
| **REQ153** | 16 层切片规格（平台覆盖 12/16：Function 签名 0 · Interface 8 · 时间语义弱） | **挂到 U5** + 欠账 #69（本体七要素缺口）。~~⚠️ 这 4 层缺口与欠账 #69 是同一件事，不许当两件做~~ 🔧 **2026-08-10 订正（欠账 #167 · `docs/RECONCILE-slice-16-layers-two-sets.md` §5）**：这句话是**过度声称** —— 实际只重合 **2/4**（Interface + Function）；时间语义/数据绑定 **不在** #69 里，而 #69 的 Security列级/Action回写声明 **不在** REQ153 里。形态：「我用『两边都提到 Interface 和 Function』当作『这两套缺口是同一套』的证据」。**真正同源的只有一处、且在根因层**：`G-UPSERTTYPE-DROPS-FIELDS` 同时是 #69「Interface 残片落不了库」与契约十六层 ⑦状态(a)/⑮行动(a) 恒空的根因，**一处修两边好**。另：本行的「12/16」是 REQ153 那套的口径，**与契约 `SLICE_LAYER_IDS` 的 12/16 无关**（两者弱层集合交集为空），引用前必读对账文 §2.3 |
