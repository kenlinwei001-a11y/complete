# 本体切片连通性缺口报告（ONTOLOGY-SLICE-GAPS）

> 门禁 `slice-connectivity:check` 产物（`scripts/check-slice-connectivity.mjs`）。请勿手改——重跑门禁即刷新。
> 节点=切片，边=两切片可 join（共享 spannedType 或 桥接 link）；孤岛=度 0 切片（§8 G-BUILD-LINK 检测面）。

## 连通图摘要

- 本体：类型 **96**，链路 **107**
- 切片库：**56**（域内 7 · 跨域 49）
- 连通边：**732**（shared-type 212 · bridge-link 520）
- 孤岛：**0**（豁免 1 · 非豁免 0）

## 孤岛表

_无孤岛：所有切片至少与一个其他切片可 join。_

## bridge-link join 字段清单

> 每条经 link 桥接的切片对（非共享类型直连），记 linkKey + join 字段 from→to type。这是「跨切片关联字段真实存在本体图」的正向闭包证据（R12）。

| 切片 A | 切片 B | join（linkKey: from→to） |
|---|---|---|
| `biz.commercial.arinvoice` | `biz.x.order_to_orderpromise` | order_of_customer: Order→Customer |
| `biz.commercial.arinvoice` | `biz.x.order_to_plantarget` | order_of_customer: Order→Customer |
| `biz.commercial.arinvoice` | `biz.x.orderpromise_to_order` | order_of_customer: Order→Customer |
| `biz.commercial.arinvoice` | `biz.x.workorder_to_order` | order_of_customer: Order→Customer |
| `biz.equip.equipment` | `biz.x.line_to_process` | equip_used_in: Equipment→Process |
| `biz.equip.equipment` | `biz.x.process_to_line` | equip_used_in: Equipment→Process |
| `biz.factory.base` | `biz.people.operatorattendance` | att_for_line: OperatorAttendance→Line |
| `biz.factory.base` | `biz.x.defectrecord_to_wiplot` | wip_on_line: WIPLot→Line |
| `biz.factory.base` | `biz.x.equipment_to_process` | line_has_process: Line→Process |
| `biz.factory.base` | `biz.x.finishedgoodsinventory_to_model` | fg_at_warehouse: FinishedGoodsInventory→Warehouse |
| `biz.factory.base` | `biz.x.interbasetransfer_to_model` | base_dispatches_transfer: Base→InterBaseTransfer |
| `biz.factory.base` | `biz.x.inventorytxn_to_workorder` | line_runs_work_order: Line→WorkOrder |
| `biz.factory.base` | `biz.x.material_to_model` | model_certified_on: Model→Line |
| `biz.factory.base` | `biz.x.model_to_certification` | model_certified_on: Model→Line |
| `biz.factory.base` | `biz.x.model_to_changeovermatrix` | model_certified_on: Model→Line |
| `biz.factory.base` | `biz.x.model_to_finishedgoodsinventory` | fg_at_warehouse: FinishedGoodsInventory→Warehouse |
| `biz.factory.base` | `biz.x.model_to_material` | model_certified_on: Model→Line |
| `biz.factory.base` | `biz.x.process_to_equipment` | line_has_process: Line→Process |
| `biz.factory.base` | `biz.x.qualitylot_to_workorder` | line_runs_work_order: Line→WorkOrder |
| `biz.factory.base` | `biz.x.qualitystandard_to_model` | model_certified_on: Model→Line |
| `biz.factory.base` | `biz.x.routing_to_model` | model_certified_on: Model→Line |
| `biz.factory.base` | `biz.x.wiplot_to_defectrecord` | wip_on_line: WIPLot→Line |
| `biz.factory.base` | `biz.x.wipqualitycheckpoint_to_wiplot` | wip_on_line: WIPLot→Line |
| `biz.factory.base` | `biz.x.workorder_to_capacitypool` | has_capacity: Line→CapacityPool |
| `biz.factory.base` | `biz.x.workorder_to_model` | line_runs_work_order: Line→WorkOrder |
| `biz.factory.base` | `biz.x.workorder_to_order` | line_runs_work_order: Line→WorkOrder |
| `biz.factory.base` | `biz.x.workorder_to_qualitylot` | line_runs_work_order: Line→WorkOrder |
| `biz.people.operatorattendance` | `biz.x.line_to_capacitypool` | att_for_line: OperatorAttendance→Line |
| `biz.people.operatorattendance` | `biz.x.line_to_process` | att_for_line: OperatorAttendance→Line |
| `biz.people.operatorattendance` | `biz.x.line_to_workorder` | att_for_line: OperatorAttendance→Line |
| `biz.people.operatorattendance` | `biz.x.model_to_line` | att_for_line: OperatorAttendance→Line |
| `biz.people.operatorattendance` | `biz.x.process_to_line` | att_for_line: OperatorAttendance→Line |
| `biz.people.operatorattendance` | `biz.x.shiftplan_to_line` | att_for_line: OperatorAttendance→Line |
| `biz.people.operatorattendance` | `biz.x.wiplot_to_line` | att_for_line: OperatorAttendance→Line |
| `biz.people.operatorattendance` | `biz.x.workorder_to_line` | att_for_line: OperatorAttendance→Line |
| `biz.plan.annualscenario` | `biz.x.metric_to_principal` | plantarget_ownedby: PlanTarget→Principal |
| `biz.plan.annualscenario` | `biz.x.order_to_customer` | order_to_plantarget: Order→PlanTarget |
| `biz.plan.annualscenario` | `biz.x.order_to_orderpromise` | order_to_plantarget: Order→PlanTarget |
| `biz.plan.annualscenario` | `biz.x.orderpromise_to_order` | order_to_plantarget: Order→PlanTarget |
| `biz.plan.annualscenario` | `biz.x.workorder_to_order` | order_to_plantarget: Order→PlanTarget |
| `biz.process.operation` | `biz.x.finishedgoodsinventory_to_model` | routing_belongs_to_model: Routing→Model |
| `biz.process.operation` | `biz.x.interbasetransfer_to_model` | routing_belongs_to_model: Routing→Model |
| `biz.process.operation` | `biz.x.material_to_model` | routing_belongs_to_model: Routing→Model |
| `biz.process.operation` | `biz.x.model_to_base` | routing_belongs_to_model: Routing→Model |
| `biz.process.operation` | `biz.x.model_to_certification` | routing_belongs_to_model: Routing→Model |
| `biz.process.operation` | `biz.x.model_to_changeovermatrix` | routing_belongs_to_model: Routing→Model |
| `biz.process.operation` | `biz.x.model_to_finishedgoodsinventory` | routing_belongs_to_model: Routing→Model |
| `biz.process.operation` | `biz.x.model_to_line` | routing_belongs_to_model: Routing→Model |
| `biz.process.operation` | `biz.x.model_to_material` | routing_belongs_to_model: Routing→Model |
| `biz.process.operation` | `biz.x.qualitystandard_to_model` | routing_belongs_to_model: Routing→Model |
| `biz.process.operation` | `biz.x.workorder_to_model` | routing_belongs_to_model: Routing→Model |
| `biz.product.bomdetail` | `biz.x.material_to_materialbalance` | detail_uses_material: BOMDetail→Material |
| `biz.product.bomdetail` | `biz.x.material_to_model` | detail_uses_material: BOMDetail→Material |
| `biz.product.bomdetail` | `biz.x.model_to_material` | detail_uses_material: BOMDetail→Material |
| `biz.x.annualscenario_to_financemetric` | `biz.x.order_to_plantarget` | scenario_to_target: AnnualScenario→PlanTarget |
| `biz.x.annualscenario_to_financemetric` | `biz.x.plantarget_to_principal` | scenario_to_target: AnnualScenario→PlanTarget |
| `biz.x.base_to_datasourcehealth` | `biz.x.finishedgoodsinventory_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_datasourcehealth` | `biz.x.finishedgoodsinventory_to_warehouse` | warehouse_of_base: Base→Warehouse |
| `biz.x.base_to_datasourcehealth` | `biz.x.interbasetransfer_to_model` | base_dispatches_transfer: Base→InterBaseTransfer |
| `biz.x.base_to_datasourcehealth` | `biz.x.line_to_capacitypool` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_datasourcehealth` | `biz.x.line_to_process` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_datasourcehealth` | `biz.x.line_to_workorder` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_datasourcehealth` | `biz.x.material_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_datasourcehealth` | `biz.x.model_to_certification` | model_producible_at: Model→Base |
| `biz.x.base_to_datasourcehealth` | `biz.x.model_to_changeovermatrix` | model_producible_at: Model→Base |
| `biz.x.base_to_datasourcehealth` | `biz.x.model_to_finishedgoodsinventory` | model_producible_at: Model→Base |
| `biz.x.base_to_datasourcehealth` | `biz.x.model_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_datasourcehealth` | `biz.x.model_to_material` | model_producible_at: Model→Base |
| `biz.x.base_to_datasourcehealth` | `biz.x.operatorattendance_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_datasourcehealth` | `biz.x.process_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_datasourcehealth` | `biz.x.qualitystandard_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_datasourcehealth` | `biz.x.routing_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_datasourcehealth` | `biz.x.shiftplan_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_datasourcehealth` | `biz.x.wiplot_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_datasourcehealth` | `biz.x.workorder_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_datasourcehealth` | `biz.x.workorder_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_financeaccount` | `biz.x.finishedgoodsinventory_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_financeaccount` | `biz.x.finishedgoodsinventory_to_warehouse` | warehouse_of_base: Base→Warehouse |
| `biz.x.base_to_financeaccount` | `biz.x.interbasetransfer_to_model` | base_dispatches_transfer: Base→InterBaseTransfer |
| `biz.x.base_to_financeaccount` | `biz.x.line_to_capacitypool` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_financeaccount` | `biz.x.line_to_process` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_financeaccount` | `biz.x.line_to_workorder` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_financeaccount` | `biz.x.material_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_financeaccount` | `biz.x.model_to_certification` | model_producible_at: Model→Base |
| `biz.x.base_to_financeaccount` | `biz.x.model_to_changeovermatrix` | model_producible_at: Model→Base |
| `biz.x.base_to_financeaccount` | `biz.x.model_to_finishedgoodsinventory` | model_producible_at: Model→Base |
| `biz.x.base_to_financeaccount` | `biz.x.model_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_financeaccount` | `biz.x.model_to_material` | model_producible_at: Model→Base |
| `biz.x.base_to_financeaccount` | `biz.x.operatorattendance_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_financeaccount` | `biz.x.process_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_financeaccount` | `biz.x.qualitystandard_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_financeaccount` | `biz.x.routing_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_financeaccount` | `biz.x.shiftplan_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_financeaccount` | `biz.x.wiplot_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_financeaccount` | `biz.x.workorder_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_financeaccount` | `biz.x.workorder_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_interbasetransfer` | `biz.x.finishedgoodsinventory_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_interbasetransfer` | `biz.x.finishedgoodsinventory_to_warehouse` | warehouse_of_base: Base→Warehouse |
| `biz.x.base_to_interbasetransfer` | `biz.x.line_to_capacitypool` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_interbasetransfer` | `biz.x.line_to_process` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_interbasetransfer` | `biz.x.line_to_workorder` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_interbasetransfer` | `biz.x.material_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_interbasetransfer` | `biz.x.model_to_certification` | model_producible_at: Model→Base |
| `biz.x.base_to_interbasetransfer` | `biz.x.model_to_changeovermatrix` | model_producible_at: Model→Base |
| `biz.x.base_to_interbasetransfer` | `biz.x.model_to_finishedgoodsinventory` | model_producible_at: Model→Base |
| `biz.x.base_to_interbasetransfer` | `biz.x.model_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_interbasetransfer` | `biz.x.model_to_material` | model_producible_at: Model→Base |
| `biz.x.base_to_interbasetransfer` | `biz.x.operatorattendance_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_interbasetransfer` | `biz.x.process_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_interbasetransfer` | `biz.x.qualitystandard_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_interbasetransfer` | `biz.x.routing_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_interbasetransfer` | `biz.x.shiftplan_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_interbasetransfer` | `biz.x.wiplot_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_interbasetransfer` | `biz.x.workorder_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_interbasetransfer` | `biz.x.workorder_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_maintplan` | `biz.x.finishedgoodsinventory_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_maintplan` | `biz.x.finishedgoodsinventory_to_warehouse` | warehouse_of_base: Base→Warehouse |
| `biz.x.base_to_maintplan` | `biz.x.interbasetransfer_to_model` | base_dispatches_transfer: Base→InterBaseTransfer |
| `biz.x.base_to_maintplan` | `biz.x.line_to_capacitypool` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_maintplan` | `biz.x.line_to_process` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_maintplan` | `biz.x.line_to_workorder` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_maintplan` | `biz.x.material_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_maintplan` | `biz.x.model_to_certification` | model_producible_at: Model→Base |
| `biz.x.base_to_maintplan` | `biz.x.model_to_changeovermatrix` | model_producible_at: Model→Base |
| `biz.x.base_to_maintplan` | `biz.x.model_to_finishedgoodsinventory` | model_producible_at: Model→Base |
| `biz.x.base_to_maintplan` | `biz.x.model_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_maintplan` | `biz.x.model_to_material` | model_producible_at: Model→Base |
| `biz.x.base_to_maintplan` | `biz.x.operatorattendance_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_maintplan` | `biz.x.process_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_maintplan` | `biz.x.qualitystandard_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_maintplan` | `biz.x.routing_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_maintplan` | `biz.x.shiftplan_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_maintplan` | `biz.x.wiplot_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_maintplan` | `biz.x.workorder_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_maintplan` | `biz.x.workorder_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_shipment` | `biz.x.finishedgoodsinventory_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_shipment` | `biz.x.finishedgoodsinventory_to_warehouse` | warehouse_of_base: Base→Warehouse |
| `biz.x.base_to_shipment` | `biz.x.interbasetransfer_to_model` | base_dispatches_transfer: Base→InterBaseTransfer |
| `biz.x.base_to_shipment` | `biz.x.line_to_capacitypool` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_shipment` | `biz.x.line_to_process` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_shipment` | `biz.x.line_to_workorder` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_shipment` | `biz.x.material_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_shipment` | `biz.x.model_to_certification` | model_producible_at: Model→Base |
| `biz.x.base_to_shipment` | `biz.x.model_to_changeovermatrix` | model_producible_at: Model→Base |
| `biz.x.base_to_shipment` | `biz.x.model_to_finishedgoodsinventory` | model_producible_at: Model→Base |
| `biz.x.base_to_shipment` | `biz.x.model_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_shipment` | `biz.x.model_to_material` | model_producible_at: Model→Base |
| `biz.x.base_to_shipment` | `biz.x.operatorattendance_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_shipment` | `biz.x.process_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_shipment` | `biz.x.qualitystandard_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_shipment` | `biz.x.routing_to_model` | model_producible_at: Model→Base |
| `biz.x.base_to_shipment` | `biz.x.shiftplan_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_shipment` | `biz.x.wiplot_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_shipment` | `biz.x.workorder_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.base_to_shipment` | `biz.x.workorder_to_model` | model_producible_at: Model→Base |
| `biz.x.bomdetail_to_material` | `biz.x.finishedgoodsinventory_to_model` | material_used_by_model: Material→Model |
| `biz.x.bomdetail_to_material` | `biz.x.interbasetransfer_to_model` | material_used_by_model: Material→Model |
| `biz.x.bomdetail_to_material` | `biz.x.model_to_base` | material_used_by_model: Material→Model |
| `biz.x.bomdetail_to_material` | `biz.x.model_to_certification` | material_used_by_model: Material→Model |
| `biz.x.bomdetail_to_material` | `biz.x.model_to_changeovermatrix` | material_used_by_model: Material→Model |
| `biz.x.bomdetail_to_material` | `biz.x.model_to_finishedgoodsinventory` | material_used_by_model: Material→Model |
| `biz.x.bomdetail_to_material` | `biz.x.model_to_line` | material_used_by_model: Material→Model |
| `biz.x.bomdetail_to_material` | `biz.x.purchaseorder_to_incominginspection` | material_supplied_by_po: Material→PurchaseOrder |
| `biz.x.bomdetail_to_material` | `biz.x.qualitystandard_to_model` | material_used_by_model: Material→Model |
| `biz.x.bomdetail_to_material` | `biz.x.routing_to_model` | material_used_by_model: Material→Model |
| `biz.x.bomdetail_to_material` | `biz.x.workorder_to_model` | material_used_by_model: Material→Model |
| `biz.x.customer_to_overduerecord` | `biz.x.order_to_orderpromise` | order_of_customer: Order→Customer |
| `biz.x.customer_to_overduerecord` | `biz.x.order_to_plantarget` | order_of_customer: Order→Customer |
| `biz.x.customer_to_overduerecord` | `biz.x.orderpromise_to_order` | order_of_customer: Order→Customer |
| `biz.x.customer_to_overduerecord` | `biz.x.workorder_to_order` | order_of_customer: Order→Customer |
| `biz.x.defectrecord_to_wiplot` | `biz.x.exceptionevent_to_equipmentdowntime` | defect_raises_exception: DefectRecord→ExceptionEvent |
| `biz.x.defectrecord_to_wiplot` | `biz.x.inventorytxn_to_workorder` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.defectrecord_to_wiplot` | `biz.x.line_to_capacitypool` | wip_on_line: WIPLot→Line |
| `biz.x.defectrecord_to_wiplot` | `biz.x.line_to_process` | wip_on_line: WIPLot→Line |
| `biz.x.defectrecord_to_wiplot` | `biz.x.line_to_workorder` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.defectrecord_to_wiplot` | `biz.x.model_to_line` | wip_on_line: WIPLot→Line |
| `biz.x.defectrecord_to_wiplot` | `biz.x.operatorattendance_to_line` | wip_on_line: WIPLot→Line |
| `biz.x.defectrecord_to_wiplot` | `biz.x.process_to_line` | wip_on_line: WIPLot→Line |
| `biz.x.defectrecord_to_wiplot` | `biz.x.qualitylot_to_workorder` | defect_for_qlot: DefectRecord→QualityLot |
| `biz.x.defectrecord_to_wiplot` | `biz.x.shiftplan_to_line` | wip_on_line: WIPLot→Line |
| `biz.x.defectrecord_to_wiplot` | `biz.x.workorder_to_capacitypool` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.defectrecord_to_wiplot` | `biz.x.workorder_to_line` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.defectrecord_to_wiplot` | `biz.x.workorder_to_model` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.defectrecord_to_wiplot` | `biz.x.workorder_to_order` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.defectrecord_to_wiplot` | `biz.x.workorder_to_qualitylot` | defect_for_qlot: DefectRecord→QualityLot |
| `biz.x.equipment_to_process` | `biz.x.exceptionevent_to_equipmentdowntime` | dt_for_equip: EquipmentDowntime→Equipment |
| `biz.x.equipment_to_process` | `biz.x.line_to_capacitypool` | line_has_process: Line→Process |
| `biz.x.equipment_to_process` | `biz.x.line_to_workorder` | line_has_process: Line→Process |
| `biz.x.equipment_to_process` | `biz.x.model_to_line` | line_has_process: Line→Process |
| `biz.x.equipment_to_process` | `biz.x.operatorattendance_to_line` | line_has_process: Line→Process |
| `biz.x.equipment_to_process` | `biz.x.shiftplan_to_line` | line_has_process: Line→Process |
| `biz.x.equipment_to_process` | `biz.x.wiplot_to_line` | line_has_process: Line→Process |
| `biz.x.equipment_to_process` | `biz.x.workorder_to_line` | line_has_process: Line→Process |
| `biz.x.exceptionevent_to_equipmentdowntime` | `biz.x.process_to_equipment` | dt_for_equip: EquipmentDowntime→Equipment |
| `biz.x.exceptionevent_to_equipmentdowntime` | `biz.x.wiplot_to_defectrecord` | defect_raises_exception: DefectRecord→ExceptionEvent |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.interbasetransfer_to_base` | model_producible_at: Model→Base |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.inventorytxn_to_workorder` | txn_for_fg: InventoryTxn→FinishedGoodsInventory |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.line_to_capacitypool` | model_certified_on: Model→Line |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.line_to_process` | model_certified_on: Model→Line |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.line_to_workorder` | model_certified_on: Model→Line |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.material_to_materialbalance` | material_used_by_model: Material→Model |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.operatorattendance_to_line` | model_certified_on: Model→Line |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.order_to_customer` | model_demanded_by_order: Model→Order |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.order_to_orderpromise` | model_demanded_by_order: Model→Order |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.order_to_plantarget` | model_demanded_by_order: Model→Order |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.orderpromise_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.process_to_line` | model_certified_on: Model→Line |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.qualitylot_to_workorder` | wo_for_model: WorkOrder→Model |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.shiftplan_to_line` | model_certified_on: Model→Line |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.wiplot_to_line` | model_certified_on: Model→Line |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.workorder_to_capacitypool` | wo_for_model: WorkOrder→Model |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.workorder_to_line` | model_certified_on: Model→Line |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.workorder_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.finishedgoodsinventory_to_model` | `biz.x.workorder_to_qualitylot` | wo_for_model: WorkOrder→Model |
| `biz.x.finishedgoodsinventory_to_warehouse` | `biz.x.interbasetransfer_to_base` | warehouse_of_base: Base→Warehouse |
| `biz.x.finishedgoodsinventory_to_warehouse` | `biz.x.interbasetransfer_to_model` | fg_of_model: FinishedGoodsInventory→Model |
| `biz.x.finishedgoodsinventory_to_warehouse` | `biz.x.inventorytxn_to_workorder` | txn_for_fg: InventoryTxn→FinishedGoodsInventory |
| `biz.x.finishedgoodsinventory_to_warehouse` | `biz.x.material_to_model` | fg_of_model: FinishedGoodsInventory→Model |
| `biz.x.finishedgoodsinventory_to_warehouse` | `biz.x.model_to_base` | fg_of_model: FinishedGoodsInventory→Model |
| `biz.x.finishedgoodsinventory_to_warehouse` | `biz.x.model_to_certification` | fg_of_model: FinishedGoodsInventory→Model |
| `biz.x.finishedgoodsinventory_to_warehouse` | `biz.x.model_to_changeovermatrix` | fg_of_model: FinishedGoodsInventory→Model |
| `biz.x.finishedgoodsinventory_to_warehouse` | `biz.x.model_to_line` | fg_of_model: FinishedGoodsInventory→Model |
| `biz.x.finishedgoodsinventory_to_warehouse` | `biz.x.model_to_material` | fg_of_model: FinishedGoodsInventory→Model |
| `biz.x.finishedgoodsinventory_to_warehouse` | `biz.x.qualitystandard_to_model` | fg_of_model: FinishedGoodsInventory→Model |
| `biz.x.finishedgoodsinventory_to_warehouse` | `biz.x.routing_to_model` | fg_of_model: FinishedGoodsInventory→Model |
| `biz.x.finishedgoodsinventory_to_warehouse` | `biz.x.workorder_to_model` | fg_of_model: FinishedGoodsInventory→Model |
| `biz.x.interbasetransfer_to_base` | `biz.x.line_to_capacitypool` | line_belongs_to_base: Base→Line |
| `biz.x.interbasetransfer_to_base` | `biz.x.line_to_process` | line_belongs_to_base: Base→Line |
| `biz.x.interbasetransfer_to_base` | `biz.x.line_to_workorder` | line_belongs_to_base: Base→Line |
| `biz.x.interbasetransfer_to_base` | `biz.x.material_to_model` | model_producible_at: Model→Base |
| `biz.x.interbasetransfer_to_base` | `biz.x.model_to_certification` | model_producible_at: Model→Base |
| `biz.x.interbasetransfer_to_base` | `biz.x.model_to_changeovermatrix` | model_producible_at: Model→Base |
| `biz.x.interbasetransfer_to_base` | `biz.x.model_to_finishedgoodsinventory` | model_producible_at: Model→Base |
| `biz.x.interbasetransfer_to_base` | `biz.x.model_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.interbasetransfer_to_base` | `biz.x.model_to_material` | model_producible_at: Model→Base |
| `biz.x.interbasetransfer_to_base` | `biz.x.operatorattendance_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.interbasetransfer_to_base` | `biz.x.process_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.interbasetransfer_to_base` | `biz.x.qualitystandard_to_model` | model_producible_at: Model→Base |
| `biz.x.interbasetransfer_to_base` | `biz.x.routing_to_model` | model_producible_at: Model→Base |
| `biz.x.interbasetransfer_to_base` | `biz.x.shiftplan_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.interbasetransfer_to_base` | `biz.x.wiplot_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.interbasetransfer_to_base` | `biz.x.workorder_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.interbasetransfer_to_base` | `biz.x.workorder_to_model` | model_producible_at: Model→Base |
| `biz.x.interbasetransfer_to_model` | `biz.x.inventorytxn_to_workorder` | wo_for_model: WorkOrder→Model |
| `biz.x.interbasetransfer_to_model` | `biz.x.line_to_capacitypool` | model_certified_on: Model→Line |
| `biz.x.interbasetransfer_to_model` | `biz.x.line_to_process` | model_certified_on: Model→Line |
| `biz.x.interbasetransfer_to_model` | `biz.x.line_to_workorder` | model_certified_on: Model→Line |
| `biz.x.interbasetransfer_to_model` | `biz.x.material_to_materialbalance` | material_used_by_model: Material→Model |
| `biz.x.interbasetransfer_to_model` | `biz.x.operatorattendance_to_line` | model_certified_on: Model→Line |
| `biz.x.interbasetransfer_to_model` | `biz.x.order_to_customer` | model_demanded_by_order: Model→Order |
| `biz.x.interbasetransfer_to_model` | `biz.x.order_to_orderpromise` | model_demanded_by_order: Model→Order |
| `biz.x.interbasetransfer_to_model` | `biz.x.order_to_plantarget` | model_demanded_by_order: Model→Order |
| `biz.x.interbasetransfer_to_model` | `biz.x.orderpromise_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.interbasetransfer_to_model` | `biz.x.process_to_line` | model_certified_on: Model→Line |
| `biz.x.interbasetransfer_to_model` | `biz.x.qualitylot_to_workorder` | wo_for_model: WorkOrder→Model |
| `biz.x.interbasetransfer_to_model` | `biz.x.shiftplan_to_line` | model_certified_on: Model→Line |
| `biz.x.interbasetransfer_to_model` | `biz.x.wiplot_to_line` | model_certified_on: Model→Line |
| `biz.x.interbasetransfer_to_model` | `biz.x.workorder_to_capacitypool` | wo_for_model: WorkOrder→Model |
| `biz.x.interbasetransfer_to_model` | `biz.x.workorder_to_line` | model_certified_on: Model→Line |
| `biz.x.interbasetransfer_to_model` | `biz.x.workorder_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.interbasetransfer_to_model` | `biz.x.workorder_to_qualitylot` | wo_for_model: WorkOrder→Model |
| `biz.x.inventorytxn_to_workorder` | `biz.x.line_to_capacitypool` | consumes_capacity: WorkOrder→CapacityPool |
| `biz.x.inventorytxn_to_workorder` | `biz.x.line_to_process` | line_runs_work_order: Line→WorkOrder |
| `biz.x.inventorytxn_to_workorder` | `biz.x.material_to_model` | wo_for_model: WorkOrder→Model |
| `biz.x.inventorytxn_to_workorder` | `biz.x.model_to_base` | wo_for_model: WorkOrder→Model |
| `biz.x.inventorytxn_to_workorder` | `biz.x.model_to_certification` | wo_for_model: WorkOrder→Model |
| `biz.x.inventorytxn_to_workorder` | `biz.x.model_to_changeovermatrix` | wo_for_model: WorkOrder→Model |
| `biz.x.inventorytxn_to_workorder` | `biz.x.model_to_finishedgoodsinventory` | txn_for_fg: InventoryTxn→FinishedGoodsInventory |
| `biz.x.inventorytxn_to_workorder` | `biz.x.model_to_line` | line_runs_work_order: Line→WorkOrder |
| `biz.x.inventorytxn_to_workorder` | `biz.x.model_to_material` | wo_for_model: WorkOrder→Model |
| `biz.x.inventorytxn_to_workorder` | `biz.x.operatorattendance_to_line` | line_runs_work_order: Line→WorkOrder |
| `biz.x.inventorytxn_to_workorder` | `biz.x.order_to_customer` | fulfills: WorkOrder→Order |
| `biz.x.inventorytxn_to_workorder` | `biz.x.order_to_orderpromise` | fulfills: WorkOrder→Order |
| `biz.x.inventorytxn_to_workorder` | `biz.x.order_to_plantarget` | fulfills: WorkOrder→Order |
| `biz.x.inventorytxn_to_workorder` | `biz.x.orderpromise_to_order` | fulfills: WorkOrder→Order |
| `biz.x.inventorytxn_to_workorder` | `biz.x.process_to_line` | line_runs_work_order: Line→WorkOrder |
| `biz.x.inventorytxn_to_workorder` | `biz.x.qualitystandard_to_model` | wo_for_model: WorkOrder→Model |
| `biz.x.inventorytxn_to_workorder` | `biz.x.routing_to_model` | wo_for_model: WorkOrder→Model |
| `biz.x.inventorytxn_to_workorder` | `biz.x.shiftplan_to_line` | line_runs_work_order: Line→WorkOrder |
| `biz.x.inventorytxn_to_workorder` | `biz.x.wiplot_to_defectrecord` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.inventorytxn_to_workorder` | `biz.x.wiplot_to_line` | line_runs_work_order: Line→WorkOrder |
| `biz.x.inventorytxn_to_workorder` | `biz.x.wipqualitycheckpoint_to_wiplot` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.line_to_capacitypool` | `biz.x.material_to_model` | model_certified_on: Model→Line |
| `biz.x.line_to_capacitypool` | `biz.x.model_to_base` | line_belongs_to_base: Base→Line |
| `biz.x.line_to_capacitypool` | `biz.x.model_to_certification` | model_certified_on: Model→Line |
| `biz.x.line_to_capacitypool` | `biz.x.model_to_changeovermatrix` | model_certified_on: Model→Line |
| `biz.x.line_to_capacitypool` | `biz.x.model_to_finishedgoodsinventory` | model_certified_on: Model→Line |
| `biz.x.line_to_capacitypool` | `biz.x.model_to_material` | model_certified_on: Model→Line |
| `biz.x.line_to_capacitypool` | `biz.x.process_to_equipment` | line_has_process: Line→Process |
| `biz.x.line_to_capacitypool` | `biz.x.qualitylot_to_workorder` | consumes_capacity: WorkOrder→CapacityPool |
| `biz.x.line_to_capacitypool` | `biz.x.qualitystandard_to_model` | model_certified_on: Model→Line |
| `biz.x.line_to_capacitypool` | `biz.x.routing_to_model` | model_certified_on: Model→Line |
| `biz.x.line_to_capacitypool` | `biz.x.wiplot_to_defectrecord` | wip_on_line: WIPLot→Line |
| `biz.x.line_to_capacitypool` | `biz.x.wipqualitycheckpoint_to_wiplot` | wip_on_line: WIPLot→Line |
| `biz.x.line_to_capacitypool` | `biz.x.workorder_to_model` | consumes_capacity: WorkOrder→CapacityPool |
| `biz.x.line_to_capacitypool` | `biz.x.workorder_to_order` | consumes_capacity: WorkOrder→CapacityPool |
| `biz.x.line_to_capacitypool` | `biz.x.workorder_to_qualitylot` | consumes_capacity: WorkOrder→CapacityPool |
| `biz.x.line_to_process` | `biz.x.material_to_model` | model_certified_on: Model→Line |
| `biz.x.line_to_process` | `biz.x.model_to_base` | line_belongs_to_base: Base→Line |
| `biz.x.line_to_process` | `biz.x.model_to_certification` | model_certified_on: Model→Line |
| `biz.x.line_to_process` | `biz.x.model_to_changeovermatrix` | model_certified_on: Model→Line |
| `biz.x.line_to_process` | `biz.x.model_to_finishedgoodsinventory` | model_certified_on: Model→Line |
| `biz.x.line_to_process` | `biz.x.model_to_material` | model_certified_on: Model→Line |
| `biz.x.line_to_process` | `biz.x.qualitylot_to_workorder` | line_runs_work_order: Line→WorkOrder |
| `biz.x.line_to_process` | `biz.x.qualitystandard_to_model` | model_certified_on: Model→Line |
| `biz.x.line_to_process` | `biz.x.routing_to_model` | model_certified_on: Model→Line |
| `biz.x.line_to_process` | `biz.x.wiplot_to_defectrecord` | wip_on_line: WIPLot→Line |
| `biz.x.line_to_process` | `biz.x.wipqualitycheckpoint_to_wiplot` | wip_on_line: WIPLot→Line |
| `biz.x.line_to_process` | `biz.x.workorder_to_capacitypool` | has_capacity: Line→CapacityPool |
| `biz.x.line_to_process` | `biz.x.workorder_to_model` | line_runs_work_order: Line→WorkOrder |
| `biz.x.line_to_process` | `biz.x.workorder_to_order` | line_runs_work_order: Line→WorkOrder |
| `biz.x.line_to_process` | `biz.x.workorder_to_qualitylot` | line_runs_work_order: Line→WorkOrder |
| `biz.x.line_to_workorder` | `biz.x.material_to_model` | model_certified_on: Model→Line |
| `biz.x.line_to_workorder` | `biz.x.model_to_base` | line_belongs_to_base: Base→Line |
| `biz.x.line_to_workorder` | `biz.x.model_to_certification` | model_certified_on: Model→Line |
| `biz.x.line_to_workorder` | `biz.x.model_to_changeovermatrix` | model_certified_on: Model→Line |
| `biz.x.line_to_workorder` | `biz.x.model_to_finishedgoodsinventory` | model_certified_on: Model→Line |
| `biz.x.line_to_workorder` | `biz.x.model_to_material` | model_certified_on: Model→Line |
| `biz.x.line_to_workorder` | `biz.x.order_to_customer` | fulfills: WorkOrder→Order |
| `biz.x.line_to_workorder` | `biz.x.order_to_orderpromise` | fulfills: WorkOrder→Order |
| `biz.x.line_to_workorder` | `biz.x.order_to_plantarget` | fulfills: WorkOrder→Order |
| `biz.x.line_to_workorder` | `biz.x.orderpromise_to_order` | fulfills: WorkOrder→Order |
| `biz.x.line_to_workorder` | `biz.x.process_to_equipment` | line_has_process: Line→Process |
| `biz.x.line_to_workorder` | `biz.x.qualitystandard_to_model` | model_certified_on: Model→Line |
| `biz.x.line_to_workorder` | `biz.x.routing_to_model` | model_certified_on: Model→Line |
| `biz.x.line_to_workorder` | `biz.x.wiplot_to_defectrecord` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.line_to_workorder` | `biz.x.wipqualitycheckpoint_to_wiplot` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.material_to_materialbalance` | `biz.x.model_to_base` | material_used_by_model: Material→Model |
| `biz.x.material_to_materialbalance` | `biz.x.model_to_certification` | material_used_by_model: Material→Model |
| `biz.x.material_to_materialbalance` | `biz.x.model_to_changeovermatrix` | material_used_by_model: Material→Model |
| `biz.x.material_to_materialbalance` | `biz.x.model_to_finishedgoodsinventory` | material_used_by_model: Material→Model |
| `biz.x.material_to_materialbalance` | `biz.x.model_to_line` | material_used_by_model: Material→Model |
| `biz.x.material_to_materialbalance` | `biz.x.purchaseorder_to_incominginspection` | material_supplied_by_po: Material→PurchaseOrder |
| `biz.x.material_to_materialbalance` | `biz.x.qualitystandard_to_model` | material_used_by_model: Material→Model |
| `biz.x.material_to_materialbalance` | `biz.x.routing_to_model` | material_used_by_model: Material→Model |
| `biz.x.material_to_materialbalance` | `biz.x.workorder_to_model` | material_used_by_model: Material→Model |
| `biz.x.material_to_model` | `biz.x.operatorattendance_to_line` | model_certified_on: Model→Line |
| `biz.x.material_to_model` | `biz.x.order_to_customer` | model_demanded_by_order: Model→Order |
| `biz.x.material_to_model` | `biz.x.order_to_orderpromise` | model_demanded_by_order: Model→Order |
| `biz.x.material_to_model` | `biz.x.order_to_plantarget` | model_demanded_by_order: Model→Order |
| `biz.x.material_to_model` | `biz.x.orderpromise_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.material_to_model` | `biz.x.process_to_line` | model_certified_on: Model→Line |
| `biz.x.material_to_model` | `biz.x.purchaseorder_to_incominginspection` | material_supplied_by_po: Material→PurchaseOrder |
| `biz.x.material_to_model` | `biz.x.qualitylot_to_workorder` | wo_for_model: WorkOrder→Model |
| `biz.x.material_to_model` | `biz.x.shiftplan_to_line` | model_certified_on: Model→Line |
| `biz.x.material_to_model` | `biz.x.wiplot_to_line` | model_certified_on: Model→Line |
| `biz.x.material_to_model` | `biz.x.workorder_to_capacitypool` | wo_for_model: WorkOrder→Model |
| `biz.x.material_to_model` | `biz.x.workorder_to_line` | model_certified_on: Model→Line |
| `biz.x.material_to_model` | `biz.x.workorder_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.material_to_model` | `biz.x.workorder_to_qualitylot` | wo_for_model: WorkOrder→Model |
| `biz.x.metric_to_principal` | `biz.x.order_to_plantarget` | plantarget_ownedby: PlanTarget→Principal |
| `biz.x.model_to_base` | `biz.x.operatorattendance_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.model_to_base` | `biz.x.order_to_customer` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_base` | `biz.x.order_to_orderpromise` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_base` | `biz.x.order_to_plantarget` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_base` | `biz.x.orderpromise_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_base` | `biz.x.process_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.model_to_base` | `biz.x.qualitylot_to_workorder` | wo_for_model: WorkOrder→Model |
| `biz.x.model_to_base` | `biz.x.shiftplan_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.model_to_base` | `biz.x.wiplot_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.model_to_base` | `biz.x.workorder_to_capacitypool` | wo_for_model: WorkOrder→Model |
| `biz.x.model_to_base` | `biz.x.workorder_to_line` | line_belongs_to_base: Base→Line |
| `biz.x.model_to_base` | `biz.x.workorder_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_base` | `biz.x.workorder_to_qualitylot` | wo_for_model: WorkOrder→Model |
| `biz.x.model_to_certification` | `biz.x.operatorattendance_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_certification` | `biz.x.order_to_customer` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_certification` | `biz.x.order_to_orderpromise` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_certification` | `biz.x.order_to_plantarget` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_certification` | `biz.x.orderpromise_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_certification` | `biz.x.process_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_certification` | `biz.x.qualitylot_to_workorder` | wo_for_model: WorkOrder→Model |
| `biz.x.model_to_certification` | `biz.x.shiftplan_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_certification` | `biz.x.wiplot_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_certification` | `biz.x.workorder_to_capacitypool` | wo_for_model: WorkOrder→Model |
| `biz.x.model_to_certification` | `biz.x.workorder_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_certification` | `biz.x.workorder_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_certification` | `biz.x.workorder_to_qualitylot` | wo_for_model: WorkOrder→Model |
| `biz.x.model_to_changeovermatrix` | `biz.x.operatorattendance_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_changeovermatrix` | `biz.x.order_to_customer` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_changeovermatrix` | `biz.x.order_to_orderpromise` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_changeovermatrix` | `biz.x.order_to_plantarget` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_changeovermatrix` | `biz.x.orderpromise_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_changeovermatrix` | `biz.x.process_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_changeovermatrix` | `biz.x.qualitylot_to_workorder` | wo_for_model: WorkOrder→Model |
| `biz.x.model_to_changeovermatrix` | `biz.x.shiftplan_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_changeovermatrix` | `biz.x.wiplot_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_changeovermatrix` | `biz.x.workorder_to_capacitypool` | wo_for_model: WorkOrder→Model |
| `biz.x.model_to_changeovermatrix` | `biz.x.workorder_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_changeovermatrix` | `biz.x.workorder_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_changeovermatrix` | `biz.x.workorder_to_qualitylot` | wo_for_model: WorkOrder→Model |
| `biz.x.model_to_finishedgoodsinventory` | `biz.x.operatorattendance_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_finishedgoodsinventory` | `biz.x.order_to_customer` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_finishedgoodsinventory` | `biz.x.order_to_orderpromise` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_finishedgoodsinventory` | `biz.x.order_to_plantarget` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_finishedgoodsinventory` | `biz.x.orderpromise_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_finishedgoodsinventory` | `biz.x.process_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_finishedgoodsinventory` | `biz.x.qualitylot_to_workorder` | wo_for_model: WorkOrder→Model |
| `biz.x.model_to_finishedgoodsinventory` | `biz.x.shiftplan_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_finishedgoodsinventory` | `biz.x.wiplot_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_finishedgoodsinventory` | `biz.x.workorder_to_capacitypool` | wo_for_model: WorkOrder→Model |
| `biz.x.model_to_finishedgoodsinventory` | `biz.x.workorder_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_finishedgoodsinventory` | `biz.x.workorder_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_finishedgoodsinventory` | `biz.x.workorder_to_qualitylot` | wo_for_model: WorkOrder→Model |
| `biz.x.model_to_line` | `biz.x.order_to_customer` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_line` | `biz.x.order_to_orderpromise` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_line` | `biz.x.order_to_plantarget` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_line` | `biz.x.orderpromise_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_line` | `biz.x.process_to_equipment` | line_has_process: Line→Process |
| `biz.x.model_to_line` | `biz.x.qualitylot_to_workorder` | line_runs_work_order: Line→WorkOrder |
| `biz.x.model_to_line` | `biz.x.wiplot_to_defectrecord` | wip_on_line: WIPLot→Line |
| `biz.x.model_to_line` | `biz.x.wipqualitycheckpoint_to_wiplot` | wip_on_line: WIPLot→Line |
| `biz.x.model_to_line` | `biz.x.workorder_to_capacitypool` | has_capacity: Line→CapacityPool |
| `biz.x.model_to_line` | `biz.x.workorder_to_order` | line_runs_work_order: Line→WorkOrder |
| `biz.x.model_to_line` | `biz.x.workorder_to_qualitylot` | line_runs_work_order: Line→WorkOrder |
| `biz.x.model_to_material` | `biz.x.operatorattendance_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_material` | `biz.x.order_to_customer` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_material` | `biz.x.order_to_orderpromise` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_material` | `biz.x.order_to_plantarget` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_material` | `biz.x.orderpromise_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_material` | `biz.x.process_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_material` | `biz.x.purchaseorder_to_incominginspection` | material_supplied_by_po: Material→PurchaseOrder |
| `biz.x.model_to_material` | `biz.x.qualitylot_to_workorder` | wo_for_model: WorkOrder→Model |
| `biz.x.model_to_material` | `biz.x.shiftplan_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_material` | `biz.x.wiplot_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_material` | `biz.x.workorder_to_capacitypool` | wo_for_model: WorkOrder→Model |
| `biz.x.model_to_material` | `biz.x.workorder_to_line` | model_certified_on: Model→Line |
| `biz.x.model_to_material` | `biz.x.workorder_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.model_to_material` | `biz.x.workorder_to_qualitylot` | wo_for_model: WorkOrder→Model |
| `biz.x.operatorattendance_to_line` | `biz.x.process_to_equipment` | line_has_process: Line→Process |
| `biz.x.operatorattendance_to_line` | `biz.x.qualitylot_to_workorder` | line_runs_work_order: Line→WorkOrder |
| `biz.x.operatorattendance_to_line` | `biz.x.qualitystandard_to_model` | model_certified_on: Model→Line |
| `biz.x.operatorattendance_to_line` | `biz.x.routing_to_model` | model_certified_on: Model→Line |
| `biz.x.operatorattendance_to_line` | `biz.x.wiplot_to_defectrecord` | wip_on_line: WIPLot→Line |
| `biz.x.operatorattendance_to_line` | `biz.x.wipqualitycheckpoint_to_wiplot` | wip_on_line: WIPLot→Line |
| `biz.x.operatorattendance_to_line` | `biz.x.workorder_to_capacitypool` | has_capacity: Line→CapacityPool |
| `biz.x.operatorattendance_to_line` | `biz.x.workorder_to_model` | line_runs_work_order: Line→WorkOrder |
| `biz.x.operatorattendance_to_line` | `biz.x.workorder_to_order` | line_runs_work_order: Line→WorkOrder |
| `biz.x.operatorattendance_to_line` | `biz.x.workorder_to_qualitylot` | line_runs_work_order: Line→WorkOrder |
| `biz.x.order_to_customer` | `biz.x.plantarget_to_principal` | order_to_plantarget: Order→PlanTarget |
| `biz.x.order_to_customer` | `biz.x.qualitylot_to_workorder` | fulfills: WorkOrder→Order |
| `biz.x.order_to_customer` | `biz.x.qualitystandard_to_model` | model_demanded_by_order: Model→Order |
| `biz.x.order_to_customer` | `biz.x.routing_to_model` | model_demanded_by_order: Model→Order |
| `biz.x.order_to_customer` | `biz.x.workorder_to_capacitypool` | fulfills: WorkOrder→Order |
| `biz.x.order_to_customer` | `biz.x.workorder_to_line` | fulfills: WorkOrder→Order |
| `biz.x.order_to_customer` | `biz.x.workorder_to_model` | fulfills: WorkOrder→Order |
| `biz.x.order_to_customer` | `biz.x.workorder_to_qualitylot` | fulfills: WorkOrder→Order |
| `biz.x.order_to_orderpromise` | `biz.x.plantarget_to_principal` | order_to_plantarget: Order→PlanTarget |
| `biz.x.order_to_orderpromise` | `biz.x.qualitylot_to_workorder` | fulfills: WorkOrder→Order |
| `biz.x.order_to_orderpromise` | `biz.x.qualitystandard_to_model` | model_demanded_by_order: Model→Order |
| `biz.x.order_to_orderpromise` | `biz.x.routing_to_model` | model_demanded_by_order: Model→Order |
| `biz.x.order_to_orderpromise` | `biz.x.workorder_to_capacitypool` | fulfills: WorkOrder→Order |
| `biz.x.order_to_orderpromise` | `biz.x.workorder_to_line` | fulfills: WorkOrder→Order |
| `biz.x.order_to_orderpromise` | `biz.x.workorder_to_model` | fulfills: WorkOrder→Order |
| `biz.x.order_to_orderpromise` | `biz.x.workorder_to_qualitylot` | fulfills: WorkOrder→Order |
| `biz.x.order_to_plantarget` | `biz.x.qualitylot_to_workorder` | fulfills: WorkOrder→Order |
| `biz.x.order_to_plantarget` | `biz.x.qualitystandard_to_model` | model_demanded_by_order: Model→Order |
| `biz.x.order_to_plantarget` | `biz.x.routing_to_model` | model_demanded_by_order: Model→Order |
| `biz.x.order_to_plantarget` | `biz.x.workorder_to_capacitypool` | fulfills: WorkOrder→Order |
| `biz.x.order_to_plantarget` | `biz.x.workorder_to_line` | fulfills: WorkOrder→Order |
| `biz.x.order_to_plantarget` | `biz.x.workorder_to_model` | fulfills: WorkOrder→Order |
| `biz.x.order_to_plantarget` | `biz.x.workorder_to_qualitylot` | fulfills: WorkOrder→Order |
| `biz.x.orderpromise_to_order` | `biz.x.plantarget_to_principal` | order_to_plantarget: Order→PlanTarget |
| `biz.x.orderpromise_to_order` | `biz.x.qualitylot_to_workorder` | fulfills: WorkOrder→Order |
| `biz.x.orderpromise_to_order` | `biz.x.qualitystandard_to_model` | model_demanded_by_order: Model→Order |
| `biz.x.orderpromise_to_order` | `biz.x.routing_to_model` | model_demanded_by_order: Model→Order |
| `biz.x.orderpromise_to_order` | `biz.x.workorder_to_capacitypool` | fulfills: WorkOrder→Order |
| `biz.x.orderpromise_to_order` | `biz.x.workorder_to_line` | fulfills: WorkOrder→Order |
| `biz.x.orderpromise_to_order` | `biz.x.workorder_to_model` | fulfills: WorkOrder→Order |
| `biz.x.orderpromise_to_order` | `biz.x.workorder_to_qualitylot` | fulfills: WorkOrder→Order |
| `biz.x.plantarget_to_principal` | `biz.x.workorder_to_order` | order_to_plantarget: Order→PlanTarget |
| `biz.x.process_to_equipment` | `biz.x.shiftplan_to_line` | line_has_process: Line→Process |
| `biz.x.process_to_equipment` | `biz.x.wiplot_to_line` | line_has_process: Line→Process |
| `biz.x.process_to_equipment` | `biz.x.workorder_to_line` | line_has_process: Line→Process |
| `biz.x.process_to_line` | `biz.x.qualitylot_to_workorder` | line_runs_work_order: Line→WorkOrder |
| `biz.x.process_to_line` | `biz.x.qualitystandard_to_model` | model_certified_on: Model→Line |
| `biz.x.process_to_line` | `biz.x.routing_to_model` | model_certified_on: Model→Line |
| `biz.x.process_to_line` | `biz.x.wiplot_to_defectrecord` | wip_on_line: WIPLot→Line |
| `biz.x.process_to_line` | `biz.x.wipqualitycheckpoint_to_wiplot` | wip_on_line: WIPLot→Line |
| `biz.x.process_to_line` | `biz.x.workorder_to_capacitypool` | has_capacity: Line→CapacityPool |
| `biz.x.process_to_line` | `biz.x.workorder_to_model` | line_runs_work_order: Line→WorkOrder |
| `biz.x.process_to_line` | `biz.x.workorder_to_order` | line_runs_work_order: Line→WorkOrder |
| `biz.x.process_to_line` | `biz.x.workorder_to_qualitylot` | line_runs_work_order: Line→WorkOrder |
| `biz.x.qualitylot_to_workorder` | `biz.x.qualitystandard_to_model` | wo_for_model: WorkOrder→Model |
| `biz.x.qualitylot_to_workorder` | `biz.x.routing_to_model` | wo_for_model: WorkOrder→Model |
| `biz.x.qualitylot_to_workorder` | `biz.x.shiftplan_to_line` | line_runs_work_order: Line→WorkOrder |
| `biz.x.qualitylot_to_workorder` | `biz.x.wiplot_to_defectrecord` | defect_for_qlot: DefectRecord→QualityLot |
| `biz.x.qualitylot_to_workorder` | `biz.x.wiplot_to_line` | line_runs_work_order: Line→WorkOrder |
| `biz.x.qualitylot_to_workorder` | `biz.x.wipqualitycheckpoint_to_wiplot` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.qualitystandard_to_model` | `biz.x.shiftplan_to_line` | model_certified_on: Model→Line |
| `biz.x.qualitystandard_to_model` | `biz.x.wiplot_to_line` | model_certified_on: Model→Line |
| `biz.x.qualitystandard_to_model` | `biz.x.workorder_to_capacitypool` | wo_for_model: WorkOrder→Model |
| `biz.x.qualitystandard_to_model` | `biz.x.workorder_to_line` | model_certified_on: Model→Line |
| `biz.x.qualitystandard_to_model` | `biz.x.workorder_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.qualitystandard_to_model` | `biz.x.workorder_to_qualitylot` | wo_for_model: WorkOrder→Model |
| `biz.x.routing_to_model` | `biz.x.shiftplan_to_line` | model_certified_on: Model→Line |
| `biz.x.routing_to_model` | `biz.x.wiplot_to_line` | model_certified_on: Model→Line |
| `biz.x.routing_to_model` | `biz.x.workorder_to_capacitypool` | wo_for_model: WorkOrder→Model |
| `biz.x.routing_to_model` | `biz.x.workorder_to_line` | model_certified_on: Model→Line |
| `biz.x.routing_to_model` | `biz.x.workorder_to_order` | model_demanded_by_order: Model→Order |
| `biz.x.routing_to_model` | `biz.x.workorder_to_qualitylot` | wo_for_model: WorkOrder→Model |
| `biz.x.shiftplan_to_line` | `biz.x.wiplot_to_defectrecord` | wip_on_line: WIPLot→Line |
| `biz.x.shiftplan_to_line` | `biz.x.wipqualitycheckpoint_to_wiplot` | wip_on_line: WIPLot→Line |
| `biz.x.shiftplan_to_line` | `biz.x.workorder_to_capacitypool` | has_capacity: Line→CapacityPool |
| `biz.x.shiftplan_to_line` | `biz.x.workorder_to_model` | line_runs_work_order: Line→WorkOrder |
| `biz.x.shiftplan_to_line` | `biz.x.workorder_to_order` | line_runs_work_order: Line→WorkOrder |
| `biz.x.shiftplan_to_line` | `biz.x.workorder_to_qualitylot` | line_runs_work_order: Line→WorkOrder |
| `biz.x.wiplot_to_defectrecord` | `biz.x.workorder_to_capacitypool` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.wiplot_to_defectrecord` | `biz.x.workorder_to_line` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.wiplot_to_defectrecord` | `biz.x.workorder_to_model` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.wiplot_to_defectrecord` | `biz.x.workorder_to_order` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.wiplot_to_defectrecord` | `biz.x.workorder_to_qualitylot` | defect_for_qlot: DefectRecord→QualityLot |
| `biz.x.wiplot_to_line` | `biz.x.workorder_to_capacitypool` | has_capacity: Line→CapacityPool |
| `biz.x.wiplot_to_line` | `biz.x.workorder_to_model` | line_runs_work_order: Line→WorkOrder |
| `biz.x.wiplot_to_line` | `biz.x.workorder_to_order` | line_runs_work_order: Line→WorkOrder |
| `biz.x.wiplot_to_line` | `biz.x.workorder_to_qualitylot` | line_runs_work_order: Line→WorkOrder |
| `biz.x.wipqualitycheckpoint_to_wiplot` | `biz.x.workorder_to_capacitypool` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.wipqualitycheckpoint_to_wiplot` | `biz.x.workorder_to_line` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.wipqualitycheckpoint_to_wiplot` | `biz.x.workorder_to_model` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.wipqualitycheckpoint_to_wiplot` | `biz.x.workorder_to_order` | wip_for_wo: WIPLot→WorkOrder |
| `biz.x.wipqualitycheckpoint_to_wiplot` | `biz.x.workorder_to_qualitylot` | wip_for_wo: WIPLot→WorkOrder |
