...
> | 项 | 值 |
> |---|---|
> | 原基线 sha | `c45f1eba1167960085f12cb7686c4dae5af46f91`（2026-08-09 当日的 canonical） |
> | 距 canonical | **472 个提交**（canonical `9730a99f` @ 2026-08-13） |
> | 收编来源分支 | `claude/handoff-prd-coverage-full`（`2cdc882d7d`） |
> | 本次复验范围 | **只抽查 3 条，其余一条都未复验** |
>
> **⛔ 本文自己在开头就写了「所有 `file:line` 只对 `c45f1eb` 有效。换 commit 必须重取证」——
> 那条自我限定今天照旧生效，且已经过了 472 个提交。**
> 本文的价值主要在**方法学**（§0 的金丝雀矩阵与 5 个工具陷阱），逐份 PRD 的覆盖判定必须重取证后才可引用。
>
> | # | 原文断言 | 2026-08-13 实测 | 判定 |
> |---|---|---|---|
> | CF-1 | 覆盖 `docs/` 下**全部 147 份** PRD 类文档；`ls docs/*.md` = **237** 份 | canonical 树实测 `docs/*.md` = **286**（+49）；`docs/PRD-*.md` = **133**，`docs/industrial-prd/PRD-*.md` = **12**（合 145，与 147 差 2，口径未对齐） | ⚠️ 规模数字已过期·**「100% 覆盖」这个自称对今天不成立** |
>
> ⚠️ **CF-1 复验时我自己出过一次错，照铁律 0.6 记账**：第一版写的是 **294**，
> 那个数是在**已经把本单 8 份文档 checkout 进工作树之后**用 `ls docs/*.md` 数的 ——
> **把自己刚放进去的东西数进了「canonical 的现状」**。改用
> `git ls-tree -r --name-only <canonical> -- docs/` 直接数 canonical 树，得 **286**。
> 形态：「我用『工作树里的文件数』当作『canonical 的文件数』的证据，而前者并不度量后者。」
> | CF-2 | §0.3① 金丝雀 `registerRoutes` MISS ⇒ 当场报「工具坏了」，因为**本仓根本没有这个符号**（是凭印象编的） | 全仓 `apps packages scripts` 仍 **0 命中**（正控 `validateClosure`=29 命中，工具正常） | ✅ 完全成立·这条记账是真的 |
> | CF-3 | §0.3② `apps/datacore/src/pipeline/service.ts` 真实存在（差点被误报成死代码） | 文件仍在（8770 字节） | ✅ 完全成立 |
>
> **未重测的部分**：本文对 147 份 PRD 的逐份覆盖判定、实现率、死锚点计数，本次一条都没有重跑。
