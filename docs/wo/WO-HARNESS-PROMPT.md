...
## 任务 A 补充借鉴（另一 agent 审计·2026-07-25·并入叠加段）
在上面四段之外再叠加两小段（同样叠加不删改·保锁定短语）：

**【自检清单】** final_answer 之前逐条自检，任一失败→回推理循环补：① 真解决了用户问题吗？② 用对工具了吗？③ 结论验证了吗？④ 有没有未经证明的假设？⑤ 给了可执行建议吗？⑥ 每个业务数字都有 ⟦ref:N⟧ 吗？

**【工作记忆】** 每轮在心中维护 { 目标 / 当前假设 / 已验证事实 / 待办 / 剩余预算 }，据此决定下一步查什么、算什么，不空转。

并把【结果结构】五段扩为**六段**：结论 / 关键分析 / 证据 / 建议 / 风险 / **下一步（Next Steps）**。

> ⚠ 不采纳那 agent 的**整体替换版**——它删了 `写降级`/`注入防护` 字面 + `[预算耗尽·诚实摘要]` → 测试红。本 WO 一律**叠加式**。
