...
| # | 差异 | 登记处 | 分流口径 |
|---|---|---|---|
| W-1 | **B5 并行到达序**：并行工具调用按网络到达序各自 tryConsume，序不确定（dualrun 只声明串行预算场景） | RECONCILIATION §3 #11（W8主 落线回填 2026-08-21；team-lead 2026-08-21 归因订正）。文件锚：`apps/agentcore/src/server.ts` /b/v1/dsh/tool-execute 端点 · `apps/agentcore/src/engine.ts` dshToolExecuteRuns 登记 | 「并行工具调用到达序不一致」类工单 ⇒ 白名单；排序之外的**内容**差不许借本条目放行 |
...
| §5 W-1（B5 并行到达序） | **W8主** 落线回填（2026-08-21）——RECONCILIATION §3 #11 登记 + 文件锚 `server.ts` tool-execute 端点 / `engine.ts` dshToolExecuteRuns（team-lead 2026-08-21 归因订正：与 W5 块4 无关） |
...
- ❌ 没有给 W-1（B5 并行到达序）造文件锚——~~它现在是台账登记，锚落线前引用它必须连带本说明~~（**已回填**：W8主 2026-08-21 落线，锚 = RECONCILIATION §3 #11 + `server.ts` tool-execute 端点 / `engine.ts` dshToolExecuteRuns；本行保留作回填史）。
