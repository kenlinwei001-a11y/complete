...
| # | 差异 | 登记处 | 分流口径 |
|---|---|---|---|
| W-1 | **B5 并行到达序**：fan-out 并行场景下事件到达序两臂不保证一致 | 战役派单台账（W7 派单输入 #10）。⚠ 文件级 file:line 锚**尚未落线**（W5 块4 在途），落线后回填本格 | 「并行子任务事件顺序不一致」类工单 ⇒ 白名单；排序之外的**内容**差不许借本条目放行 |
...
| §5 W-1（B5 并行到达序） | **块4（W5 块4 在途）**——战役台账已登记，文件锚待落线回填（本单如实标注，不冒充已锚） |
