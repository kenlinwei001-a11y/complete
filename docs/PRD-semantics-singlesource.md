...
6. 键类型里**每一个**类型引用都必须解析得动（契约导入名 / 本文件可展开的 `type` 别名），
   **解析不动即不豁免**。这条是第 2 条的补漏，不补就留了个它看不见的洞：
   `Record<Rid | Loose, …>` 里 `Loose` 从别处 import 进来、定义很可能就是 `string`，
   而本文件的 AST 里根本不出现 `string` 这个关键字 ⇒ 只查第 2 条会把一张**已经被泡宽**的表当成锚定的放行。
...
「类型锚自检」10 段内嵌样本正反对拍：4 段该豁免（`Partial<Record<…>>` / 裸 `Record<…>` / `satisfies` / 别名链多跳）、
6 段**必须不豁免**（本地伪造 `= string` / 掺 `string` 泡宽 / `as` 断言 / 值位 / 非契约模块锚点 / 联合里掺解析不动的外来类型）。
任一条对不上 ⇒ 报「**门自己瞎了**」（与业务违规分开报，修法完全不同），不是「代码干净」。
正反两侧的**条数从样例表算**、不在通过行里手抄一个数 —— 治手抄的门自己手抄一个数是同一种病。
...
| ⑨ | 撤掉反伪造第 6 条（`!sawUnresolvable`） | 「**门自己瞎了**」 | ✅ 第 6 条反例样本单独报红（证明它不是恒真的摆设） |
