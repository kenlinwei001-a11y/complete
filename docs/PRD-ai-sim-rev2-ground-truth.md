...
⚠ **同时查到一处「很像真实、其实不是」的**：`apps/datacore/src/synthetic/service.ts:663-668`——
**合成播种器创建了 6 条 `rest_api` 类型的连接记录**（「CRM 订单」「IoT 时序通道」「MES 制造执行系统」…）。
**记录是真实类型，数据是合成的。** 判读这条线时必须分清「连接记录的类型」与「那条连接真跑过 sync」。
...
| 程 | 内容 | 出口判据 |
|---|---|---|
| **M0（新）实料地基** | **F1 F2 F3 · A11** | 回流面三入口可用且拒绝 SEED；`GET /calibration/debt` 带 `expected` 上屏且进 build；四项学习能力统一闸生效（变异反证过）；解释切片 ≤20 且 `coverage` 必填 |
