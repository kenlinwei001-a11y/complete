...
> **这份文档存在的理由**：`docs/ontology-graph/` 曾经在 git 里装着
> **53 份 YAML / 154,518 行**，同目录的**手写** markdown 只有 **384 行** ——
> 也就是说这个目录 **99.75% 是机器生成的快照**
> （连 `scripts/ontology-graph/**` 一起算，整个子系统 154,518 / 158,800 = **97.3%**）。
> 仓主的裁决是：
...
> 「这 6,273 个原子从哪来 · 21,520 条边分几种 · 三态怎么算出来的 · 这套抽取看不见什么」。

**本文所有数字均为本次实测**（`generatedFrom = 9ff14e15`，TypeScript 5.9.3，
**全仓 `pnpm -r build` 之后**抽取，耗时 18.1 s，峰值 RSS 2,210 MB）。

⚠ **两条必须先知道的事**，否则拿本文的数去对账会对不上：

1. **数字随代码库变。** 见 §8.3「怎么重新量」。⛔ 别把本文的数当常量引用，要引就引产生它的**命令**。
2. **边数还随「你 build 了哪几个包」变**（§1.4 有实测对照表）。
   只 build contracts+llm-adapters+datacore 时是 **21,032** 条边；全仓 build 是 **21,520** 条。
   差的 488 条全在 `registers` 上。**这不是不确定性，是可预测的依赖 —— 但它确实意味着
   「边有多少条」这句话必须带上 build 状态才完整。**
...
|---|---:|---:|---:|---:|---:|
| datacore | 559 | 198 | 361 | 4,190 ms | 694 MB |
| agentcore | 332 | 117 | 215 | 2,132 ms | 1,053 MB |
| frontend-shell | 624 | 302 | 322 | 5,205 ms | 1,790 MB |
| contracts | 101 | 92 | 9 | 761 ms | 1,907 MB |
| llm-adapters | 10 | 10 | 0 | 916 ms | 2,034 MB |
| **合计** | **1,626** | **719** | **907** | — | **峰值 2,210 MB** |
...
### 1.4 ⚠ `dist/` 是软前置，而且它**改变边的条数**

`registers` 的条目数**优先 import 已 build 的 `dist/` 真求值**，而不是数 AST。
没有 `dist/` 时它退化成 `confidence: "parsed"`（不可信），且**切片层整个跳过**。

**所以跑之前要先 build。⛔ 但别只 build 那三个包 —— 实测差 488 条边：**

| | 只 `contracts` + `llm-adapters` + `datacore` | 全仓 `pnpm -r build` | Δ |
|---|---:|---:|---:|
| 认出来的注册表 | 119 | **132** | **+13** |
| ├ `evaluated` | 47 | 49 | +2 |
| ├ `evaluated-uncountable` | 51 | **64** | **+13** |
| └ `parsed`（⛔ 不可信） | 21 | 19 | −2 |
| 可信的注册表 | 98 | **113** | +15 |
| `registers` 边 | 3,257 | **3,745** | **+488** |
| **总边数** | **21,032** | **21,520** | **+488** |

> **机制**：`agentcore` / `frontend-shell` 的 `dist/` 不存在时，它们那些
> 「AST 数不出来、只有求值数得出」的注册表（展开 `[...A,...B]` / 计算键 / `Object.keys(...)`）
> **根本不会被认成注册表** —— 不是"计数不准"，是**整张表连同它的条目一起不存在**。
> 屏上看不出缺了什么：`INDEX.registries` 就是短了 13 条，而它本来也没承诺自己有多长。
>
> 形态（CLAUDE.md 铁律 0.6 句式）：
> **「我用『README 说 dist 是软前置、退化成 parsed』当作『没 build 只是降可信度』的证据，
> 而前者并不度量后者 —— 真实后果是 13 张表连同 488 条边整个消失。」**

**推荐命令（本文全部数字都是在这条命令之后量的）**：
...
| `registries` | 注册表真值表（名字/坐标/两个计数/confidence/trustworthy） | 132（可信 113） |
...
本轮 **21,520 条**，分布如下（实测 `INDEX.counts.byEdgeKind`，另经 psql 独立复核逐种类相等）：

| 边 | 条数 | 占比 | `from → to` |
|---|---:|---:|---|
| `asserts` | 7,963 | 37.0% | `file:<测试文件>` → `field:<字段名>` |
| `declares` | 5,437 | 25.3% | 原子 → `field:<字段名>` |
| `registers` | 3,745 | 17.4% | 注册表原子 → `entry:<条目>` |
| `seeds` | 2,284 | 10.6% | `file:<种子文件>` → `field:<字段名>` |
| `inSlice` | 1,734 | 8.1% | 原子 → `slice:<sliceKey>` |
| `reads` | 266 | 1.2% | 原子 → `state:<路径>` |
| `reexports` | 91 | 0.4% | 薄壳原子 → 真实现原子 |
| **`calls`** | **不单列** | — | 见下 |

⚠ `registers` 这一行**随 build 状态变**（3,257 ↔ 3,745），见 §1.4。其余六种不随它变。
...
实测 **132 张**注册表，**可信 113**（`evaluated` 49 + `evaluated-uncountable` 64，另 19 张 `parsed` 不可信）；
那 113 张里 `registers` 边数 **== 真数组 `.length`**，**113/113 全中**（金丝雀 ⑯）。
`evaluated-uncountable` 实测 **1,649 条边** —— 这就是「AST 数不出来但确实存在」的规模。
...
| 14 | `evaluated` 置信度的 `registers` 边 | **`dist/` 没建**，全退化成 `parsed` | 1,839 |
| 15 | AST 数不出、靠求值拿到的 `registers` 边 | 求值这一步**没有鉴别力** ⇒「AST 与求值一致」不构成证据 | 1,649 |
| 16 | `registers` 边数 == 真数组 `.length` | 求值与发边两侧漂了 | 113/113 |
...
`evaluated-uncountable` 有 **1,649 条** ⇒ 求值确实在做 AST 做不到的事 ⇒ 对照才算数。
...
| 量 | 本轮实测（`9ff14e15`，全仓 build 后） |
|---|---:|
| 原子 | **6,273** |
| 边 | **21,520** |
| 切片 | **47**（域内 7 · 跨域 40） |
| YAML 产物 | **53 份 · 156,671 行 · 6.9 MB** |
| 扫描文件 | 1,626（719 src + 907 test） |
| 被声明的字段 / 其中有鉴别力的 | 2,265 / 1,993 |
| 抽取耗时 / 峰值内存 | 18.1 s / 2,210 MB |
...
⚠ 分片边合计 **19,786** ≠ 总边 **21,520**，差 **1,734** —— 那正是 `inSlice` 边：
它**不属于任何包**（没有 `pkg`），在 YAML 里只以切片的成员表形式隐含存在。
**落库那一份把它显式存了**，所以 `counts.edges` 这个数在数据库里第一次可以被逐条核对
（`SELECT kind, count(*) FROM ontograph_edge GROUP BY kind` 逐种类对上 `INDEX.byEdgeKind`）。
...
| `asserts` | 测试里的字符串数据键 | **最大的一种**（37.0% 的边） |
| `registers` | 注册表条目总数（⚠ 还受 build 状态影响，§1.4） | 0.60 边/原子 |
| `inSlice` | 切片数 × 平均成员数 | 1,734 / 47 ≈ 36.9 成员/切片 |
| YAML 体积 | ≈ 25.0 行/原子 | 6,273 × 25.0 ≈ 157 k |
...
```bash
pnpm -r build                                     # ⚠ 全仓，别只 build 三个包（§1.4：差 488 条边）
...
报数字的时候**连 build 状态一起报**，否则「边有 21,032 条」与「边有 21,520 条」
会看起来像两个人量错了一个，而实际上两个都对。
...
| `ontograph_edge` | 一条边（**含 `inSlice`**） | 21,520 |
| `ontograph_slice` | 一条切片 | 47 |

**为什么四张而不是一张**：四种东西的基数差三个数量级（1 / 47 / 6,273 / 21,520）。
...
> ⚠ 第二条**在边表上是必炸不是可能炸**：21,520 行 × 5 列 = **107,600 个绑定参数**，
> 而 pg 线协议上限是 int16 的 **65,535**。一次 INSERT 打完整批直接报
> `bind message has N parameter formats but M parameters`。
> ⛔ 所以 chunk 必须**按实际列数反算**（`floor(60000 / 列数)`）而不是写死行数 ——
> 写死 4,000 行在今天的 5 列表上安全，加一列就悄悄越界。
...
「21,520 条」落库变成「20,xxx 条」，**而两侧都不报错**。
...
| 条数不翻倍 | 同上，比三个基数 | 6,273 / 21,520 / 47 **不变** |
