...
- **触及不变量**（§5）：R1 · R3 · R13 · R14
- **触及断点**（§8）：（无特定断点）
- **范畴**：前端 Workspace Shell + 决策工作台：契约只读引用、entitlement、溯源、去电池
...
3. 必须先实现 **Mock 模式**（§9，MSW），使前端在两个后端都不在场时可完整开发与演示。
   ⚠️ **验收口径已于 2026-08-28 由仓主改定（原文：「以后所有 PRD 都配套真实测试，而不是功能测试」「从前端开始测试验证用真实后端数据，而不是跑冒烟数据测试」）**：
   **验收用例（§11）一律跑在真起的后端上**（datacore 内存模式 `SEED_DEMO=1`），**不再「默认 Mock 模式跑 + 少量真连冒烟」**。
   Mock 只保留两个职能：① 无后端时的开发/演示；② 单元测试里当替身。**Mock 下的结果不构成交付验证的依据。**
...
## 11. 验收标准（**真实后端**，playwright/vitest 自动化）

> **口径（2026-08-28 仓主改定）**：本节全部用例跑在**真起的 datacore**（内存模式 `SEED_DEMO=1`）+ 真前端上，
> **禁止设 `VITE_MOCK`**。理由是实测教训：只读源码或跑桩得出的量纲判断，被真实后端一次性推翻过 4 条
> （`Line.utilization` 判成 0–1 实为 92.55、两个 `samplingRate` 判成比率实为 [5,100]、`Material.devPct` 照名字判成 % 实为 0–1 小数）。
> **桩验不出这一类错——它把桩里的假设原样还给你。**
> 见 CLAUDE.md 铁律 1.5 判据三。
