...
| 序 | 动作 | 防住的**具体**故障 | 类 | 档位 |
|---:|---|---|:--:|:--:|
| **P0** | 补并 `datacore/test/modeling-provenance.test.ts` | 有人删/改 `synthetic/service.ts:1142-1148` 的 A 路回填 → **部署态 demo 的已发布本体重新大量"无来源"、数据源左栏重新"未建模"**，而四包 gate 全绿（唯一相关测试走 `chainMode=true` 的另一分支） | A | **S** |
