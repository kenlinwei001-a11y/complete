...
2. **病名「实现在正线、咬它的测试不在」对 22 个里只有 3 个成立**
   （`modeling-provenance` · `multi-intent-seam` 的 R7 残条 · `e2e-dialogue-acceptance` 的 S7 效果层）。
...
| 类 | 判据 | 修法 | 判定条目 | 涉及文件 |
|---|---|---|---:|---:|
| **A · 真无覆盖** | 实现在 canonical **且生产路径真走到**，但符号与行为在 `apps/*/test` 双双 0 | 补测试 | **2** | 2 |
| **B · 有覆盖但咬的是组件不是链路** | 测试直接 import 该模块断言，生产路径没被驱动 | 补**可达门** | **0** | 0（见下说明） |
| **C · 有覆盖但断言太弱** | 有等价测试，但少了具体分支/更强的判据 | 加强断言（补丁，不新增文件） | **4** | 4 |
| **D · 不该有测试 / 应丢弃** | 行为已被别处覆盖、或原文件与现语义冲突 | 从清单剔除并说明 | **2** | 2 |
| **E · ⛔ 实现也不在正线**（**新增类**） | 测试所依赖的符号/模块/迁移在 canonical 0 命中 | **不属本 WO** —— 这是"整单要不要并"的裁决 | **16** | 16 |

> **对账**：判定条目 24 > 文件 22，因为 `multi-intent-seam.test.ts` 一个文件跨 A / C / D 三类
> （R7 残条属 A、其余残条属 C、原文件本体属 D），按类各记一条。
> **去重后**：A∪C∪D = 6 个文件（`modeling-provenance` · `multi-intent-seam` · `e2e-dialogue-acceptance` ·
> `deterministic-multi-domain-seam` · `metric-aware-composition` · `tier2-bc-route`），E = 16 个文件，**6 + 16 = 22** ✓
