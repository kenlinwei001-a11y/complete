...
| **③ 样例形状** | factlock 门的 **13 条金丝雀全是单行、单实参、无尾逗号的手写样例**，与 prettier 格式化后的**生产形状交集为空** ⇒ 站点被静默跳过而门照绿。（同一个 `it` 里 4 行之下的同形态断言被咬中了，这条没有 —— 差别只在换行。） | **金丝雀样例的形状必须取自生产实物；手写样例可能与真实形状交集为空。** |
