...
### 第 5 条已达第 3 次的机制 —— **`$?` 必须紧跟被判的那条命令，中间不许隔任何东西**

**本文件已有一条「门必须显式捕获退出码」，但它把规则写窄了**：只点名了**管道**
（`cmd | tail -n; echo "EXIT=$?"` 取的是管道末端 `tail` 的退出码，恒 0）。
于是同族的其它形态一路裸奔 —— 2026-08-16 又栽一次：调度侧报「exit code 0」，
而日志里明明写着 `Test Files 1 failed`。**第 3 次，故推广。**

**⚠️ 先订正一个会误导人的病因（照 0.5 判据 5，实测过才敢写）**：
最初的复盘把元凶记成「**重定向**」（`cmd > log; echo "RC=$?"`）。**这是错的，照它修会修错方向。**
本机 bash 实测四种形态：

| 写法 | 实测 RC | 真相 |
|---|---|---|
| `false > log; echo "RC=$?"` | **1 ✅** | 重定向**不影响** `$?`；这条写法其实是**对的** |
| `false > log; echo "…"; echo "RC=$?"` | **0 ❌** | 中间隔了一条 `echo` ⇒ 取到的是**那条 echo** 的码 |
| `false \| tail -1; echo "RC=$?"` | **0 ❌** | 管道末端的码（既有条文点名的那个） |
| `out=$(false 2>&1); rc=$?` | **1 ✅** | 正解 |

**真机制不是「管道」也不是「重定向」，是「插队」**：`$?` 只保存**上一条**命令的退出码，
中间任何一条命令（哪怕只是 `echo`）都会把它冲掉。管道之所以出事，是因为
「上一条命令」是**整条管道**、而管道的码取自末端。

**来历（本条自己就是活标本）**：写这条戒律的同一次会话里，我在验一道门时写下
`node scripts/check-gate-roster-handcopied.mjs 2>&1 | head -3; echo "RC=$?"`，
当场打印出 **`RC=0`**，而那道门的真实退出码是 **1**。
**我正在为「$? 会骗人」写戒律，同时被 `$?` 骗了一次。**
改成 `node … > 文件 2>&1` 再单独 `rc=$?` 之后，结论从「干净」翻成「9 条违规」。

**形态**（照 0.6 句式）：
> **「我用『`$?` 打印出来的那个数』当作『我要判的那条命令的退出码』的证据，
> 而前者只度量了紧挨着它的那一条。」**

**判据（取代原来那条「不许用管道」）**：
> **`$?` 必须紧跟你要判的那条命令，中间不许隔任何东西 —— 包括 `echo`、`tail`、`|`、`&&`。**

**机制（唯一允许的写法，别的一律当没测）**：
```bash
out=$(cmd 2>&1); rc=$?          # 要拿输出
cmd > log 2>&1; rc=$?           # 输出太大就落盘，rc 仍紧跟 cmd
echo "RC=$rc"; grep -E 'error TS|FAIL|AssertionError' log   # 判完再打印/再筛
```
**反面判据**：报「RC=0 / 通过 / 全绿」之前，先自问「这个 0 是谁的 0？」——
答不上来就等于没测。失败时须打印 `error TS|FAIL|AssertionError` 原文，
不许只 `tail` 几行把错误挤掉。

### 第 6 条已达第 3 次的机制 —— **分支名不度量产出存亡；查存亡要按主题词全扫远端**

**来历**：2026-08-16 我（审核方）按**自己在工单里指定的**分支名去远端查，查不到，
于是宣布「四条分支零远端、**产出全丢**」，并据此给所有工单加了一条新纪律。
**实际那个 dev 用了他自己起的名** —— `wo-typecheck-testblind` vs 我指定的
`wo-test-typecheck-blind`，**只是词序相反**，4 个提交好好地躺在远端。
我不但错报了一次事故，还拿一个不存在的问题去改了所有人的流程。

**形态**（照 0.6 句式）：
> **「我用『我指定的那个分支名在远端不存在』当作『那个 dev 的产出丢了』的证据，
> 而前者只度量了我自己的命名习惯。」**

这条和「派单里判断分支对不对」（上面第 2 条）是**一对**：那次是拿**文件存在性**
当分支新旧的证据，这次是拿**分支名存在性**当产出存亡的证据 —— 同一个病的两面。

**判据**：dev 起什么名不受我控制，**唯一受我控制的是查法**。查产出存亡一律**按主题词全扫**：

```bash
git ls-remote origin 'refs/heads/*' | grep -iE '<主题词1|主题词2>'
```

主题词取工单名里**语义上最独特的那几个词根**（如 `typecheck`），
**不要**整串比对、**不要**假定词序。今日实测：远端 **549** 条分支，
`grep -iE 'typecheck'` 一次把两条同主题分支（我指定的名 + dev 自起的名）**都捞了出来**；
只查我指定的那一个名，就会漏掉另一条。

**机制（写进每次「产出是否丢了」的判断）**：三步，缺一步就不许下结论。
1. **全扫**：`git ls-remote origin 'refs/heads/*' > /tmp/remotes.txt; rc=$?`（`$?` 紧跟，见上一条）
2. **自证工具**：先 `grep -c` 一个**你确定存在**的主题词（如当前 canonical 的名字）。
   它若也报 0，那是网络/权限坏了，**不是分支没了** —— 只许报「我没查出来」。
3. **主题词模糊匹配**，命中为 0 才允许说「没推」。
   **零命中前必须同时给出第 2 步的金丝雀证据**（0.6 明令：否定结论必须自带度量范围）。

**反面判据**：`git ls-remote` 报 0 命中，和「那个 dev 没推」，是两个不同的命题。
中间隔着「他起了什么名」「网络通不通」「我 grep 的词对不对」三层，一层都不许跳。

## ⛔ 铁律 0.7 · **金丝雀只证明工具没瞎，不证明它扫对了、扫全了、扫的是真形状**（三层各有实证）

**铁律 0.6 落地的机制是「扫描类结论一律先跑金丝雀」。这条机制是对的，但它被读窄了** ——
读成了「金丝雀全中 = 这次扫描可信」。**2026-08-16 一天之内，三个不同的门/dev 各栽一层，
三次金丝雀都是全中的。** 第 3 次，故立此条。

**同一个病的三个深度：①扫的地方对不对 ②扫全了没有 ③样例像不像真的。**

| 层 | 实证（都是金丝雀全中的情况下出的事） | 一句话 |
|---|---|---|
| **① 扫描面** | 一个 dev grep 某句话，在**分支**上零命中，就判「这话不是 POC 说的」。实际那句在 POC 的**报告**里 —— **报告根本不在分支上**。当时 grep 是好的、金丝雀也中。 | **金丝雀证明的是工具没瞎，不是扫描面选对了。扫描面选错时，金丝雀会陪着你一起给出一个自信的错误答案。** |
| **② 覆盖率** | 接缝门的类型参数正则漏掉 `{`，而 `api.a<{…}>(…)` 这种写法有 **126 处** ⇒ **367** 个调用位只认出 **231**，少的 **133** 处全落进「方法未知」分支 = **对所有方法豁免**。**当时金丝雀 21/21 全中**。抓住它的是**拿独立口径对总数**。 | **金丝雀全中也不保证抽取器覆盖全 —— 覆盖率要用独立口径对总数，不能靠样例。** |
| **③ 样例形状** | factlock 门的 **12 条金丝雀全是单行、单实参、无尾逗号的手写样例**，与 prettier 格式化后的**生产形状交集为空** ⇒ 站点被静默跳过而门照绿（后补的 C7 补上生产形状）。（同一个 `it` 里 4 行之下的同形态断言被咬中了，这条没有 —— 差别只在换行。） | **金丝雀样例的形状必须取自生产实物；手写样例可能与真实形状交集为空。** |

**形态**（照 0.6 句式，三层共用一句）：
> **「我用『金丝雀全中』当作『这次扫描的结论可信』的证据，而前者只度量了
> 『在我想得到的那几个输入上，代码没写错』。」**

### 机制：三层各配一件，**缺一层就是缺一层**（新门评审逐层核，少一层退回）

- **① 扫描面自证**：门里必须有一段**与金丝雀分开**的代码，断言
  **「本该必在扫描面里的那几个对象，真的在」**（锚点必中）+ **扫描面规模下界**。
  不满足 ⇒ 报 **RC=2「工具坏了」**，**不许**报「仓库很干净」。
  **实证（2026-08-17 建 `md-section-dup` 门时机器复现的）**：把受检目录从 `docs` 写成 `doc`，
  **金丝雀 13/13 全中**，而受检集塌成 **0 份** —— 若只有金丝雀，门会一本正经地打印
  「受检 md **0** 份 … ✓ 无同名小节」。只有锚点断言拦住了它。
- **② 覆盖率用独立口径对总数**：另取一条**结构上不同**的代码路径数同一个总数，
  两者的差额必须**逐条解释干净**，剩一条都算漏。
  ⚠️ **但这条自己也有个坑，实测过**：`md-section-dup` 的守恒式
  `裸标题数 = 留下 + 围栏内被抑制`（当日 485 = 477 + 8）看着在守，其实**在今天的语料上是半瞎的** ——
  语料里 6 级标题 **0** 条，两个口径唯一会分歧的地方生产上一次都走不到；
  把解析正则从 `#{1,6}` 改窄成 `#{1,5}`（= 静默丢掉所有 6 级标题），
  **15 份文档守恒照样成立、门照样绿**。
  ⇒ **「守恒式成立」不度量「覆盖全了」，它只度量「在今天的语料上两个口径没分歧」。**
  修法是给那条分歧路径**补一条金丝雀**，让它在样例里走一遍（补完同一变异当场 RC=2）。
- **③ 样例形状取自生产实物**：金丝雀里必须有 `real:` 一档 —— **直接拿仓里的真文件跑主逻辑**，
  断言已知为真的事实（`check-gate-exit-discipline.mjs` 拿 `check-ui-first-layer.mjs`、
  `md-section-dup` 拿 `docs/SYSTEM-ONTOLOGY.md`）。
  判据：**手写样例过了，只说明「我想象中的形状」能过**。生产真形状必须**亲自喂一遍**。

**报否定结论时的强制格式**（0.6「否定结论必须自带金丝雀证据」的加强版）：
> 报「0 命中 / 不存在 / 没有违规」时，必须同时给出**三样**：
> ① 金丝雀命中证据 ② **扫描面是什么**（扫了哪些对象、多少个）③ **覆盖率怎么对上的**（独立口径的数）。
> 只给①的，等于只说了「我的工具没坏」，没说「我查过了」。
...
2. **先判机器，再判任务。** `uptime` 小于静默时长 → 容器刚重启 → 全体**阵亡**而非卡死，处置方向完全不同：
   ...
   ⚠️ **查法有讲究，写窄了会错报事故**：**不许**只查「我在工单里指定的那个分支名」——
   dev 用自己起的名是常态（实测 `wo-typecheck-testblind` vs 我指定的 `wo-test-typecheck-blind`，
   **只是词序相反**，我据此宣布「产出全丢」，而 4 个提交好好在远端）。
   必须 **按主题词全扫**：`git ls-remote origin 'refs/heads/*' | grep -iE '<主题词>'`。
   详见**铁律 0.6 第 6 条**（含「先自证工具再报零命中」的三步机制）。
3. **四种态各有处置，不许混为一谈**：
...
- **门必须显式捕获退出码（违反即事故·已真实发生）**：交付门一律走 `bash scripts/gate.sh`。**禁止** `cmd | tail -n; echo "EXIT=$?"` —— `$?` 取的是管道末端 `tail` 的退出码（恒 0），曾据此把一个 **agentcore 编译失败**的 commit 判为"BUILD 通过"并入正线，直到部署方 build 失败才暴露（错误原文当时就在日志里，被假绿盖过）。失败时须打印 `error TS|FAIL|AssertionError` 原文，不许只 tail 几行把错误挤掉。
  ⚠️ **这条只点名了管道，把规则写窄了，同族形态一路裸奔到 2026-08-16 又栽一次**。已推广并订正病因（**元凶是「插队」不是「管道」也不是「重定向」**，四种形态各有实测）：见**铁律 0.6 第 5 条**。判据一句话：**`$?` 必须紧跟你要判的那条命令，中间不许隔任何东西**；唯一允许的写法是 `out=$(cmd 2>&1); rc=$?`。
