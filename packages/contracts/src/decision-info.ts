 ...
 *
 * ══ WO-CAPSIM-THREE-SLOTS · 为什么这里要拆成两个字段 ══════════════════════════
 * 修前：**结论与它的证明过程被拼成一个 `reason` 字符串**（实测 `penalty.reason` 316 字），
 * 前端拿到的只有一坨文本 ⇒ **想降层也降不了**，只能整段摆在第一层。
 * 仓主两次点名的正是这个后果：「点击每个卡片，瀑布式展示非常多信息，没有重点」。
 *
 * 判据（仓主定的那把尺，不是"重要/次要"）：
 *   | | 决策信息 | 可信度与溯源 |
 *   |何时要| **做决定那一刻** | **怀疑这个数那一刻** |
 ...
 *   |篇幅| 越短越好 | **越全越好** |
 * 两者最优篇幅方向**相反** —— 放同一层必然打架。故拆：
 *   · `verdict` = 决策信息 → **第一层**，一句话，不含证明过程
 *   · `reason` / `missingFields` / `checked` = 可信度与溯源 → **第二层**，越全越好
 *
 * ⚠ **为什么不把 `reason` 改名成 `evidence`**（这是刻意的，不是偷懒）：
 * 旧名会以**字符串形态**存在于测试对象字面量 / `toContain` 期望串 / mock 数据键里，
 * `typecheck` 一个都看不见（CLAUDE.md 铁律 0.6 第 4 条，本仓已因此红过三次）。
 * 而真正的缺陷是「**压根没有短结论字段**」，不是「`reason` 名字不对」——
 * 加字段零破坏、证据零丢失、**降层守恒天然成立**（一个字都没删，只多了一句结论）。
 */
export const MissingEvidenceSchema = z.object({
  status: z.literal("EMPTY"),
  /**
   * **第一层的短结论**（决策信息）：一句话说清「这个格子为什么空着」，**不含证明过程**。
   * 形如「违约金：算不出（本体无承载）」。屏上配 08-28 设计稿的删除线记号
   * （`docs/design/UI-sim-console-20260828.html` 页脚：**删除线 = 这次算不出来，不是 0，
   * 也不是「无变化」**）—— 记号本身承载「不是 0」这个断言，不必每个空格子重复一段话。
   */
  verdict: z.string(),
  /** **第二层的长证明**（可信度与溯源）：凭什么说算不出。越全越好，⛔ 不许为了短而删。 */
  reason: z.string(),
  /** 补哪些字段才能点亮。⚠ 这是**建模负责人**的决策信息，不是运营的 —— 屏上归属见前端注释。 */
