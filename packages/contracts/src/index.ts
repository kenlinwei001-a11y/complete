...
export * from "./process-instance.js"; // WO-FLOWTIME · 业务流程**实例**层（ProcessInstance + 站间流转时长）：从既有带时间戳单据**反推**（origin=DERIVED_FROM_DOCUMENT，每条溯回具体单据 id+字段+原值），补上 impact-analysis 自述的「答不出哪一条实例卡住/卡在谁那里/卡了多久」；waitState 复用 PROCESS_WAIT_KINDS；⛔ 一次都不读 stdDurationDays（不拿标准工期冒充实测）
