...
export * from "./process-runtime.js"; // WO-PROCESS-INSTANCE · 流程**运行时**层（模板层之上另一层，不是它的一半）：ProcessInstance/ProcessTask 八字段（Start/End/Duration/Owner/Status/Input/Output/Decision）；五个等待态由 PROCESS_WAIT_KINDS **派生**+WAITING_APPROVAL（承载物=S2 ActionDraft，模板层四值一字未动）；evaluateGate() 是五态唯一产地、时钟注入（R6·欠账 #141）
