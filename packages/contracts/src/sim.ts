...
/**
 * ⛔ **为什么要收 `equal_share`（WO-SIM-DESAT-3 实测立此账）—— 它补的是「没有差异化计量值」那一档**
 *
 * 上面四条口径都要求本体里**拿得到一个差异化的计量值**（BOM 成本占比 / 源数量 / 源金额 / 敞口）。
 * 拿不到时，今天的落法是 `weightRef: null` —— 而 `null` **不是「不分摊」，是「每条入边各加一份满额」**：
 * `combine:"sum"` 对每个源实例各累一次，⇒ **Σ 权重 = N**，入流随源的条数线性膨胀。
 *
 * **今天的行为 X（修前实测·真后端 `SEED_DEMO=1`·第 24 拍 tick 回执）**：
 * `Model.costPressure` 的两条入边系数几乎一样（0.5 vs 0.65），**每目标入流差 51 倍**——
 *  · `demo_material_price_to_model_cost`（`bom_cost_share` ⇒ Σw=1）：42 条边 → 每目标 **42.44**；
 *  · `demo_wo_release_to_model_cost`（`weightRef: null` ⇒ Σw=N）：**43.3 个工单各加一份满额** → 每目标 **2162.42**。
 * ⇒ 「**工单越多，型号成本压力越大**，而每个工单的受阻程度根本没变」——
 * 正是上表「强度型目标用 MEAN 会怎样」那一行点名的错，只是它以 `null` 的形态出现，**判据表覆盖不到**。
 *
 * **应该的 Y**：强度型目标该收**加权平均**（Σ=1）。分不出轻重时，等份就是那个诚实的平均。
 *
 * ⚠ **为什么不复用 `source_qty_relative` 之类「凑一个计量值」**：那会**编造**一个本体里没有的轻重排序
 * （工单没有可比的"大小"字段）。本口径把「答不出谁更重要」如实表达成"都一样"，
 * 而不是挂着"已按 X 分摊"的名义跑一个编出来的 X —— 那是本仓反复治的那种最难查的假绿。
 *
 * ⚠ **它只配强度型目标**。广延型（`demandLoad`「单越多负荷越大」）**绝不能**用本口径：
 * Σ=1 会把 N 张单塌缩成一张的平均，把"负载"重新定义成均值（上表第二行实测过：8 → 0.0989）。
 */
