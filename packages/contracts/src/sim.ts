 ...
 *
 * 取值 = 200 的理由：实测每条指标在 2 格窗口下约 **286 字节**（116,859,540 ÷ 408,528），
 * 200 条 ≈ 57KB；即便窗口拉到 100 格也在数百 KB 量级，**且与世界里有多少对象无关** ——
 * 这正是本闸要的性质：**回包规模由请求决定，不由世界规模决定**。
 * 200 同时给下钻留了余量（白名单筛出某个基地的全部产线仍够用）。
 */
export const SIM_METRIC_SERIES_MAX_LIMIT = 200;
