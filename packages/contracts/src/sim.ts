 ...
 *    它补的是「本体里没有差异化计量值」那一档**
 *
 * 上面四条口径都要求本体里**拿得到一个差异化的计量值**（BOM 成本占比 / 源数量 / 源金额 / 敞口）。
 * 拿不到时，今天的落法是 `weightRef: null` —— 而 `null` **不是「不分摊」，是「每条入边各加一份满额」**：
 * `combine:"sum"` 对每个源实例各累一次 ⇒ **Σ 权重 = N**，入流随源的**条数**线性膨胀。
 *
 * **今天的行为 X（实测·第 24 拍）**：`Model.costPressure` 的两条入边系数几乎一样（0.5 vs 0.65），
 * 而**每目标入流差 51 倍** ——
 *  · `demo_material_price_to_model_cost`（`bom_cost_share` ⇒ Σw=1）：每目标 **42.44**；
 *  · `demo_wo_release_to_model_cost`（`weightRef: null` ⇒ Σw=N）：43.3 个工单各加一份满额 → **2162.42**。
 ...
 * （工单之间没有可比的"大小"字段）。本口径把「答不出谁更重要」如实表达成"都一样"，
 * 而不是挂着"已按 X 分摊"的名义跑一个编出来的 X。
 *
 * ⚠⚠ **它只配强度型目标。广延型绝不能用** —— `Model.demandLoad`（「单越多负荷越大」）是广延量，
 * Σ=1 会把 N 张单塌缩成一张的平均，把"负载"重新定义成均值（实测：8 → 0.0989）。
 * **`demandLoad` 的两条入边因此保持 `source_qty_relative`（IN_EDGES_MEAN·Σw=N），
 * 由每格增益预算去压系数，而不是改归一口径** —— 两者修的不是同一件事，换错了会把量纲改掉。
 */
