...
export const PAIR_WEIGHT_BASIS_REGISTRY = [
  ...
  },
  {
    ...
    measure:
      ...
      "唯一但决定性的区别，见 `PairWeightNormalize` 上方第三行。",
  },
  {
  ...
  {
    key: "target_on_hand_gate",
    normalize: "NONE",
    measure:
      "目标订单**是否还在手**（`isOnHandOrderStatus(Order.status)`）：在手 ⇒ 1，已交付关闭 ⇒ 0。" +
      "这是**闸门不是份额** —— 取值只有 0/1，不归一、不在同组之间重新分配，" +
      "故 `normalize:\"NONE\"`（权重原样进 `amount = 强度 × 权重 × 源态`）。",
  },
] as const;

/**
 * ⛔ **为什么要有第四种归一方向 `NONE`（WO-PRESSURE-TO-MONEY 实测立此账）**
 *
 * ── 今天的行为是 X（实测·真后端 SEED_DEMO=1·seed 42）────────────────────────────
 * 推演层**根本不看订单状态**：`propagation.ts` 的 `baseAmount = coeff × drive × factor`
 * 没有状态项，`propagation-inputs.ts` 建图时 `listByType` **不按状态筛**，
 * `deriveSeedBaseSnapshot` 也只滤 `mergedInto`。实测 **500/500 张单**（含 350 张
 * `COMPLETED`）全都挂着 `costPressure` 格子 ⇒ 一次原料涨价把**已交付关闭**的单也推了一遍。
 * 屏上「被推动 500 张 · 占订单簿 100.0%」不是巧合，是必然。
 * 而这 350 张占订单簿金额的 **65.55%（298.01 亿 / 454.64 亿）** ——
 * 拿这个敞口当分母去算毛利，得到的是**精确的错数**，比「算不出来」更坏。
 *
 * ── 应该是 Y ──────────────────────────────────────────────────────────────────
 * `COMPLETED` 的本体语义（`order-status.ts` 原文）是「**已交付关闭，不再占用产能、
 * 不再需要承诺**」⇒ 料已耗用、成本已锁定、钱已结 ⇒ 涨价**不该**再推它。
 *
 * ── ⛔ 为什么这**不是**内联一个业务常数（RL5 红线）────────────────────────────────
 * 判据不是拍出来的 0/0.5/1 三档，而是**复用既有登记册** `ON_HAND_ORDER_STATUSES`
 * （`order-status.ts`，原文「在手 = 全簿 − 已完成」，且明令「逐处 `!== "COMPLETED"`
 * 的字面量比较一律换成 `isOnHandOrderStatus`」）。本口径只是把那条既有谓词接进传导层，
 * **没有发明任何新系数** —— 0/1 是谓词的真假值，不是强度参数。
 *
 * ── ⚠ 已知缺口：`IN_PRODUCTION` 今天按**全额**推（不是三档）──────────────────────
 * 业务上在制单应当「已投料的锁定、未投料的受影响」⇒ 需要一个「已投料比例」。
 * 实测该数据**全仓零出处**（`issuedRatio|materialIssued|已投料|投料比例|wipRatio|consumedRatio`
 * 六个词在 `apps/datacore/src` + `packages/contracts/src` **0 命中**；
 * 金丝雀：同范围 `outsourceRatio` 命中 20 ⇒ 搜法是活的）。
 * 故本口径**只做 0/1 这一档**（`COMPLETED ⇒ 0`），**不为第三档编一个系数** ——
 * 那正是本仓 RL5 要防的那件事。缺口留在这里，不留在一个拍出来的 0.5 上。
 *
 * ── 为什么必须是 `NONE` 而不能复用 `IN_EDGES*` ─────────────────────────────────
 * ① **量纲**：`Model.costPressure --model_demanded_by_order--> Order.costPressure` 是
 *    **1:N 出边**，目标拿到的是同一个**率**；`weightRef` 字段注释原文「一源分多目标时…
 *    分摊它反而是把一个率切成几块，量纲不成立」。闸门不切这个率，只决定**该不该落**。
 * ② **数学**：该边每张订单只有**一条**入边 ⇒ `IN_EDGES` 给 `measure/measure = 1`、
 *    `IN_EDGES_MEAN` 给 1（见上方 `SOURCE_POOL_MEAN` 那段的同一条退化情形）——
 *    组内归一会把 0/1 闸门**归一回 1**，闸门当场失效。
 * ③ **不许重新分配**：若把已完成单的份额摊给剩下的单（任何归一都会这么做），
 *    总量不变、只是换人承担 —— 而本条要的是**总量真的变小**（那 350 张单本就不该被推）。
 */
 ...
 * | **敞口**（金额暴露：`receivablePressure` 是"这个客户压着我多少钱"） | `Order→Customer` 应收 | `IN_EDGES_GLOBAL_MEAN`（分母=**全域**均值） | 用组内均值会让读数退回**按条数**算 —— 见下方 WO-EDGE-MONEY-WEIGHT 那一段 |
 *
 * 本单第一版把 `Order→Model` 也做成了 Σ=1，实测把 `Model.demandLoad` 从 8 打到 0.0989
 * （≈ 1/83）—— 那不是"按用量加权"，那是把负载重新定义成了平均值。
 * **判据一句话：目标格子回答的是「多快/多高」「多少」还是「多少钱」？**
 * 「多快」用 Σ=1，「多少」用组内均值=1，「多少钱」用**全域**均值=1。
 *
 * ── ⛔ 第三种归一为什么非有不可（WO-EDGE-MONEY-WEIGHT · 实测逼出来的，不是补全枚举）──────
 * **今天的行为 X**（修前实测·真后端 SEED_DEMO=1·seed 42·三元正极 +15%·tick×4）：
 * `Order.costPressure --order_of_customer--> Customer.receivablePressure` 这条边
 * `weightRef: null` ⇒ 每张单落**同一个额**，客户读数 = `0.5 × Σ(逐单成本压力)`。
 * 于是**四个金额天差地别的客户拿到逐字节相同的 15.137334**：
 * 东风 10.02 亿(4 单) · 深蓝 8.32 亿(7 单) · 上汽通用五菱 7.19 亿(7 单) · 零跑 5.78 亿(8 单)。
 * 按它排「先催谁的款」= **按单数排**。全表「压力 ÷ 亿元」跨度 1.07–5.01 = **4.68× 口径失真**。
 * **应该的 Y**：压 10 亿的客户，应收压力就该比压 5.78 亿的客户大约 1.73 倍。
 *
 * ── 为什么两种既有归一**都不行**（这一段是判据，别按"少一个枚举更简洁"合并回去）──────
 * 设该客户名下 n 张单、金额 vᵢ、逐单成本压力恒为 c（同型号的单确实如此）：
 *  · `IN_EDGES`（Σ=1）      ⇒ 读数 = `0.5·c`，**与 n 无关也与金额无关** —— 全部客户同一个数，
 *    连"谁的敞口大"都答不了，比修前更糟。
 *  · `IN_EDGES_MEAN`（组内均值=1）⇒ Σ权重 ≡ n ⇒ 读数 = `0.5·c·n`，**逐字节等于修前** ——
 *    挂着"已按金额分摊"的名义跑的还是按条数算。这正是本仓反复治的那种**最难查的假绿**。
 *  · `IN_EDGES_GLOBAL_MEAN`（全域均值=1）⇒ Σ权重 = `Σvᵢ ÷ 全域均值` ∝ **该客户的金额敞口**
 *    ⇒ 读数 = `0.5·c·(客户金额 ÷ 全域均值)`。**只有这一种同时满足两条验收**：
 *    ① 金额不同、条数相同 ⇒ 读数按金额拉开；② 金额相同、条数不同 ⇒ 读数相等。
 *
 * ── 分母为什么必须是**全域**基数，不是承载集 ────────────────────────────────────
 * 组内归一（`IN_EDGES` / `IN_EDGES_MEAN`）的分母随"这个目标手上有几条边"变，
 * 于是**同一张单在大客户名下和在小客户名下权重不同**，跨目标的读数不可比 ——
 * 而"先催谁的款"恰恰是一个**跨目标比较**的问题。全域分母让权重成为订单自己的属性
 * （这张单占全域平均单的几倍），与它挂在谁名下无关，跨目标因而可比。
 * 同源判据本仓已有先例：`bom_cost_share` 的分母取**整份 BOM**而不是图里现有入边之和，
 * 理由逐字相同 —— 「按现有入边重新归一会让『加一条链路』悄悄改掉其它每一条的权重」。
 *
 * ⚠ **全域分母同时保住了总量**：Σ over 全体源实例的权重 ≡ 实例条数 N，
 * 故换上本口径后世界的总体量级不跳（实测 358 条边权重合计 356.2 vs 修前 358，差 0.5%）。
 */
